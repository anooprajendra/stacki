...
def usage():
    ...
    logger.error(
        "\t--use-existing : use the existing system settings and root password"
    )
...
opts, args = getopt.getopt(
    sys.argv[1:], "", ["stacki-iso=", "extra-iso=", "use-existing"]
)
...
for new_pkg in new_pkgs:
    if osname == "redhat" and subprocess.call(["yum", "info", new_pkg]) == 0:
        pkgs.append(new_pkg)
    elif osname == "sles" and "not found" not in str(
        run(["zypper", "info", new_pkg]).stdout
    ):
        pkgs.append(new_pkg)
...
if not os.path.exists("/tmp/site.attrs") and not os.path.exists("/tmp/rolls.xml"):
    if use_existing:
        ...
        match = re.match(
            r"\d+:\s+\S+\s+inet\s+([\d.]+)/(\d+)\s+brd\s+([\d.]+)", interface[1]
        )
        if match:
        ...
        inet_address = struct.unpack("!I", socket.inet_aton(attrs["NETWORK_ADDRESS"]))[
            0
        ]
        ...
        for line in run("ip -o link", shell=True, text=True).stdout.splitlines():
            if line.split(":")[1].strip() == interface[0]:
                attrs["MAC_ADDRESS"] = re.search(
                    r"link/ether\s+([0-9a-f:]{17})\s+", line
                ).group(1)
                break
        else:
        ...
        if len(gateways) > 1:
            ...
            for ndx, gateway in enumerate(gateways):
                logger.info(
                    "  %s) %s %s", ndx + 1, gateway[0], gateway[1],
                )
...
if not use_existing:
    ...
    run_and_warn(["usermod", "-p", attributes["Kickstart_PrivateRootPassword"], "root"])
