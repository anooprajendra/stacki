...
class App:
    def __init__(self):
        ...
        parser.add_argument(
            "--update-pkgs", help="RPM packages to add to /updates in initrd.img"
        )
    ...
    def overlaypkgs(self, pkgs, update):
        for pkg in pkgs:
            ...
            self.boot.applyRPM(
                RPM, os.path.join(os.getcwd(), pkg), flags="--noscripts --excludedocs"
            )
            ...
            os.system("cd %s ; find . -not -type d | cpio -pdu %s" % (pkg, destdir))
