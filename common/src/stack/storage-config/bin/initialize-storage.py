...
def _exec(
    ...
    **kwargs,
    ...
    return subprocess.run(
        cmd, **kwargs, stdout=stdout, stderr=stderr, encoding=encoding
    )
...
def nukeLVM(volumegroup):
    for (display, remove) in [
        ...
        p = subprocess.run(shlex.split(display), stdout=subprocess.PIPE, stderr=FNULL)
        for line in p.stdout.decode().split():
            l = line.split(":")
            if len(l) > 1 and (volumegroup == "*" or volumegroup == l[1]):

                target = l[0]
                cmd = "%s %s" % (remove, target)
                subprocess.call(
                    shlex.split(cmd), stdout=FNULL, stderr=subprocess.STDOUT
                )
...
def stopMD():
    ...
    #
    cmd = "mdadm --stop --scan"
    subprocess.call(shlex.split(cmd), stdout=FNULL, stderr=subprocess.STDOUT)
