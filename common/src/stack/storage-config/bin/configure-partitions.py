...
def doOutputExistingLVM(name, fstab):
    ...
    p = subprocess.run(
        ["/sbin/lvs", "-o", "vg_name,lv_name,dm_path", "--noheadings", "--separator=:"],
    ...
    for p in partitions:
        if p["mountpoint"] == mountpoint:
            if md_re.match(p["device"]):
                ...
                for disk in disks:
                    if disk["device"] == p["device"] and disk["nuke"]:
                        return 1
...
def getMDInfo(device):
    ...
    p = subprocess.run(
        ["mdadm", "--detail", "--export", "/dev/%s" % device],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
