...
class CLI:
    ...
    def doNuke(self, enclosure, adapter):
        ...
        for slot in self.getSlots(adapter):
            self.run(["%s/s%d" % (adapteraddress, slot), "set", "good", "force"])

    def doSecureErase(self, enclosure, adapter, slot):
        if enclosure:
            slotaddress = "/c%d/e%d/s%d" % (adapter, enclosure, slot)
        else:
        ...
        if res["Command Status"]["Status"] == "Failure":
            return
        s = res["Response Data"]["Drive %s - Detailed Information" % slotaddress][
            "Drive %s Policies/Settings" % slotaddress
    ...
    def getSlots(self, adapter):
        ...
        if res["Command Status"]["Status"] == "Failure":
            ...
            if eid:
                res = self.run(["/c%d/e%d/sall" % (adapter, eid), "show"])
    ...
    def doRaid(self, raidlevel, adapter, enclosure, slots, hotspares, flags, check):
        args = ["/c%d" % adapter, "add", "vd", "type=r%s" % raidlevel]
