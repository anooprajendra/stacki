...
class CLI:
    ...
    def doNuke(self, enclosure, adapter):
        ...
        self.run(["-AdpSetProp", "BootWithPinnedCache", "1", "-a%d" % adapter])
    ...
    def doStrippedRaid(
        self, raidlevel, adapter, enclosure, slots, hotspares, flags, check
    ):
        ...
        if vid:
            for f in setpropflags:
                cmd = ["-LDSetProp", f, "-L%d" % vid, "-a%d" % adapter]
                results = self.run(cmd, check=check)
        ...
        #
        self.doGlobalHotSpare(adapter, enclosure, hotspares, " ".join(options), check)

    def doRaid(self, raidlevel, adapter, enclosure, slots, hotspares, flags, check):

        if raidlevel in ["10", "50", "60"]:
            self.doStrippedRaid(
                raidlevel, adapter, enclosure, slots, hotspares, flags, check
            )
        else:
