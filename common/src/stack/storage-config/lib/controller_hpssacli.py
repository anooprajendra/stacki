...
class CLI:
    ...
    def getEnclosure(self, adapter, slot=None):
        ...
        #
        result = self.run(["slot=%d" % adapter, "physicaldrive", "all", "show"])
        for line in result:
            tokens = line[:-1].split()
            if len(tokens) < 1:
                continue
    ...
    def getArrays(self, adapter):
        ...
        result = self.run(["slot=%d" % adapter, "show", "config"])
        for line in result:
            tokens = line[:-1].split()
            if len(tokens) < 2:
                continue
    ...
    def doNuke(self, enclosure, adapter):
        ...
        #
        result = self.run(["slot=%d" % adapter, "delete", "forced", "override"])
