...
def getHostDisks(nukedisks):
    ...
    p = subprocess.run(
        lsblk, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    disks = []
    diskentry = None
    diskid = 1
    ...
    for l in p.stdout.decode().split("\n"):
        ...
        if (
            removable == "1"
            or readonly == "1"
            or mediatype not in ["disk", "part", "raid", "lvm"]
        ):
            continue
...
def getDiskPartNumber(disk):
    ...
    p = subprocess.run(
        ["blkid", "-o", "export", "-s", "PART_ENTRY_NUMBER", "-p", "/dev/%s" % disk],
...
def getHostPartitions(disks, host_fstab):
    ...
    for d in disks:
        ...
        for l in p.stdout.decode().split("\n"):
            ...
            if not mountpoint:
                mountpoint = getHostMountpoint(host_fstab, uuid, label)
...
def get_sles11_media_type(dev_name):
	...
	"""
    p = subprocess.run(
        f"hwinfo --block --short --only /dev/{dev_name}",
        shell=True,
        encoding="utf-8",
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
