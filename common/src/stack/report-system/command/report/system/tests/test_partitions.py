...
from stack.commands.report.system.tests.stack_test_util import (
    get_part_label,
    get_part_size,
)
...
class TestStoragePartition:
    ...
    def test_storage_partition(self, host):
        ...
        if not storage_config:
            ...
            if not report_storage or report_storage[0] == "[]":
                ...
                storage_config = json.loads(
                    report_storage[0].replace("'", '"').replace("None", '""')
                )
        ...
        assert (
            not errors
        ), f"Host {hostname} found with partitioning mismatch from original config:\n    {errors}"
