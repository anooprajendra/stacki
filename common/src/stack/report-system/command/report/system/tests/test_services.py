...
def test_tftpd_enabled_and_running(host):
    ...
    assert (
        tftpsocket.is_enabled or xinetd.is_enabled or fbtftpd.is_enabled
    ), "No tftp server is enabled."
    assert (
        tftpsocket.is_running or xinetd.is_running or fbtftpd.is_running
    ), "No tftp server is running."
