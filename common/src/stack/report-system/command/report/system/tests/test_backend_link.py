...
class TestLinkUp:
    ...
    def test_link_status(self, host):
        ...
        interfaces = [
            interface_fields(
                name=backend["interface"], ip=backend["ip"], options=backend["options"]
            )
        ...
        for link in interfaces:
            ...
            if (
                link.ip or (link.options and "dhcp" in link.options.lower())
        ...
        assert (
            not errors
        ), f'On host {hostname} the following links were found not connected or down: {", ".join(errors)}'
