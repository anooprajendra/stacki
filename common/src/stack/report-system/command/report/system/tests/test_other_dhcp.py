...
pxe_interfaces = {
    inter["interface"]: inter["mac"]
    for inter in interfaces
    if inter["network"] in pxe_networks
}
...
def test_other_dhcp_server(interface):
    ...
    try:
        send_dhcp.setsockopt(
            socket.SOL_SOCKET, 25, str(interface + "\0").encode("utf-8")
        )
    ...
    if not mac_addr:
        try:
            mac_addr = (
                pathlib.Path(f"/sys/class/net/{interface}/address").read_text().strip()
            )
    ...
    try:
        ...
        pytest.skip(
            f"Port 68 needs to be not in use for test, it is currently bound to process {dhcp_process_name}."
        )
    ...
    try:
        ...
        errors.append(
            f"On interface {interface}, DHCP server at {server_identifier} has been found offering ip {offer_ip}"
        )
