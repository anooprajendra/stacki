    ...
    def generate(self, cdata=True):
        ...
        for snippet in self.snippets:
            ...
            if source != prev:
                ...
                list.append('\t<section source="%s">%s' % (source, cdataStart))
                open = True
            list.append(text)
            prev = source
        if open:
    ...
    def collect(self, node):
        ...
        for child in node.childNodes:
            if child.nodeType in [child.TEXT_NODE, child.CDATA_SECTION_NODE]:
                ...
                try:
                    fn = eval("self.collect_%s_%s" % (child.prefix, child.localName))
...
class SetupTraversor(Traversor):
    ...
    def traverse_stack_profile(self, node):
        ...
        if node.attributes:
            ...
            self.debug("<stack:profile> missing stack:attrs", level="error")
...
class PruningTraversor(Traversor):
    ...
    def traverse(self, node):
        ...
        if arch:
            self.debug("stack:arch is deprecated (%s)" % nodefile, level="warning")
        if osname:
            self.debug("stack:os is deprecated (%s)" % nodefile, level="warning")
        if release:
            self.debug("stack:release is deprecated (%s)" % nodefile, level="warning")
...
class MainTraversor(Traversor):
    ...
    def pre(self):
        self.scripts = {
            "install-post": collections.OrderedDict(),
...
"""
    ...
    def traverse_stack_script(self, node):
        ...
        self.scripts[stage][label] = stack.gen.ProfileSnippet(
            "\n".join(fn), self.getAttr(node, "stack:file")
        )
        return False
    ...
    def traverse_stack_stacki(self, node):
		...
		"""
        self.gen.stackiSection.append(
            self.collect(node), self.getAttr(node, "stack:file")
        )
        self.removeNode(node)
        return False
...
class Generator:
    ...
    def parse(self, xml_string):
        ...
        for traversor in traversors:
            ...
            if debug:
                fout = open(
                    "/tmp/gen-parse-%d-%s.%s.xml"
                    % (i, traversor.__module__, traversor.__class__.__name__),
                    "w",
                )
                fout.write(self.root.toxml())
                fout.close()
...
class AnsibleProfileTraversor(MainTraversor):
    def pre(self):
        self.tasks = {"install-post": [], "boot-pre": [], "boot-post": []}
    ...
    def post(self):
        ...
        self.gen.shellSection.append(
            "\n".join(["---", f'- hosts: {self.gen.attrs["hostname"]}', "  tasks:"])
        )
        ...
        if enabled:
            task = ['  - name: "Packages: Install"', "    package:", "      name:"]
        ...
        if disabled:
            task = ['  - name: "Packages - Remove"', "    package:", "      name:"]
