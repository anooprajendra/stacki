...
class RPMFile(RPMBaseFile):
    ...
    def installPackage(self, root, flags=""):
        ...
        cmd = "rpm -i --nomd5 --force --nodeps --ignorearch " + "--dbpath %s %s " % (
            dbdir,
            flags,
        )
        cmd += "--badreloc --relocate /=%s %s" % (root, self.getFullName())
...
class RollFile(File):
    ...
    def getRollInfoFromXML(self):
        ...
        for dir in cdtree.getDirs():
            for file in cdtree.getFiles(dir):
                try:
                    xmlfile = stack.file.RollInfoFile(file.getFullName())
    ...
    def getRollInfo(self):
        ...
        cmd = "mount -o loop %s /mnt/cdrom > /dev/null 2>&1" % self.getFullName()
