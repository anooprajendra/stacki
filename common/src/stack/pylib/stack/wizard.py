...
class Data:
    ...
    def mountCD(self, prefix="/"):
        ...
        rc = os.system(
            "mount -o ro /tmp/stack-cdrom %s > /dev/null 2>&1" % (mountpoint)
        )
    ...
    def configNetwork(self):
        ...
        # Set resolv.conf
        self.setResolv(
            self.data.Kickstart_PrivateDNSDomain, self.data.Kickstart_PrivateDNSServers
        )
    ...
    def validateDomain(self, fqdn):
        ...
        if hostname.lower() in ["frontend", "backend"]:
            return (
                False,
                "Cannot have an appliance name as a hostname",
                "Hostname Error",
            )
        else:
            return True, "", ""
    ...
    def validateNetwork(self, tup, config_net=False):
        ...
        # incomplete entry
        if (
            not self.data.Kickstart_PrivateInterface
            or not self.data.Kickstart_PrivateAddress
            or not self.data.Kickstart_PrivateNetmask
            or not self.data.Kickstart_PrivateGateway
            or not self.data.Kickstart_PrivateDNSServers
            or not self.data.Info_FQDN
        ):
            return (False, "Please fill out all entries", "Incomplete")
        else:
            # invalid dns format
            ...
            try:
                ...
                self.data.Kickstart_PrivateGateway = str(
                    ipaddress.ip_address(str(self.data.Kickstart_PrivateGateway))
                )
    ...
    def writefiles(self):
        ...
        members = [
            ...
            and not attr == "devices"
            and not attr == "dvdrolls"
            and not attr == "netrolls"
            and not attr == "partition"
        ]
        for w in members:
