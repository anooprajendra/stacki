...
class GraphHandler(
    ...
    def parseNode(self, node, eval=True, rcl=None):
        ...
        for xmlFile in xmlFiles:
            ...
            for line in fin.readlines():
                ...
                if "STACKDEBUG" in os.environ:
                    sys.stderr.write("[parse1 %4d %s]%s" % (i, xmlFileBasename, line))
                    i += 1
                try:
                    parser.feed(line)
                except Exception as e:
                    print(
                        "XML parse error in node file - %s in file %s on line %d\n"
    ...
    def startElement_to(self, name, attrs):
        ...
        self.attrs.main.cond = stack.cond.CreateCondExpr(arch, osname, release, cond)

    def endElement_to(self, name):
        if (not self.prune) or stack.cond.EvalCondExpr(
            self.attrs.main.cond, self.attributes
        ):
    ...
    def startElement_edge(self, name, attrs):
        ...
        self.attrs.main.default.cond = stack.cond.CreateCondExpr(
            arch, osname, release, cond
        )
...
class Pass1NodeHandler(NodeHandler):
    ...
    def evalCond(self, attrs):
        ...
        return stack.cond.EvalCondExpr(
            stack.cond.CreateCondExpr(arch, osname, release, cond), self.attributes
        )
    ...
    def startTag_stack_eval(self, ns, tag, attrs):
        ...
        if self.evalShell == "python":
            self.evalShell = os.path.join(os.sep, "opt", "stack", "bin", "python3")
            self.evalText = [
...
class FrameworkEdge(Edge):
    ...
    def getDot(self, prefix=""):
        ...
        list.append(
            '%s"%s_%s_%s" '
            '[ shape=none label="%s" ];'
            % (prefix, self.parent.name, self.child.name, label, label)
        )
        list.append(
            '%s"%s" -> "%s_%s_%s" [%s arrowsize=0];'
            % (
                prefix,
                self.parent.name,
                self.parent.name,
        ...
        list.append(
            '%s"%s_%s_%s" -> "%s" [%s arrowsize=1.5];'
            % (prefix, self.parent.name, self.child.name, label, self.child.name, attrs)
        )
        return "\n".join(list)
