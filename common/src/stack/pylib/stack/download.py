...
def fetch(url, username=None, password=None, verbose=False, file_path=None):
    ...
    if r.status_code == 404:
        ...
        raise FetchError(
            f"unable to download {filename}, requests status code: {r.status_code}"
        )
    ...
    with open(local_path, "wb") as f:
        ...
        if progress < content_length:
            os.unlink(filename)
            raise FetchError(
                f"unable to download {filename}: the connection may have been prematurely closed by the server.\nFailed at {progress} MB out of {int(content_length)} MB"
            )
