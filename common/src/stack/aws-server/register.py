...
if not prevHostname:
    ...
    stack.api.Call(
        "set host interface mac", [hostname, "interface=eth0", "mac=%s" % mac]
    )
...
for row in stack.api.Call(
    "list bootaction", ["type=install", "os=%s" % boot_os, boot_action]
):
    kernel = row["kernel"]
    ramdisk = row["ramdisk"]
...
server = None
for row in stack.api.Call(
    "list host attr", [hostname, "attr=Kickstart_PrivateKickstartHost"]
):
    server = row["value"]
