...
class Backend:
    ...
    def isPostPkgInstallStage(self):
        ...
        for a in reversed(arr):
            if a.state == State.Set_Bootaction_OS and not a.isError:
                return True
    ...
    def hasStateMessage(self, sm):
        for a in self.stateArr:
            if a.state == sm.state and a.isError == sm.isError and a.msg == sm.msg:
                return True
        return False
...
class StateSequence:
    ...
    OS_STATE_MAP = {"sles-11.x": SLES_11, "sles-12.x": SLES_12, "redhat-7.x": REDHAT_7}
    ...
    def nextExpectedState(currState, os, osversion):
        ...
        for idx, s in enumerate(seqList):
            if s["state"] == currState and (idx + 1) < len(seqList):
...
class StateMessage:
    ...
    def __eq__(self, sm):
        if (
            self.ipAddr == sm.ipAddr
            and self.state == sm.state
            and self.msg == sm.msg
            and self.isError == sm.isError
        ):
            return True
