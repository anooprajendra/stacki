...
class LogParser(threading.Thread):
    ...
    def processDhcp(self, line):
        ...
        sm = StateMessage(ip, State[dhcp[0]], errorFlag, time.time(), msg)
        self.localQ.put(sm)
    ...
    def processTftp(self, line):
        ...
        if not ip or "filename" not in line or pxeFile == "pxelinux.0":
            return
    ...
    def parseSSLAccessLog(self, line):
        ...
        for key, val in LogParser.SSL_ACCESS_LOG_MAP.items():
            if key in line:
                sm = StateMessage(ip, val, errFlag, time.time(), msg)
                self.localQ.put(sm)
                return
    ...
    def run(self):
        ...
        while not self.shutdownFlag.is_set():
            ...
            for b in backendList:
                ...
                if (
                    not currState
                    or currState.isError
                    or not currState.state
                    or currState.state == State.Install_Wait
                    or currState.state == State.Installation_Stalled
                ):
                    continue
    ...
    def process(self, message):
        ...
        try:
            ...
            if "systest" in o:
                ...
                sm = StateMessage(
                    source, State.Reboot_Okay, errFlag, time.mktime(d.timetuple())
                )
                self.localQ.put(sm)
        except Exception as e:
