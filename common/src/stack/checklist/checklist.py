...
from stack.checklist.threads import CheckTimeouts, LogParser, MQProcessor
...
class Checklist(threading.Thread):
    ...
    def getHostInterfaces(self, hnameBackendMap):
        ...
        for o in op:
            hostname = o["host"]
            if o["network"] in pxeNetworkList and hostname in hnameBackendMap:
    ...
    def processTftp(self, sm):
        ...
        if "/" in pxeFile:
            ...
            backendHexIp = "{:02X}{:02X}{:02X}{:02X}".format(*map(int, backendIpArr))

            #
            # Ignore TFTP pxelinux.cfg messages in the post
            # install phase
            #
            if backend.isPostPkgInstallStage() or backendHexIp != pxeArr[-1]:
    ...
    def processQueueMsgs(self):
        while not self.shutdownFlag.is_set():
            ...
            with self.lock:
                ...
                if currState:
                    nextState = StateSequence.nextExpectedState(
                        currState.state, os, osver
                    )
                #
                ...
                if backend.hasStateMessage(sm) or (
                    Checklist.TIMEOUT_STR in sm.msg and backend.isKnownState(sm.state)
                ):
            ...
            with self.lock:
                ...
                if expectedStateList:
                    ...
                    installStalledIdx = backend.findStateMsgIndex(
                        State.Installation_Stalled
                    )
                    ...
                    #
                    stateList.sort(
                        key=lambda x: self.sortStateMessage(
                            x, installWaitIdx, installStalledIdx, expectedStateList
                        )
                    )
                ...
                if currState and not currState.isError and not currState.isAddedToRedis:
    ...
    def sortStateMessage(
        self, stateMsg, installWaitIdx, installStalledIdx, expectedStateList
    ):
    ...
    def run(self):
        ...
        self.mqProcessor = MQProcessor(context, tx, self.queue)
        self.mqProcessor.setDaemon(True)
        self.mqProcessor.start()

        self.dhcpLog = LogParser(r"/var/log/messages", self.queue)
        self.dhcpLog.setDaemon(True)
        self.dhcpLog.start()

        self.frontendOS = stack.api.Call("list.host.attr", ["localhost", "attr=os"])[0][
            "value"
        ]
    ...
    def run_redhat(self):
        self.apacheLog = LogParser(r"/var/log/httpd/ssl_access_log", self.queue)
        self.accessLog = LogParser(r"/var/log/httpd/access_log", self.queue)

    """ SLES specific edits go here """

    def run_sles(self):
        self.apacheLog = LogParser(r"/var/log/apache2/ssl_access_log", self.queue)
        self.accessLog = LogParser(r"/var/log/apache2/access_log", self.queue)
...
if __name__ == "__main__":
    if "STACKDEBUG" not in os.environ:
        lock = lockfile.pidlockfile.PIDLockFile(
            "/var/run/%s/%s.pid" % ("checklist", "checklist")
        )
