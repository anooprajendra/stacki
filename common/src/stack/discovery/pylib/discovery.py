...
class Discovery:
            ...
            for line in results.stdout.splitlines():
                ...
                if match:
                    if match.group(1) == interface:
                        ...
                        self._get_ipv4_network_for_interface_cache[
                            interface
                        ] = ipv4_network
    ...
    def _get_hosts_for_interface(self, interface):
        ...
        if ipv4_network is not None:
            ...
            for row in self._command.call("list.network"):
                if row["address"] == str(ipv4_network.network_address) and row[
                    "mask"
                    ...
                    if row["pxe"] == True:
                        ...
                        self._logger.warning(
                            "pxe not enabled on interface: %s", interface
                        )
                    break
            else:
    ...
    def _get_next_ip_address(self, interface):
        ...
        for ip_address in self._get_next_ip_address_cache[interface]:
            ...
            for row in self._command.call("list.host.interface"):
                if row["ip"] == str(ip_address) and not row["interface"].startswith(
                    "vlan"
                ):
    ...
    def _add_node(self, interface, mac_address, ip_address):
        ...
        if network is not None:
            ...
            if result.returncode != 0:
                self._logger.error(
                    "failed to add interface for host %s:\n%s",
                    self.hostname,
                    result.stderr,
                )
                return
            ...
            if result.returncode != 0:
                self._logger.error(
                    "failed to set install action for host %s:\n%s",
                    self.hostname,
                    result.stderr,
                )
                return
            ...
            result = subprocess.run(
                [
                    ...
                    "attr",
                    self.hostname,
                    "attr=discovery.install",
                    f"value={self._install}",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                encoding="utf-8",
            ...
            if result.returncode != 0:
                self._logger.error(
                    "failed to set 'discovery.install' attribute for host %s:\n%s",
                    self.hostname,
                    result.stderr,
                )
                return
            ...
            if result.returncode != 0:
                self._logger.error(
                    "failed to set boot action for host %s:\n%s",
                    self.hostname,
                    result.stderr,
                )
                return
        ...
        if match:
            ...
            for row in self._command.call("list.host.interface"):
                if row["mac"] == mac_address:
                    self._logger.debug(
                        "node is already known: %s %s", mac_address, interface
                    )
                    break
            else:
                ...
                if ip_address is None:
                    self._logger.error(
                        "no IP addresses available for interface %s", interface
                    )
                else:
                    # Add the new node
            ...
            if "DHCPDISCOVER" in line:
                self._logger.warning(
                    "DHCPDISCOVER found in line but didn't match regex:\n%s", line
                )
    ...
    def _process_kickstart_line(self, line):
        if re.search("install/sbin(/public)?/profile.cgi", line):
            ...
            try:
                ...
                self._socket.sendto(
                    message.encode(), ("localhost", stack.mq.ports.publish)
                )
            except ValueError:
    ...
    def __init__(self, logging_level=logging.INFO):
        ...
        try:
            handler = RotatingFileHandler(
                self._LOGFILE, maxBytes=10 * 1024 * 1024, backupCount=3
            )
        ...
        if not self.is_running():
            ...
            if rank is None:
                ...
                for host in command.call("list.host"):
                    if (
                        host["appliance"] == self._appliance_name
                        and int(host["rack"]) == self._rack
                        and int(host["rank"]) >= self._rank
                    ):
            ...
            if install_action is None:
                ...
                for action in command.call("list.bootaction"):
                    if (
                        action["type"] == "install"
                        and action["bootaction"] == install_action
                    ):
                        self._install_action = install_action
                        break
                else:
                    raise ValueError(
                        f"Unknown install action with name {install_action}"
                    )
