...
class Command(stack.commands.add.environment.command):
	...
	<example cmd='add environment storage controller test slot=1 raidlevel=0 arrayid=1'>
	The disk in slot 1 should be a RAID 0 disk for the test environment.
	</example>

	<example cmd='add environment storage controller test slot=2,3,4,5,6 raidlevel=6 hotspare=7,8 arrayid=2'>
