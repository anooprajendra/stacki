...
from stack.download import FetchError, fetch
...
class Command(stack.commands.CartArgumentProcessor, stack.commands.add.command):
    """
    ...
    def check_cart(self, cartfile):
        ...
        if tarfile.is_tarfile(cartfile) == True:
            with tarfile.open(cartfile, "r|*") as tar:
                ...
                dirs = [os.path.split(f.name)[-1] for f in files if f.isdir() == True]
    ...
    def download_url(self, url, dest, fname, user, password):
        r = requests.get(url, stream=True, auth=(user, password), timeout=5)
    ...
    def run(self, params, args):
        ...
        if cartfile == None:
            pass
        elif Path(cartfile).exists() == True and Path(cartfile).is_file() == True:
