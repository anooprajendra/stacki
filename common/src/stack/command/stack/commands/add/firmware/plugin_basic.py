...
from stack.exception import ArgError, ArgRequired, ArgUnique, ParamError, ParamRequired
...
class Plugin(stack.commands.Plugin):
    ...
    def validate_imp(self, make, model, imp):
        ...
        if not imp and not self.owner.model_exists(make=make, model=model):
            ...
            makes_and_models = "\n".join(
                ...
                for make_model in self.owner.call(command="list.firmware.model")
            )
            raise ParamError(
                cmd=self.owner,
                param="imp",
        ...
        if imp and self.owner.model_exists(make=make, model=model):
            existing_imp = self.owner.call(
                command="list.firmware.model", args=[model, f"make={make}"]
            )[0]["implementation"]
    ...
    def validate_hash_alg_supported(self, hash_alg):
        ...
        try:
            ...
            raise ParamError(
                cmd=self.owner, param="hash_alg", msg=f"{exception}",
            ) from exception
    ...
    def create_missing_make(self, make, cleanup):
        ...
        if not self.owner.make_exists(make=make):
            ...
            cleanup.callback(
                self.owner.call, command="remove.firmware.make", args=[make]
            )
    ...
    def create_missing_model(self, make, model, imp, cleanup):
        ...
        if not self.owner.model_exists(make=make, model=model):
            self.owner.call(
                command="add.firmware.model", args=[model, f"make={make}", f"imp={imp}"]
            )
            cleanup.callback(
                self.owner.call,
    ...
    def fetch_firmware(self, source, make, model, cleanup):
        ...
        try:
            ...
            raise ParamError(
                cmd=self.owner, param="source", msg=f"{exception}",
            ) from exception
    ...
    def calculate_hash(self, file_path, hash_alg, hash_value):
        ...
        try:
            return stack.firmware.calculate_hash(
                file_path=file_path, hash_alg=hash_alg, hash_value=hash_value
            )
        except stack.firmware.FirmwareError as exception:
            raise ParamError(
                cmd=self.owner, param="hash", msg=f"{exception}",
            ) from exception

    def add_firmware(
        self, source, make, model, version, imp, hash_value, hash_alg, cleanup
    ):
        ...
        file_path = self.fetch_firmware(
            source=source, make=make, model=model, cleanup=cleanup,
        )
        ...
        self.create_missing_related_entries(
            make=make, model=model, imp=imp, cleanup=cleanup
        )
        ...
        cleanup.callback(
            ...
            args=[version, f"make={make}", f"model={model}"],
        )
