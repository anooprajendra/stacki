    ...
    def run(self, params, args):
        ...
        if slot:
            for s in slot.split(","):
                ...
                if s == "*":
                    ...
                    if s in slots:
                        raise ParamError(self, "slot", f'"{s}" is listed twice')
                ...
                for scope_mapping in scope_mappings:
					...
					""",
                            (adapter, enclosure, s, *scope_mapping),
                        )
                        != 0
                    ):
                        raise CommandError(
        ...
        if hotspare:
            for h in hotspare.split(","):
                ...
                if h in hotspares:
                    raise ParamError(self, "hotspare", f'"{h}" is listed twice')

                if h in slots:
                    raise ParamError(self, "hotspare", f'"{h}" is listed in slots')
                ...
                for scope_mapping in scope_mappings:
                    ...
                    if (
						...
						AND scope_map.node_id <=> %s
					""",
                            (adapter, enclosure, h, *scope_mapping),
                        )
                        != 0
                    ):
                        raise CommandError(
