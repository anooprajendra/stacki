...
class Command(PalletArgumentProcessor, stack.commands.enable.command):
    """
    ...
    def run(self, params, args):
        ...
        for pallet in self.get_pallets(args, params):
            ...
            if box_os != pallet.os:
                raise CommandError(
                    self, f'incompatible pallet "{pallet.name}" with OS "{pallet.os}"'
                )
            ...
            if (
                ...
                self.db.execute(
                    "insert into stacks(box, roll) values (%s, %s)", (box_id, pallet.id)
                )
