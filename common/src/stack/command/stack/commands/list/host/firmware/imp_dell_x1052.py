...
class Implementation(stack.commands.Implementation):
    def list_firmware(self, host, switch_attrs):
        ...
        with SwitchDellX1052(
            switch_ip_address=host, switchname=host, **kwargs
        ) as x1052_switch:
    ...
    def run(self, args):
        ...
        x1052_attrs_per_host = {host_make_model.host: {} for host_make_model in args}
        ...
        return {
            host_make_model: results_by_host[host_make_model.host].boot
            if "boot" in host_make_model.model
            else results_by_host[host_make_model.host].software
