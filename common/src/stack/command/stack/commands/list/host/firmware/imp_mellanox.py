...
class Implementation(stack.commands.Implementation):
    def list_firmware(self, host, switch_attrs):
        ...
        with SwitchMellanoxM7800(host, **kwargs) as m7800_switch:
            ...
            installed_image = image_listing.installed_images[
                image_listing.last_boot_partition
            ]
    ...
    def run(self, args):
        ...
        with ThreadPoolExecutor(
            thread_name_prefix="mellanox_firmware_listing"
        ) as executor:
            futures_by_host_make_model = {
                host_make_model: executor.submit(
                    self.list_firmware, host=host_make_model.host, switch_attrs=attrs
                )
