    ...
    def run(self, args):
        ...
        for host_make_model, firmware_info in database_values_dict.items():
            if firmware_info.firmware_imp in mapped_by_imp_name:
                mapped_by_imp_name[firmware_info.firmware_imp][
                    host_make_model
                ] = host_attrs[host_make_model.host]
            else:
                mapped_by_imp_name[firmware_info.firmware_imp] = {
                    host_make_model: host_attrs[host_make_model.host]
                }

        # run the implementations in parallel.
        results_by_imp = self.owner.run_implementations_parallel(
            implementation_mapping=mapped_by_imp_name, display_progress=True,
        )
        ...
        results_by_host_make_model = {
            ...
            for host_make_model, version in flatten(
                ...
                for results in results_by_imp.values()
        ...
        return CommonResult(
            header=["current_firmware_version"], values=results_by_host_make_model
        )
