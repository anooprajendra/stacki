...
class Command(stack.commands.ScopeArgumentProcessor, stack.commands.list.command):
    ...
    def run(self, params, args):
        ...
        is_glob = (
            attr is not None and re.match("^[a-zA-Z_][a-zA-Z0-9_.]*$", attr) is None
        )
        ...
        if var:
            if resolve and scope == "host":
                ...
                hostnames = flatten(
                    self.db.select(
                        "nodes.name FROM nodes WHERE nodes.id IN %s", [node_ids]
                    )
                )
                ...
                if shadow:
                    ...
                    for host, *row in self.db.select(
                        query, values, prepend_select=False
                    ):
                        if row[2] not in seen[host]:
