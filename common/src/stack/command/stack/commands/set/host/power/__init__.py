...
import stack.mq
from stack.exception import ArgRequired, CommandError, ParamError
...
class Command(stack.commands.set.host.command):
    ...
    def run(self, params, args):
        ...
        cmd, debug, force_imp = self.fillParams(
            [("command", None, True), ("debug", False), ("method", None)]
        )
        host_errors = []
        ...
        if force_imp:
            if force_imp not in self.impl_list:
                raise ParamError(
                    self, "method", f"{force_imp} not found as a valid implementation"
                )
            imp_names = [force_imp]
        ...
        if cmd not in ["on", "off", "reset", "status"]:
            raise ParamError(
                self, "command", 'must be "on", "off", "reset", or "status"'
            )
            ...
            for imp in imp_names:
                ...
                if self.debug:
                    ...
                    if not imp_msg.success and imp_msg.debug:
                        self.addOutput(
                            host, f'Failed to set power via {imp}: "{imp_msg.debug}"'
                        )
