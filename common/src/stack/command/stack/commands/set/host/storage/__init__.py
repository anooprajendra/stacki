...
class Command(stack.commands.set.host.command):
    ...
    def run(self, params, args):
        (encid, slot, action) = self.fillParams(
            [("enclosure", None, True), ("slot", None, True), ("action", None, True)]
        )

        validactions = ["online", "offline", "configure", "locate-on", "locate-off"]
        if action not in validactions:
            raise CommandError(
                self, '"action" must one of: %s' % ", ".join(validactions)
            )
        ...
        for host in self.getHostnames(args):
            ...
            for row in self.call("list.host.storage", [host]):
                if row["enclosure"] == encid and row["slot"] == slot:

                    found = 1

                    if action == "online" and row["status"] == "online":

                        msg = "storage device at "
                        msg += "%d:%d " % (encid, slot)
                        msg += "is already online"
                        print(msg)
                    elif action == "offline" and row["status"] == "offline":

                        msg = "storage device at "
                        msg += "%d:%d " % (encid, slot)
                        msg += "is already offline"
                        print(msg)
                    elif action == "configure" and row["status"] != "unconfigured":
                        ...
                        self.setState(host, adapter, encid, slot, action)
