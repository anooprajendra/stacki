    ...
    def run(self, args):
        ...
        if cmd == "on":
            ...
            cmd_output = self.owner._exec(
                f'ssh {host} "shutdown -h now"', shlexsplit=True
            )
            err = cmd_output.stderr
            if (
                cmd_output.returncode != 0
                and f"Connection to {host} closed by remote host" not in err
            ):
            ...
            if cmd_output.returncode != 0:
                return impl_output(
                    "", f"Chassis Power is unreachable via ssh: {err.strip()}", False
                )
