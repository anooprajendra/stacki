...
class command(stack.commands.set.command, stack.commands.OSArgumentProcessor):
    ...
    def actionExists(self, b_action, b_type, b_os=None):
        for row in self.call(
            "list.bootaction", [b_action, f"type={b_type}", f"os={b_os}"]
        ):
            if b_os == "":
                b_os = None

            if (
                b_action == row["bootaction"]
                and b_type == row["type"]
                and b_os == row["os"]
            ):
                return True
    ...
    def bootNameExists(self, b_action, b_type):
        return (
            self.db.count(
                "(ID) from bootnames where name=%s and type=%s", (b_action, b_type)
            )
            > 0
        )
...
class Command(command):
    ...
    def run(self, params, args):
        ...
        (b_kernel, b_ramdisk, b_args, force) = self.fillParams(
            [("kernel", "", True), ("ramdisk", ""), ("args", ""), ("force", True)]
        )
        ...
        if not self.bootNameExists(b_action, b_type):
            self.db.execute(
                "insert into bootnames(name, type) values (%s, %s)", (b_action, b_type)
            )
        ...
        for flag, command in [
            ...
            if flag:
                self.command(
                    f"set.bootaction.{command}",
                    [b_action, f"type={b_type}", f"os={b_os}", f"{command}={flag}"],
                )
