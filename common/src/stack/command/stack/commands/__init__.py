...
class EnvironmentArgumentProcessor:
    ...
    def getEnvironmentNames(self, args=None):
        ...
        for arg in args:
            names = flatten(
                self.db.select("name from environments where name like %s", (arg,))
            )
...
class ApplianceArgumentProcessor:
    ...
    def getApplianceNames(self, args=None):
        ...
        for arg in args:
            names = flatten(
                self.db.select("name from appliances where name like %s", (arg,))
            )
...
class NetworkArgumentProcessor:
    ...
    def getNetworkNames(self, args=None):
        ...
        for arg in args:
            names = flatten(
                self.db.select("name from subnets where name like %s", (arg,))
            )
...
class SwitchArgumentProcessor:
    ...
    def doSwitchHost(self, switch, port, host, interface, action):
        ...
        if not host_interface:
            raise CommandError(
                self, "Interface '%s' isn't defined for host '%s'" % (interface, switch)
            )
...
class CartArgumentProcessor:
    ...
    def getCartNames(self, args):
        ...
        for arg in args:
            names = flatten(
                self.db.select("name from carts where name like binary %s", (arg,))
            )
...
class HostArgumentProcessor:
    ...
    def getHostnames(
        self, names=[], managed_only=False, subnet=None, host_filter=None, order="asc"
    ):
        """
    ...
    def getRunHosts(self, hosts):
        ...
        for host in hosts:
            ...
            if "stack.network" in attrs[host]:
                h[host] = self.getHostnames(
                    [host], subnet=attrs[host]["stack.network"]
                )[0]
            ...
            # just use the first one returned by the database.
                ...
                net = [
                    f["network"]
                    for f in host_if
                    if f["pxe"] == True and f["host"] == host and f["default"] == True
                ]
                if not net:
...
class ScopeArgumentProcessor(
    ...
    def getScopeMappings(self, args=None, scope=None):
        ...
        if scope == "global":
            ...
            if args:
                raise CommandError(
                    cmd=self, msg="Arguments are not allowed at the global scope.",
                )

            scope_mappings.append(ScopeMapping(scope, None, None, None, None))
            ...
            for appliance_id in flatten(
                self.db.select("id from appliances where name in %s", (names,))
            ):
            ...
            # Now we have to convert the names to the primary keys
            for os_id in flatten(
                self.db.select("id from oses where name in %s", (names,))
            ):
                scope_mappings.append(ScopeMapping(scope, None, os_id, None, None))
            ...
            if names:
                ...
                # Now we have to convert the names to the primary keys
                for environment_id in flatten(
                    self.db.select("id from environments where name in %s", (names,))
                ):
                    scope_mappings.append(
            ...
            # Now we have to convert the names to the primary keys
            for node_id in flatten(
                self.db.select("id from nodes where name in %s", (names,))
            ):
                scope_mappings.append(ScopeMapping(scope, None, None, None, node_id))
    ...
    def execute(self, command, args=None, many=False):
        ...
        if self.link:
            ...
            if stack.commands._debug:
                ...
                if many:
                    commands = "\n".join(
                        (self.link.mogrify(command, arg) for arg in args)
                    )
                    Debug(
                        "SQL EX: %.4d rows in %.3fs <- %s"
                        % (result, (t1 - t0), commands)
                    )
                else:
                    Debug(
                        "SQL EX: %.4d rows in %.3fs <- %s"
                        % (result, (t1 - t0), self.link.mogrify(command, args))
                    )
        ...
        if not addr:
            if self.link:
                ...
                if self.count("(id) from appliances where name=%s", (hostname,)):
                    ...
                    message = (
                        f'use "e:{hostname}" for hosts in the {hostname} environment'
                    )
...
class Command:
    ...
    def __init__(self, database, *, debug=None):
        ...
        if os.path.exists("/etc/centos-release") or os.path.exists(
            "/etc/redhat-release"
        ):
            self.os = "redhat"
    ...
    def command(self, command, args=[], verbose_errors=True):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to run {name} {' '.join(args)}") from exception
    ...
    def run_implementations_parallel(
        self, implementation_mapping, display_progress=False
    ):
        ...
        imps_to_run = {
            imp_name: args
            for imp_name, args in implementation_mapping.items()
            ...
            if display_progress and futures_by_imp:
                ...
                while not all(future.done() for future in futures_by_imp.values()):
                    if not timer.is_alive():
                        new_futures_done = len(
                            [
                                future
                                for future in futures_by_imp.values()
                                if future.done()
                            ]
                        )
                        ...
                        if new_futures_done > futures_done:
                            sys.stdout.write(
                                "*" * chunk_size * (new_futures_done - futures_done)
                            )
        ...
        for imp, future in futures_by_imp.items():
            results_by_imp[imp] = namedtuple(
                "ParallelImpResults", ("result", "exception")
            )(
    ...
    def endOutput(self, header=[], padChar="-", trimOwner=False, trimHeader=False):
        ...
        if (
            ...
            and header
            and startOfLine == 0
            and (sum(colwidth) + len(line)) > self.width
        ):
            maxWidth = 0
    ...
    def getHostAttrDict(self, host, attr=None):
        ...
        return {
            ...
            for k, v in groupby(self.call("list.host.attr", params), itemgetter("host"))
        }
