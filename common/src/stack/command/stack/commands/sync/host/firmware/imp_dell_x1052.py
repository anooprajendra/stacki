...
class Implementation(stack.commands.Implementation):
    def update_firmware(
        ...
        **kwargs,
        ...
        try:
            x1052_switch = SwitchDellX1052(
                switch_ip_address=switch_name, switchname=switch_name, **kwargs
            )
    ...
    def run(self, args):
        ...
        for switch_name_make_model, firmware_info in args.items():
            ...
            x1052_firmware_args_per_host[switch_name_make_model.host].update(
                {
                    update_key: firmware_file.file,
                    "tftp_ip": firmware_info.frontend_ip,
                    **kwargs,
                }
            )
        ...
        with ThreadPoolExecutor(thread_name_prefix="dell_firmware_update") as executor:
            futures = [
                executor.submit(
                    self.update_firmware, switch_name=switch_name, **switch_args
                )
