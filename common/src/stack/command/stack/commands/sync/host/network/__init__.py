...
from stack.commands.sync.host import Parallel, timeout
...
class Command(stack.commands.sync.host.command):
    ...
    def run(self, params, args):
        ...
        for h in run_hosts:
            ...
            manage_hostsfile = self.str2bool(
                host_attrs[host].get("manage.hostsfile", False)
            )
            sync_hostsfile = self.str2bool(
                host_attrs[host].get("sync.hostsfile", False)
            )
