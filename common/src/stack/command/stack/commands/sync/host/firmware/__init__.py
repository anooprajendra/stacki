...
class Command(stack.commands.sync.host.command, FirmwareArgumentProcessor):
    ...
    def _get_invalid_mappings(self, results, force, host_current_firmwares):
        ...
        for host_make_model, firmware_info in results.items():
            ...
            regex_obj = self.try_get_version_regex(
                make=host_make_model.make, model=host_make_model.model,
            )
            for firmware_file in firmware_info.firmware_files:
                ...
                if regex_obj and not re.search(
                    regex_obj.regex, firmware_file.version, re.IGNORECASE
                ):
                    invalid_mappings.add(host_make_model)
                    self.notify(
                ...
                if (
                    not force
                    and firmware_file.version == host_current_firmwares[host_make_model]
                ):
                    invalid_mappings.add(host_make_model)
                    self.notify(
                        f"Skipping The host + make + model combination of {host_make_model.host} {host_make_model.make} {host_make_model.model}"
    ...
    def _sync_required_firmware_files(self, results):
        ...
        firmware_files_to_sync = {
            ...
            for make_model, values in itertools.groupby(
                sorted(results.items(), key=keyfunc), key=keyfunc,
            )
        }
        ...
        for make_model, versions in firmware_files_to_sync.items():
            self.call(
                command="sync.firmware",
                args=[*versions, f"make={make_model[0]}", f"model={make_model[1]}"],
            )

    def _get_host_firmwares(
        self, common_key, hosts, host_attrs, host_current_firmwares, force
    ):
        ...
        FirmwareFile = make_dataclass("FirmwareFile", ("file", "version", "url",))
        for row in self.db.select(
            ...
            if host_make_model in results:
                ...
                results[host_make_model] = FirmwareInfo(
                    ...
                    force=force,
                    frontend_ip=self.get_common_frontend_ip(
        ...
        invalid_mappings = self._get_invalid_mappings(
            results=results, force=force, host_current_firmwares=host_current_firmwares,
        )
        ...
        hosts_with_mapped_firmware = [
            host_make_model.host for host_make_model in results
        ]
        for host in hosts:
            if host not in hosts_with_mapped_firmware:
                self.notify(f"Skipping {host} because no firmware is mapped to it.")
        ...
        for host_make_model, firmware_info in results.items():
            for firmware_file in firmware_info.firmware_files:
                firmware_file.url = self.get_firmware_url(
                    hostname=host_make_model.host, firmware_file=firmware_file.file,
                )
    ...
    def run(self, params, args):
        ...
        (force,) = self.fillParams(names=[("force", False)], params=params)
        ...
        current_firmware_versions = {
            CommonKey(result["host"], result["make"], result["model"]): result[
