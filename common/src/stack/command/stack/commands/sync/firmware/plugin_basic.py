...
from stack.exception import ArgError, CommandError, ParamRequired
...
class Plugin(stack.commands.Plugin):
    ...
    def run(self, args):
        ...
        make, model = self.owner.fillParams(
            names=[("make", None), ("model", None)], params=params
        )
        ...
        for row in self.owner.db.select(
            ...
            if not local_file.exists():
                try:
                    local_file = stack.firmware.fetch_firmware(
                        source=source, make=make, model=model, filename=local_file.name
                    )
            ...
            try:
                stack.firmware.calculate_hash(
                    file_path=local_file, hash_alg=hash_alg, hash_value=hash_value
                )
            except stack.firmware.FirmwareError as exception:
                raise CommandError(
                    cmd=self.owner, msg=f"Error during file verification: {exception}"
                )
        ...
        files_expected = [
            ...
            for file_path in (
                row[0] for row in self.owner.db.select("firmware.file FROM firmware")
            )
        ]
