...
class Implementation(
    ...
    def checkAppliance(self, appliance):
        if appliance not in self.appliances:
            msg = 'appliance "%s" does not exist in the database' % appliance
    ...
    def checkMAC(self, mac):
        for o in self.list_host_interface:
            if o["mac"].lower() == mac:
                msg = 'MAC "%s" is already in the database' % mac
                raise CommandError(self.owner, msg)

    def checkNetwork(self, network):
        if network not in self.networks:
            msg = 'network "%s" does not exist in the database' % network
            raise CommandError(self.owner, msg)
    ...
    def run(self, args):
        ...
        self.list_host_interface = self.owner.call("list.host.interface")
        self.appliances = self.getApplianceNames()
        # need all the info from networks(/subnets)
        self.networks = dict(
            (k, next(v))
            for k, v in groupby(self.owner.call("list.network"), itemgetter("network"))
        )
        ...
        try:
            ...
            for row in reader:
                ...
                if not header:
                    ...
                    if len(required) > 0:
                        msg = (
                            'the following required fields are not present in the input file: "%s"'
                            % ", ".join(required)
                        )
                        raise CommandError(self.owner, msg)
                ...
                if appliance:
                    if (
                        "appliance" in self.owner.hosts[name].keys()
                        and self.owner.hosts[name]["appliance"] != appliance
                    ):
                        msg = (
                            'two different appliance types specified for host "%s"'
                            % name
                        )
                        raise CommandError(self.owner, msg)
                ...
                if rack:
                    if (
                        "rack" in self.owner.hosts[name].keys()
                        and self.owner.hosts[name]["rack"] != rack
                    ):
                        msg = (
                            'two different rack numbers specified for host "%s"' % name
                        )
                        raise CommandError(self.owner, msg)
                ...
                if rank:
                    if (
                        "rank" in self.owner.hosts[name].keys()
                        and self.owner.hosts[name]["rank"] != rank
                    ):
                        msg = (
                            'two different rank numbers specified for host "%s"' % name
                        )
                        raise CommandError(self.owner, msg)
                ...
                if ip:
                    if not network:
                        raise CommandError(
                            self.owner, "inclusion of IP requires inclusion of network"
                        )
                ...
                if comment:
                    if "comment" not in self.owner.hosts[name].keys():
                        ...
                        self.owner.hosts[name]["comment"] += ", %s" % comment
                if installaction:
                    if installaction not in self.actions:
                        msg = (
                            'installaction "%s" does not exist in the database'
                            % installaction
                        )
                        raise CommandError(self.owner, msg)
                    else:
                ...
                if osaction:
                    if osaction not in self.actions:
                        msg = (
                            'bootaction "%s" does not exist in the database' % osaction
                        )
                        raise CommandError(self.owner, msg)
                    else:
        ...
        for name in self.owner.hosts.keys():
            ...
            if len(ifaces) == 1:
                ...
                for interface in ifaces:
                    if (
                        "default" in self.owner.interfaces[name][interface]
                        and str2bool(self.owner.interfaces[name][interface]["default"])
                        is True
                    ):
                        if not default:
                            default = True
                ...
                if not default:
                    msg = (
                        'host "%s" has multiple interfaces but none of the interfaces are designated as\na "default" interface. add a "default" column to your spreadsheet and\nmark one of the interfaces as "True" in the default column'
                        % name
                    )
                    raise CommandError(self.owner, msg)
                ...
                if multiple_defaults:
                    msg = (
                        'host "%s" has more than one interface designated as the "default" interface.\nedit your spreadsheet so that only one interface is the "default".'
                        % name
                    )
                    raise CommandError(self.owner, msg)
            ...
            for interface in self.owner.interfaces[name].keys():
                ...
                if network:
                    ...
                    if ip:
                        ...
                        if ip != "auto" and ipaddress.IPv4Address(ip) not in ipnetwork:
                ...
                if re.match("bond[0-9]+$", interface):
                    ...
                    if not found:
                        ...
                        msg += (
                            'but there is no channel "%s" specified for any interface associated with this host'
                            % (interface)
                        )
                        raise CommandError(self.owner, msg)
