...
class command(stack.commands.Command):
    ...
    def _validate_raid_device(self, device, partitions_list, raid_devices):
        ...
        for partition in partitions_list:
            ...
            if not partition.get("options"):
                raise CommandError(
                    self, f'missing options for software raid device "{device}"'
                )
    ...
    def validate_partition(self, partitions):
        ...
        devices = {partition.get("device"): [] for partition in partitions}
        ...
        if unknown_devices:
            raise CommandError(
                self, f"unknown device(s) detected: {', '.join(unknown_devices)}"
            )
    ...
    def load_partition(self, partitions, target=None):
        ...
        self.stack(
            f"remove.{scope}.storage.partition"
            if scope != "global"
            else "remove.storage.partition",
            target,
            device="*",
        )
    ...
    def run(self, params, args):
        ...
        document, self.exec_commands, self.force = self.fillParams(
            [("document", None), ("exec", False), ("force", False),]
        )
