...
class Implementation(
    ...
    def process_target(
        self, target, device, partid, mountpoint, size, fstype, options, line
    ):
        if device is None:
            raise CommandError(
                self.owner, f'empty value found for "device" column at line {line}'
            )

        if size is None:
            raise CommandError(
                self.owner, f'empty value found for "size" column at line {line}'
            )
        ...
        try:
            ...
            for line, row in enumerate(reader, 1):
                    ...
                    if header[ndx] == "name":
                        ...
                        if field.lower() in ["recommended", "hibernation"]:
                            ...
                            try:
                                ...
                                raise CommandError(
                                    self.owner, f'size "{field}" must be an integer'
                                )

                            if size < 0:
                                raise CommandError(
                                    self.owner, f'size "{size}" must be >= 0'
                                )
                        ...
                        try:
                            ...
                            raise CommandError(
                                self.owner, f'partid "{field}" must be an integer'
                            )

                        if partid < 0:
                            raise CommandError(
                                self.owner, f'partid "{partid}" must be >= 0'
                            )
                ...
                for target in targets:
                    self.process_target(
                        target, device, partid, mountpoint, size, fstype, options, line
                    )
