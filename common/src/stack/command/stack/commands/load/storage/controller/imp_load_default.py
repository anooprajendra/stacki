...
class Implementation(
    stack.commands.ApplianceArgumentProcessor, stack.commands.Implementation
):
    """
    ...
    def process_target(
        self, host, slot, enclosure, adapter, raid, array, options, line
    ):
        if slot is None:
            raise CommandError(
                self.owner, f'empty value found for "slot" column at line {line}'
            )

        if raid is None:
            raise CommandError(
                self.owner, f'empty value found for "raid level" column at line {line}'
            )

        if array is None:
            raise CommandError(
                self.owner, f'empty value found for "array id" column at line {line}'
            )
        ...
        if slot in self.owner.hosts[host][array]["slot"]:
            raise CommandError(
                self.owner,
                f'duplicate slot "{slot}" found in the ' f"spreadsheet at line {line}",
            )
    ...
    def run(self, args):
        ...
        try:
            ...
            for line, row in enumerate(reader, 1):
                    ...
                    if header[ndx] == "name":
                        ...
                        if field == "*":
                            ...
                            try:
                                ...
                                raise CommandError(
                                    self.owner, f'slot "{field}" must be an integer'
                                )

                            if slot < 0:
                                raise CommandError(
                                    self.owner, f'slot "{slot}" must be >= 0'
                                )
                        ...
                        if field.lower() == "global":
                            ...
                            try:
                                ...
                                raise CommandError(
                                    self.owner,
                                    f'array id "{field}" must ' f"be an integer",
                                )

                            if array < 0:
                                raise CommandError(
                                    self.owner, f'array id "{array}" must be >= 0'
                                )
                ...
                if not name:
                    raise CommandError(
                        self.owner, 'empty host name found in "name" column'
                    )
                ...
                for target in targets:
                    self.process_target(
                        target, slot, enclosure, adapter, raid, array, options, line
                    )
