...
class Command(stack.commands.load.command, stack.commands.HostArgumentProcessor):
    ...
    def checkAttr(self, attr):
        ...
        if " " in attr:
            raise CommandError(
                self, 'attribute "%s" cannot have a space character' % attr
            )

        a = attr.split("/")
        if len(a) > 2:
            raise CommandError(
                self, 'attribute "%s" cannot have more than one "/"' % attr
            )
        ...
        for t in a:
            for token in t.split("."):
                if not re.match(ctoken, token):
                    raise CommandError(
                        self,
                        'attribute "%s" contains an invalid character.\n"%s" must be a valid ctoken'
                        % (attr, token),
                    )
