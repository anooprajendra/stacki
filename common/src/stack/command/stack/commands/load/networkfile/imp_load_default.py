...
class Implementation(
    stack.commands.NetworkArgumentProcessor, stack.commands.Implementation
):
    ...
    def run(self, args):
        ...
        if len(self.owner.networks.keys()) < line - 1:
            msg = "I've detected %s lines in the file but only " % (line - 1)
            msg += "%s networks. Do you have duplicates?" % len(
                self.owner.networks.keys()
            )
            raise CommandError(self.owner, msg)
