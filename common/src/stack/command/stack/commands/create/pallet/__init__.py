...
class Builder:
    ...
    def writerepo(self, name, version, release, OS, arch):
        ...
        basedir = os.getcwd()
        palletdir = os.path.join(basedir, "disk1", name, version, release, OS, arch)
        os.chdir(palletdir)
    ...
    def copyRoll(self, roll, dir):
        if roll.isRollForeign():
            self.command(
                "add.pallet", [roll.getFullName(), "dir=%s" % dir, "updatedb=n"]
            )
            ...
            os.makedirs(tmp)
            os.system(
                "mount -o loop -t iso9660 %s %s > /dev/null 2>&1"
...
class RollBuilder(Builder, stack.dist.Arch):
    ...
    def getExternalRPMS(self):
        ...
        xml = self.command(
            "list.node.xml", ["everything", "eval=n", "attrs=%s" % attrs]
        )
        ...
        rpms = [
            pkg
            for node_pkgs in generator.packageSet.getPackages()["enabled"].values()
    ...
    def run(self):
        ...
        for (name, id, size, files) in self.spanDisks(list):
            ...
            self.stampDisk(
                name, self.config.getRollName(), self.config.getRollArch(), id
            )
...
class GitRollBuilder(Builder):
    ...
    def fetch_from_git(self):
        ...
        if os.path.exists(self.repo_full_path + "/.git"):
            ...
            msg = (
                'Error, directory "%s" already exists and is not a git repo'
                % self.repo_full_path
            )
        ...
        print(
            'Retrieving git repo %s with "%s", this may take a while' % (self.url, cmd)
        )
...
class Command(stack.commands.create.command, stack.commands.HostArgumentProcessor):
    ...
    def run(self, params, args):
        ...
        if len(args) == 1:
            ...
            builder = MetaRollBuilder(args, name, version, release, self.command)
