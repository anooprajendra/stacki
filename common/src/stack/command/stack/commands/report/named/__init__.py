    ...
    def run(self, params, args):
        ...
        if local_conf.is_file():
            template_file = jinja2.Template(
                local_conf.read_text(), lstrip_blocks=True, trim_blocks=True
            )
        else:
            raise CommandError(
                self,
                "Unable to parse template file: /opt/stack/share/templates/named.conf.j2",
            )
        ...
        for network in networks:
            acl.append(
                str(ipaddress.IPv4Interface(f"{network['address']}/{network['mask']}"))
            )
        ...
        for zone in z:
            if len(z[zone]) == 1:
                ...
                comment_name = "networks - %s" % ",".join(
                    list(map(lambda x: x["network"], z[zone]))
                )
