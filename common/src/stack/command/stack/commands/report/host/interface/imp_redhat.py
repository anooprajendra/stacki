...
class Implementation(stack.commands.Implementation):
    ...
    def writeConfig(
        self, host, mac, ip, device, netmask, vlanid, mtu, options, channel, default
    ):
    ...
    def writeModprobe(self, host, device, module, options):
        ...
        self.owner.addOutput(
            host, 'grep -v "\<%s\>" /etc/modprobe.conf > /tmp/modprobe.conf' % (device)
        )

        self.owner.addOutput(
            host, "echo 'alias %s %s' >> /tmp/modprobe.conf" % (device, module)
        )
        ...
        if modopts:
            self.owner.addOutput(
                host, "echo 'options %s %s' >> /tmp/modprobe.conf" % (module, options)
            )
