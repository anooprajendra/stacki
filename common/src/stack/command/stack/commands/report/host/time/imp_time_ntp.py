...
class Implementation(stack.commands.Implementation):
    ...
    def client(self, host):
        ...
        for network in self.owner.getNTPNetworks(host):
            ...
            self.owner.addOutput(
                host, f"restrict {network.network_address} mask {network.netmask}"
            )
        ...
        if orphantype == "parent":
            ...
            for network in self.owner.getNTPNetworks(host):
                self.owner.addOutput(
                    host,
                    f"# Broadcast NTP over the {network.network_address}/{network.prefixlen} network",
                )
                self.owner.addOutput(
                    host, f"broadcast {network.broadcast_address} key 1"
                )
    ...
    def run(self, host):
        ...
        self.owner.addOutput(host, 'OPTIONS="-A -u ntp:ntp -p /var/run/ntpd.pid"')
        self.owner.addOutput(host, "</stack:file>")
        ...
        if self.owner.timeservers:
            self.owner.addOutput(
                host, "/usr/sbin/ntpdate %s" % self.owner.timeservers[0]
            )
