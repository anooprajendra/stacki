...
class Command(
    ...
    def writeIPMI(self, host, ip, channel, netmask, gateway, vlan=0):
        ...
        self.addOutput(host, "ipmitool lan set %s ipsrc static" % (channel))
        self.addOutput(host, "ipmitool lan set %s ipaddr %s" % (channel, ip))
        self.addOutput(host, "ipmitool lan set %s netmask %s" % (channel, netmask))

        if not gateway:
            gateway = "0.0.0.0"
        self.addOutput(host, "ipmitool lan set %s defgw ipaddr %s" % (channel, gateway))

        self.addOutput(host, "ipmitool lan set %s arp respond on" % (channel))
        ...
        self.addOutput(host, "ipmitool lan set %s access on" % (channel))
        self.addOutput(host, "ipmitool lan set %s user" % (channel))
        self.addOutput(host, "ipmitool lan set %s auth ADMIN PASSWORD" % (channel))
        ...
        self.addOutput(host, "ipmitool user set password 2 %s" % (password))
