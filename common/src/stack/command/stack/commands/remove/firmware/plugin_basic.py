...
class Plugin(stack.commands.Plugin):
    ...
    def build_query(self, make, model, versions):
        ...
        if versions:
            query = query.format(
                "firmware.version IN %s AND firmware_make.name=%s AND firmware_model.name=%s"
            )
    ...
    def run(self, args):
        ...
        make, model = lowered(
            self.owner.fillParams(names=[("make", ""), ("model", "")], params=params,)
        )
        ...
        query, query_params = self.build_query(
            make=make, model=model, versions=versions
        )
        # remove the file and then the db entry for each firmware to remove
        for firmware_id, file_path in (
            (row[0], row[1]) for row in self.owner.db.select(query, query_params)
        ):
