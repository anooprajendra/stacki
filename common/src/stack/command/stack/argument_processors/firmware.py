...
class FirmwareArgumentProcessor:
    ...
    def ensure_unique_makes(self, makes):
        ...
        existing_makes = [
            make
            for make, exists in ((make, self.make_exists(make)) for make in makes)
            if exists
        ]
        if existing_makes:
            raise CommandError(
                cmd=self,
                msg=f"The following firmware makes already exist: {existing_makes}.",
            )
    ...
    def ensure_makes_exist(self, makes):
        ...
        if missing_makes:
            raise CommandError(
                cmd=self,
                msg=f"The following firmware makes don't exist: {missing_makes}.",
            )
    ...
    def ensure_model_exists(self, make, model):
        ...
        if not self.model_exists(make=make, model=model):
            raise CommandError(
                cmd=self, msg=f"Make {make} does not exist for model {model}."
            )
    ...
    def ensure_unique_models(self, make, models):
        ...
        if existing_makes_models:
            raise CommandError(
                cmd=self,
                msg=f"The following make and model combinations already exist {existing_makes_models}.",
            )
    ...
    def ensure_firmwares_exist(self, make, model, versions):
        ...
        missing_versions = [
            ...
            for version, exists in (
                (version, self.firmware_exists(make, model, version))
    ...
    def get_firmware_id(self, make, model, version):
        ...
        if not row:
            raise CommandError(
                cmd=self,
                msg=f"Firmware version {version} doesn't exist for make {make} and model {model}.",
            )
    ...
    def get_common_frontend_ip(self, hostname):
		...
		"""
        host_interface_frontend = self.call(
            command="list.host.interface", args=["a:frontend"]
        )
        # try to get the set of all common networks between the front end and the target host
        host_networks = set(
            host["network"]
            for host in self.call(command="list.host.interface", args=[hostname])
        )
        frontend_networks = set(
            frontend_interface["network"]
            for frontend_interface in host_interface_frontend
        )
        ...
        ip_addr = [
            frontend_interface["ip"]
            for frontend_interface in host_interface_frontend
            if frontend_interface["network"] in common_networks
            and frontend_interface["ip"]
        ]
    ...
    def get_firmware_url(self, hostname, firmware_file):
        ...
        firmware_file = Path().joinpath(
            *(
                part
                for part in firmware_file.parts
                if part not in ("/", "export", "stack")
            )
        )
    ...
    def ensure_imp_exists(self, imp):
        ...
        if not self.imp_exists(imp=imp):
            raise CommandError(
                cmd=self, msg=f"Imp {imp} does not exist in the database."
            )
    ...
    def ensure_imps_exist(self, imps):
        ...
        missing_imps = [
            imp
            for imp, exists in ((imp, self.imp_exists(imp)) for imp in imps)
            if not exists
        ]
        if missing_imps:
            raise CommandError(
    ...
    def get_imp_id(self, imp):
        ...
        if not row:
            raise CommandError(
                cmd=self,
                msg=f"Firmware implementation {imp} doesn't exist in the database.",
            )
    ...
    def ensure_version_regex_exists(self, name):
        ...
        if not self.version_regex_exists(name=name):
            raise CommandError(
                cmd=self, msg=f"A version regex named {name} does not exist."
            )
    ...
    def ensure_version_regexes_exist(self, names):
        ...
        if not all(names):
            raise CommandError(
                cmd=self, msg="A version regex name cannot be an empty string."
            )
    ...
    def get_version_regex_id(self, name):
        ...
        if not row:
            raise CommandError(
                cmd=self,
                msg=f"Firmware version_regex {name} doesn't exist in the database.",
            )
