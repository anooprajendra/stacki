...
from stack.restapi.models import BlackList, SudoList
    ...
    def _check_login_(func):
        def runner(inst, *args, **kwargs):
            ...
            if request.user.is_authenticated:
                ...
                return HttpResponseForbidden(str(j), content_type="application/json")
    ...
    def post(self, request):
        ...
        if self._blacklisted(mod_name):
            return HttpResponseForbidden(
                f"Blacklisted Command: Command {mod_name} is not permitted",
        ...
        if not request.user.has_perm(mod_name):
            return HttpResponseForbidden(
                "Unauthorized Command: User %s is not allowed to run %s"
        ...
        try:
            ...
            if not command:
                ...
                return HttpResponse(
                    str(json.dumps(output)), content_type="application/json", status=404
                )
                ...
                # command module wrapper directly.
                    ...
                    # don't expose database access.
                        ...
                        if int(e.args[0]) in MYSQL_EX:
                            errortext = (
                                "Database Permission Denied. Admin privileges required"
                            )
                            status_code = 403
                        else:
                            status_code = 500
                    ...
                    try:
                        ...
                        j = {"Output": text}
                    return HttpResponse(
                        str(json.dumps(j)), content_type="application/json",
                    )
...
def log_in(request):
    ...
    if user is not None:
        login(request, user)
        s = {"login": "True"}
        return HttpResponse(str(json.dumps(s)), content_type="application/json")
    else:
        s = {"login": "False"}
        return HttpResponse(
            str(json.dumps(s)), status=401, content_type="application/json"
        )
