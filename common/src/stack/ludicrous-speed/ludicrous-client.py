...
def four_o_four(error=None):
    ...
    message = {"status": 404, "message": error_message}
...
def lookup_file(hashcode):
    try:
        ...
        res = requests.get(
            "http://%s/ludicrous/lookup/%s" % (tracker(), hashcode), timeout=(0.1, 5)
        )
        return res
    except:
        raise
...
def register_file(port, hashcode):
    ...
    while _counter < 3:
        try:
            res = requests.post(
                "http://%s/ludicrous/register/%s/%s" % (tracker(), port, hashcode),
                timeout=(0.1, 5),
            )
            break
        except requests.ConnectTimeout:
...
def unregister_file(hashcode, params):
    ...
    while _counter < 3:
        try:
            res = requests.delete(
                "http://%s/ludicrous/unregister/hashcode/%s" % (tracker(), hashcode),
                params=params,
                timeout=(0.1, 5),
            )
            break
...
def unregister_host(host):
    ...
    while _counter < 3:
        try:
            res = requests.delete(
                "http://%s/ludicrous/unregister/host/%s" % (tracker(), host),
                timeout=(0.1, 5),
            )
            break
        except requests.ConnectTimeout:
...
def get_file_locally(path, filename):
    ...
    if (
        client_settings["SAVE_FILES"]
        and im_the_requester
        and not file_exists(local_file)
    ):
    ...
    if not file_exists(local_file):
        ...
        while True:
            ...
            try:
                # timeout=(connect timeout, read timeout).
                tracker_res = requests.get(
                    "http://%s%s" % (tracker_settings["TRACKER"], remote_file),
                    timeout=(0.1, 5),
                )
                ...
                app.logger.info(
                    "Frontend Request: Error requesting %s from frontend" % filename
                )
                sleep(1)
    ...
    if file_exists(local_file):
        ...
        return redirect(
            "http://%s%s" % (tracker_settings["TRACKER"], remote_file), code=307
        )
