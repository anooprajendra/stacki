...
class Profile(profile.ProfileBase):
    def set_mac_interface(self, host, mac, interface):
        stack.api.Call(
            ...
            [host, "mac=%s" % mac, "interface=%s" % interface],
        )
    ...
    def main(self, client):
        output = stack.api.Call(
            "list host attr", [client.addr, "attr=profile.force_eth0"]
        )
        ...
        p = subprocess.run(
            ["/opt/stack/bin/stack", "list", "host", "xml", client.addr],
