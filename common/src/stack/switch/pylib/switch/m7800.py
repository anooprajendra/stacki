...
from . import Switch, SwitchException, mellanoknok
...
partition_name = re.compile(
    r"(?P<format_1>  [a-z0-9]+\s*$)|(?P<format_2>[a-z0-9]+:\s*$)", re.IGNORECASE
)
...
guid_member_format = re.compile(
    "(ALL|([0-9a-f]{2}:){7}[0-9a-f]{2}).*(full|both|limited)", re.IGNORECASE
)
...
class SwitchMellanoxM7800(Switch):
    ...
    SUPPORTED_IMAGE_FETCH_PROTOCOLS = (
        "http://",
        "https://",
        "ftp://",
        "tftp://",
    ...
    def __init__(
        self, switch_ip_address, switchname="switch", username="admin", password=""
    ):
    ...
    def connect(self):
        ...
        try:
            ...
            raise SwitchException(
                f'Connection to switch at "{self.username}@{self.switch_ip_address}" unavailable'
            )
        ...
        try:
            self._api_connection = mellanoknok.Mellanoknok(
                self.switch_ip_address, password=self.password
            )
        except:
            pass
    ...
    def set_hostname(self, hostname):
        ...
        while self.subnet_manager:
            ...
            if i > 11:
                raise SwitchException(
                    "waited over 30 seconds for subnet manager state to change"
                )
    ...
    def add_partition(self, partition="Default", pkey=None, defmember=None, ipoib=None):
		...
		"""
        info(
            f"adding partition {partition} pkey={pkey} defmember={defmember} ipoib={ipoib}"
        )
        if partition != "Default":
            if not pkey:
                raise SwitchException(
                    f"a partition key is required for partition: {partition}."
                )
            pkey = self._validate_pkey(pkey)
            if not pkey:
                raise SwitchException(
                    f"Infiniband partition keys must be between 2 and 32766, not {pkey}"
                )
        else:
            pkey = "0x7fff"
    ...
    def image_fetch(self, url):
        ...
        if not url.startswith(self.SUPPORTED_IMAGE_FETCH_PROTOCOLS):
            raise SwitchException(
                f"Image fetch URL must be one of the following supported protocols {self.SUPPORTED_IMAGE_FETCH_PROTOCOLS}"
            )
    ...
    def _get_relevant_responses(self, command_response, start_marker, end_marker):
        ...
        block_indices = [
            index
            for index, message in enumerate(command_response)
            if re.search(fr"{start_marker}|{end_marker}", message, re.IGNORECASE)
        ]
        if len(block_indices) != 2:
            raise SwitchException(
                f"Ambiguous block. Expected one start and one end marker: {block_indices}"
            )
        ...
        if not re.search(
            start_marker, command_response[start_index], re.IGNORECASE
    ...
    def _get_installed_images(self, command_response):
        ...
        for index, message in enumerate(relevant_responses):
            partition_header_match = re.search(
                partition_header_regex, message, re.IGNORECASE
            )
            if partition_header_match:
                ...
                try:
                    value = re.sub(
                        r"\s*version\s*:",
                        "",
                        relevant_responses[index + 1],
                        re.IGNORECASE,
                    ).strip()
                except IndexError:
                    raise SwitchException(
                        f"No installed image listed for partition {key} when one was expected."
                    )
        ...
        if not installed_images:
            raise SwitchException(
                "No installed images found in the show images response."
            )
        ...
        for response in remove_blank_lines(lines=command_response):
            ...
            if match:
                ...
                if partition_name in boot_partitions:
                    raise SwitchException(
                        f"Ambiguous boot partitions. Got multiple entries for boot partition {match.group('partition_name')}"
                    )
    ...
    def _get_available_images(self, command_response):
        ...
        relevant_responses = [
            ...
            if not re.search(
                r"image.*available to be installed", response, re.IGNORECASE
            )
        ...
        for index, message in enumerate(relevant_responses):
            ...
            try:
                filename = re.sub(
                    r"\s*Image\s*:", "", message, flags=re.IGNORECASE
                ).strip()
                version = re.sub(
                    r"\s*Version\s*:",
                    "",
                ...
                available_images.append(
                    AvailableImage(filename=filename, version=version)
                )
            except IndexError:
                raise SwitchException(
                    f"Missing version number for image file {message.strip()}"
                )
    ...
    def show_images(self):
        ...
        last_boot_partition, next_boot_partition = self._get_boot_partitions(
            command_response=images_text
        )
        ...
        return ImagesListing(
            ...
            images_fetched_and_available=self._get_available_images(
                command_response=images_text
            ),
        )
    ...
    def _get_errors(self, command_response):
        ...
        return [
            error_string
            for error_string in command_response
    ...
    def disable_fallback_reboot(self):
        ...
        if any(errors):
            raise SwitchException(
                f"Disabling fallback reboot failed with error: {errors}"
            )
