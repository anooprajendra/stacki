        ...
        if self.proc.match_index != self.CONSOLE_PROMPTS.index(self.CONSOLE_PROMPT):
            ...
            if self.proc.match_index != self.CONSOLE_PROMPTS.index(self.CONSOLE_PROMPT):
                raise SwitchException(
                    f"Could not get to the base switch console for {self.switchname}. Did a firmware update change the prompts?"
                )
...
def _requires_configure_console(member_function):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if self.proc.match_index != self.CONSOLE_PROMPTS.index(
            self.CONFIGURE_CONSOLE_PROMPT
        ):
            ...
            if self.proc.match_index != self.CONSOLE_PROMPTS.index(
                ...
                raise SwitchException(
                    f"Could not get to the configure console for {self.switchname}. Did a firmware update change the prompts?"
                )
...
class SwitchDellX1052(Switch):
    ...
    def connect(self):
        ...
        try:
            ...
            self.proc.start(
                cmd=f"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -tt {self.switch_ip_address}"
            )
    ...
    def get_mac_address_table(self):
        ...
        for line in results:
            if "dynamic" in line:
                ...
                #   from the interface
                _hosts.append(list(map(lambda x: x.split("/")[-1], line.split())))
    ...
    def _read_paged_output(self, cmd):
        ...
        return [
            re.sub(self.PAGING_PROMPT, "", line, flags=re.IGNORECASE)
            for line in results
        ]
    ...
    def download(self):
        ...
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to download the running config from {self.switchname}"
            )
    ...
    def upload(self):
        ...
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to upload the new config to {self.switchname}"
            )
        ...
        #
        results = self.proc.ask(
            cmd="copy temp running-config", prompt=prompts, timeout=30
        )
        # If it's prompting to confirm overwrite, say yes
        if self.proc.match_index == prompts.index(self.OVERWRITE_PROMPT):
        ...
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to ovewrite the running config on {self.switchname}"
            )
    ...
    def apply_configuration(self):
        ...
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to apply the new config to {self.switchname}"
            )
    ...
    def _parse_versions(self, results):
        ...
        for line in results:
            ...
            software_match = re.search(
                pattern=software_regex, string=line, flags=re.IGNORECASE
            )
            ...
            if boot_match:
                boot.append(
                    # Drop the heading and strip leading and trailing whitespace.
                    re.sub(
                        pattern=boot_regex, repl="", string=line, flags=re.IGNORECASE
                    ).strip()
                )

            hardware_regex = r"HW version"
            hardware_match = re.search(
                pattern=hardware_regex, string=line, flags=re.IGNORECASE
            )
        ...
        return namedtuple("ParsedVersions", ("software", "boot", "hardware"))(
            software=software, boot=boot, hardware=hardware,
        )
    ...
    def _check_parsed_versions(self, parsed_versions):
        ...
        if not parsed_versions.software:
            ...
            errors.append(
                f"Multiple software versions reported for {self.switchname}: {parsed_versions.software}."
            )
        # Else we're good
        ...
        if not parsed_versions.boot:
            ...
            errors.append(
                f"Multiple boot versions reported for {self.switchname}: {parsed_versions.boot}."
            )
        # Else we're good
        ...
        if not parsed_versions.hardware:
            ...
            errors.append(
                f"Multiple hardware versions reported for {self.switchname}: {parsed_versions.hardware}."
            )
        # Else we're good
    ...
    def upload_software(self, software_file):
        ...
        results = self._upload_tftp(
            source=software_file, destination="image", timeout=600
        )

        # ensure we copied successfully
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to upload new software file {software_file} to {self.switchname}"
            )
    ...
    def upload_boot(self, boot_file):
        ...
        if not self._check_success(results=results, success_string=self.COPY_SUCCESS):
            raise SwitchException(
                f"Failed to upload new boot file {boot_file} to {self.switchname}"
            )
