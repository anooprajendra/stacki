...
class Implementation(stack.commands.Implementation):
    def clearPort(self):
        ...
        self.owner.addOutput(
            "localhost", " switchport general allowed vlan remove 2-4094"
        )
    ...
    def doPort(self, switch, host, port):
        ...
        for s in self.owner.call("list.switch.host", [switch]):
            if s["host"] == host and s["port"] == port:
                ...
                if s["interface"] == "ipmi":
                    ...
                    self.owner.addOutput(
                        "localhost", " switchport general pvid %s" % vlan
                    )
        ...
        if not native:
            ...
            #
            self.owner.addOutput(
                "localhost", " switchport general allowed vlan add 1 untagged"
            )
            self.owner.addOutput("localhost", " switchport general pvid 1")
    ...
    def run(self, args):
        ...
        switch_network, *xargs = self.owner.call(
            "list.network", [switch_interface["network"]]
        )
