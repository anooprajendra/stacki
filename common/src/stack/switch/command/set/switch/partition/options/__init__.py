    ...
    def run(self, params, args):
        ...
        if options_param:
            ...
            if options:
                ...
                raise CommandError(
                    self, msg + " ".join(f"{k}={v}" for k, v in options.items())
                )
            if bad_options:
        ...
        sw_ids = dict(
            (row[0], row[1]) for row in self.db.select(ids_sql, tuple(switches))
        )
        ...
        for switch in switches:
            ...
            if not stack_set:
                raise CommandError(
                    self, f'partition "{name}" already exists on switch "{switch}"'
                )
