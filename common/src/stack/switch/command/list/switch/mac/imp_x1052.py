...
class Implementation(stack.commands.Implementation):
    def run(self, args):
        ...
        if self.owner.pinghosts:
            _host_interfaces = [
                host
                for host in self.owner.call("list.host.interface")
                if host["network"] == switch["network"]
            ]
            for host in _host_interfaces:
                x = subprocess.Popen(
                    ["ping", "-c", "1", host["ip"]], stdout=subprocess.PIPE
                )
        ...
        with SwitchDellX1052(
            ...
            try:
                ...
                for _vlan, _mac, _port, _ in hosts:
                    ...
                    if row:
                        ...
                        self.owner.addOutput(
                            switch_name, [_port, _mac, _hostname, _interface, _vlan]
                        )
                ...
                raise CommandError(
                    self,
                    f"There was an error getting the mac address table. {exception}",
                )
