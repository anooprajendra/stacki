...
class StackWSClient:
    ...
    def login(self):
        if not self.logged_in:
            ...
            self.session.headers.update(
                {"csrftoken": csrftoken, "X-CSRFToken": csrftoken,}
            )
    ...
    def run(self, cmd):
        ...
        if not self.logged_in:
            self.login()
        if (
            cmd.startswith("load ") or cmd.startswith("unload ")
        ) and "json-file=" not in cmd:
        ...
        js = (
            json.dumps({"cmd": cmd, "data": data}) if data else json.dumps({"cmd": cmd})
        )
