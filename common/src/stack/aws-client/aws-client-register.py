...
def Download(url, dst):
    ...
    p = subprocess.Popen(
        ["/usr/bin/curl", url], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    o, e = p.communicate()
    out = open(dst, "wb")
    out.write(o)
...
p = subprocess.Popen(
    [
        ...
        "1-100",
        "--insecure",
        "https://%s/install/sbin/register.cgi?%s"
        % (instructions.master, "&".join(args)),
    ],
...
if instructions.boot == "install":
    if instructions.kernel:
        Download(
            os.path.join(instructions.images_url, instructions.kernel),
            os.path.join(os.sep, "boot", instructions.kernel),
        )

    if instructions.ramdisk:
        Download(
            os.path.join(instructions.images_url, instructions.ramdisk),
