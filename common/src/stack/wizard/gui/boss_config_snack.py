...
def render_network(screen, data):
    ...
    # create input fields
    e0 = Entry(21, text=data.get("Kickstart_PrivateHostname"), scroll=1, returnExit=0)
    e1 = Entry(21, text=data.get("Kickstart_PrivateAddress"), scroll=0, returnExit=0)
    e2 = Entry(21, text=data.get("Kickstart_PrivateNetmask"), scroll=0, returnExit=0)
    e3 = Entry(21, text=data.get("Kickstart_PrivateGateway"), scroll=0, returnExit=0)
    e4 = Entry(21, text=data.get("Kickstart_PrivateDNSServers"), scroll=0, returnExit=0)
    ...
    result = (
        btn_value,
        (e0.value(), lb.current(), e1.value(), e2.value(), e3.value(), e4.value()),
    )
    return result
...
def render_pallets(screen, data):
    ...
    while True:
        ...
        # Create the buttons
        buttons = ButtonBar(
            screen, (("Continue", "continue"), ("Add Pallets", "add"), ("Back", "back"))
        )
...
def process_data(page, btn_value, result):
    ...
    if btn_value == "back":
        ...
        # timezone
        if page == 1:
            validated, message, title = data.validateTimezone(str(result))
        # network
        elif page == 2:
            fqhn, eth, ip, subnet, gateway, dns = result

            # check the FQDN for appliance names
            validated, message, title = data.validateDomain(fqhn)
            if validated:
                # check for valid network config
                validated, message, title = data.validateNetwork(
                    (fqhn, eth, ip, subnet, gateway, dns), config_net
                )
        # password
        elif page == 3:
            pw1, pw2 = result
        ...
        # pallets
            ...
            validated, message, title = data.validatePallets(dvd_pallets, net_pallets)
        ...
        if not validated:
            ButtonChoiceWindow(screen, title, message, buttons=["Ok"])
        else:
