...
class TestLoadCommand:
    ...
    @patch.object(
        target=CommandUnderTest, attribute="validate_partition", autospec=True
    )
    ...
    def test_load_partition(
        ...
        if not force:
            mock_validate_partition.assert_called_once_with(
                test_command, partitions=partitions
            )
        else:
    ...
    @patch.object(target=CommandUnderTest, attribute="stack", autospec=True)
    @patch.object(target=CommandUnderTest, attribute="get_scope", autospec=True)
    def test_load_partition_no_partitions(
        self, mock_get_scope, mock_stack, mock_validate_partition, test_input
    ):
    ...
    def test_load_partition_non_global_scope_with_no_target(
        self, mock_get_scope, mock_stack, mock_validate_partition, partitions,
    ):
    ...
    def test_stack(self, mock__exec_commands, args, params, expected_params):
        ...
        mock__exec_commands.assert_called_once_with(
            test_command, cmd=cmd, args=args, params=expected_params,
        )
