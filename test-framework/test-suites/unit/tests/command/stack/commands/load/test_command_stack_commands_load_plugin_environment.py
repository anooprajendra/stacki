...
class PluginUnderTest(Plugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.owner = create_autospec(spec=command, spec_set=True, instance=True)
...
def test_run(test_file):
    ...
    for environment in dump_json_section:
        ...
        test_plugin.owner.load_controller.assert_any_call(
            environment.get("controller"), name
        )
        test_plugin.owner.load_partition.assert_any_call(
            environment.get("partition"), name
        )
        test_plugin.owner.load_firewall.assert_any_call(
            environment.get("firewall"), name
        )
