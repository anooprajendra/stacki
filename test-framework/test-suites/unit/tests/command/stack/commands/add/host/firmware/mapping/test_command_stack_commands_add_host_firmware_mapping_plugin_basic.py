...
class TestAddHostFirmwareMappingBasicPlugin:
    ...
    def test_ensure_unique_mappings(self, basic_plugin):
        ...
        basic_plugin.ensure_unique_mappings(
            hosts=mock_hosts, version=mock_version, make=mock_make, model=mock_model,
        )

        basic_plugin.owner.db.select.assert_called_once_with(
            ANY, (mock_hosts, mock_make, mock_model, mock_version),
        )
    ...
    def test_ensure_unique_mappings_error(self, basic_plugin):
        ...
        mock_version = "bam"
        basic_plugin.owner.db.select.return_value = [
            [mock_hosts[0], mock_make, mock_model, mock_version]
        ]
    ...
    @patch(
        target="stack.commands.add.host.firmware.mapping.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.add.host.firmware.mapping.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("version", ""), ("make", ""), ("model", ""),], params=mock_params,
        )
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ...
            [
                (
                    ...
                    basic_plugin.owner.get_firmware_id.return_value,
                )
            ],
            many=True,
        )
