...
from stack.exception import ArgError, CommandError, ParamError
...
class TestAddImpBasicPlugin:
    ...
    def test_validate_imp(self, mock_getsourcefile, mock_path, basic_plugin):
        ...
        mock_path.return_value.parent.resolve.return_value.__truediv__.return_value.exists.return_value = (
            True
        )
        mock_imp = "foo"
    ...
    @patch(target="stack.commands.add.firmware.imp.plugin_basic.Path", autospec=True)
    @patch(target="inspect.getsourcefile", autospec=True)
    def test_validate_imp_already_exists(
        self, mock_getsourcefile, mock_path, basic_plugin
    ):
    ...
    @patch(target="stack.commands.add.firmware.imp.plugin_basic.Path", autospec=True)
    @patch(target="inspect.getsourcefile", autospec=True)
    def test_validate_imp_missing_files(
        self, mock_getsourcefile, mock_path, return_values, basic_plugin
    ):
        """Test that validate_imp fails when the implementation files are missing from the filesystem."""
        basic_plugin.owner.imp_exists.return_value = False
        mock_path.return_value.parent.resolve.return_value.__truediv__.return_value.exists.side_effect = (
            return_values
        )
        mock_imp = "foo"
    ...
    @patch(
        target="stack.commands.add.firmware.imp.plugin_basic.ExitStack", autospec=True
    )
    ...
    def test_run(
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("make", ""), ("models", "")], params=mock_params,
        )
        ...
        mock_exit_stack.return_value.__enter__.return_value.callback.assert_called_once_with(
            basic_plugin.owner.call, command="remove.firmware.imp", args=[expected_imp],
        )
    ...
    @pytest.mark.parametrize(
        "failure_mock", ("ensure_models_exist", "validate_imp", "validate_args")
    )
    @patch(target="stack.commands.add.firmware.imp.plugin_basic.lowered", autospec=True)
    @patch(
        target="stack.commands.add.firmware.imp.plugin_basic.ExitStack", autospec=True
    ...
    def test_run_cleanup(
        ...
        basic_plugin.owner.call.side_effect = CommandError(
            cmd=basic_plugin.owner, msg="Test error"
        )
