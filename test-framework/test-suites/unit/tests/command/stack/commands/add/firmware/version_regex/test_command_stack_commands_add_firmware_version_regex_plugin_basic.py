...
class TestAddVersionRegexBasicPlugin:
    ...
    @pytest.mark.parametrize("test_input, return_value", (("", False), ("foo", True),))
    ...
    @patch(
        target="stack.commands.add.firmware.version_regex.plugin_basic.unique_everseen",
        autospec=True,
    )
    @patch(
        target="stack.commands.add.firmware.version_regex.plugin_basic.ExitStack",
        autospec=True,
    )
    ...
    def test_run(
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("name", ""), ("description", ""), ("make", ""), ("models", ""),],
            params=mock_params,
        )
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, (mock_args[0], expected_name, mock_params["description"]),
        )
        basic_plugin.owner.call.assert_called_once_with(
            command="set.firmware.model.version_regex",
            args=[
                *expected_models,
                f"make={expected_make}",
                f"version_regex={expected_name}",
            ],
        )
