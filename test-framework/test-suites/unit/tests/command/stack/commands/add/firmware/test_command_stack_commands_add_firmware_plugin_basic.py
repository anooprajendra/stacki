...
from stack.exception import ArgError, CommandError, ParamError, ParamRequired
...
class TestAddFirmwareBasicPlugin:
    ...
    def test_validate_imp_errors(
        self, imp, model_exists_return, call_return, basic_plugin
    ):
    ...
    def test_validate_hash_alg_supported(
        self, mock_ensure_hash_alg_supported, basic_plugin
    ):
    ...
    def test_validate_version(self, mock_search, regex, basic_plugin):
        ...
        basic_plugin.validate_version(
            version=mock_version, make=mock_make, model=mock_model
        )
        ...
        if regex:
            mock_search.assert_called_once_with(
                pattern=regex.regex, string=mock_version, flags=re.IGNORECASE,
            )
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                MockRegexReturn(name="test", regex="foo", description="test"),
                False,
                False,
            ),
            (
    ...
    @patch(target="re.search", autospec=True)
    def test_validate_version_errors(
        self, mock_search, regex, search_return, firmware_return, basic_plugin
    ):
        """Test that validate version regex works as expected when the version is valid."""
        mock_version = "fizz"
        mock_make = "buzz"
    ...
    def test_validate_inputs(
        ...
        mock_validate_version.assert_called_once_with(
            basic_plugin, make=mock_make, model=mock_model, version=mock_version,
        )
        mock_validate_imp.assert_called_once_with(
            basic_plugin, imp=mock_imp, make=mock_make, model=mock_model,
        )
        mock_validate_hash_alg_supported.assert_called_once_with(
            basic_plugin, hash_alg=mock_hash_alg,
        )
    ...
    def test_create_missing_imp(self, basic_plugin):
        ...
        with mock_exit_stack() as mock_cleanup:
            ...
            basic_plugin.owner.call.assert_called_once_with(
                command="add.firmware.imp", args=[mock_imp],
            )
            mock_cleanup.callback.assert_called_once_with(
                basic_plugin.owner.call, command="remove.firmware.imp", args=[mock_imp],
            )
    ...
    def test_create_missing_make(self, basic_plugin):
        ...
        with mock_exit_stack() as mock_cleanup:
            ...
            # Make sure appropriate calls were made
            basic_plugin.owner.call.assert_called_once_with(
                command="add.firmware.make", args=[mock_make],
            )
            mock_cleanup.callback.assert_called_once_with(
    ...
    def test_create_missing_model(self, basic_plugin):
        ...
        with mock_exit_stack() as mock_cleanup:
            basic_plugin.create_missing_model(
                make=mock_make, model=mock_model, imp=mock_imp, cleanup=mock_cleanup,
            )
    ...
    def test_create_missing_related_entries(
        ...
        with mock_exit_stack() as mock_cleanup:
            ...
            mock_create_missing_make.assert_called_once_with(
                basic_plugin, make=mock_make, cleanup=mock_cleanup,
            )
            mock_create_missing_imp.assert_called_once_with(
                basic_plugin, imp=mock_imp, cleanup=mock_cleanup,
            )

    @pytest.mark.parametrize(
        "failure_mock",
        ("create_missing_imp", "create_missing_make", "create_missing_model"),
    )
    @patch.object(target=Plugin, attribute="create_missing_imp", autospec=True)
    @patch.object(target=Plugin, attribute="create_missing_make", autospec=True)
    @patch.object(target=Plugin, attribute="create_missing_model", autospec=True)
    def test_create_missing_related_entries_errors(
        ...
        mock_creation_functions[failure_mock].side_effect = CommandError(
            cmd=basic_plugin.owner, msg="Test error"
        )
    ...
    def test_fetch_firmware(self, mock_file_cleanup, mock_fetch_firmware, basic_plugin):
        ...
        with mock_exit_stack() as mock_cleanup:
            ...
            mock_cleanup.callback.assert_called_once_with(
                basic_plugin.file_cleanup, file_path=mock_fetch_firmware.return_value,
            )
            assert mock_fetch_firmware.return_value == result

    @patch(target="stack.firmware.fetch_firmware", autospec=True)
    @patch.object(target=Plugin, attribute="file_cleanup", autospec=True)
    def test_fetch_firmware_error(
        self, mock_file_cleanup, mock_fetch_firmware, basic_plugin
    ):
    ...
    @patch.object(
        target=Plugin, attribute="create_missing_related_entries", autospec=True
    )
    ...
    def test_add_firmware_errors(
        ...
        mock_functions[failure_mock].side_effect = CommandError(
            cmd=basic_plugin.owner, msg="Test error"
        )
    ...
    @patch(
        target="stack.commands.add.firmware.plugin_basic.unique_everseen", autospec=True
    )
    ...
    def test_run(
        ...
        expected_hosts = tuple(
            host.strip() for host in mock_params["hosts"].split(",") if host.strip()
        )
