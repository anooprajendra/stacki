...
class TestAddFirmwareModelBasicPlugin:
    ...
    def test_create_missing_make(self, basic_plugin):
        ...
        mock_exit_stack.return_value.__enter__.return_value.callback.assert_called_once_with(
            basic_plugin.owner.call, command="remove.firmware.make", args=[mock_make],
        )
    ...
    @patch(
        target="stack.commands.add.firmware.model.plugin_basic.ExitStack", autospec=True
    )
    @patch(
        target="stack.commands.add.firmware.model.plugin_basic.lowered", autospec=True
    )
    @patch(
        target="stack.commands.add.firmware.model.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        basic_plugin.owner.ensure_unique_models.assert_called_once_with(
            models=expected_models, make=mock_params["make"],
        )
