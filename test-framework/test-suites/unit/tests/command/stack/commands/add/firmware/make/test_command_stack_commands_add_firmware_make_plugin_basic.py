...
class TestAddMakeBasicPlugin:
    ...
    @patch(
        target="stack.commands.add.firmware.make.plugin_basic.lowered", autospec=True
    )
    @patch(
        target="stack.commands.add.firmware.make.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, [(expected_make,) for expected_make in expected_makes], many=True,
        )
