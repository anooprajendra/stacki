...
class CommandUnderTest(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.db = create_autospec(spec=DatabaseConnection, instance=True,)
...
class TestCommand:
    ...
    def test_run_implementations_parallel(self, mock_executor, mock_loadImplementation):
        ...
        mock_future2.exception.return_value = Exception(
            "And that's when Jimmy burst into flames."
        )
        ...
        assert result == {
            "foo": namedtuple("_", ("result", "exception"))(
                result=mock_future1.result(), exception=mock_future1.exception(),
            ),
            "baz": namedtuple("_", ("result", "exception"))(
                result=mock_future2.result(), exception=mock_future2.exception(),
            ),
        }
    ...
    def test_run_implementations_parallel_real_threads(self, mock_loadImplementation):
        ...
        def load_impl(self, name):
            """Side effect for mock_loadImplementation to set the impl_list"""
            mock_imp = create_autospec(
                spec=Implementation, spec_set=True, instance=True
            )
        ...
        assert result == {
            key: namedtuple("_", ("result", "exception"))(
                result="Look ma, I'm threading!", exception=None,
            )
            for key in test_implementation_mapping
        }

    @patch.object(Command, "loadImplementation", autospec=True)
    def test_run_implementations_parallel_real_threads_real_errors(
        self, mock_loadImplementation
    ):
        """Test the normal case with real threads, but mocking the implementation run function.
        ...
        def load_impl(self, name):
            ...
            mock_imp.run.side_effect = Exception(
                "And that's when Jimmy burst into flames."
            )
            self.impl_list[name] = mock_imp
    ...
    @patch.object(Command, "loadImplementation", autospec=True)
    @patch("stack.commands.ThreadPoolExecutor", autospec=True)
    def test_run_implementations_parallel_empty_args(
    ...
    @patch.object(Command, "loadImplementation", autospec=True)
    @patch("stack.commands.ThreadPoolExecutor", autospec=True)
    def test_run_implementations_parallel_not_loadable(
        self, mock_executor, mock_loadImplementation
    ):
    ...
    @patch.object(Command, "loadImplementation", autospec=True)
    @patch("stack.commands.ThreadPoolExecutor", autospec=True)
    def test_run_implementations_parallel_partial_loadable(
        self, mock_executor, mock_loadImplementation
    ):
        ...
        mock_executor.return_value.__enter__.return_value.submit.assert_called_once_with(
            ANY, name="baz", args="bog",
        )

    @patch.object(Command, "loadImplementation", autospec=True)
    @patch("stack.commands.ThreadPoolExecutor", autospec=True)
    def test_run_implementations_parallel_already_loaded(
        self, mock_executor, mock_loadImplementation
    ):
        ...
        assert result == {
            key: namedtuple("_", ("result", "exception"))(
                result=mock_future.result(), exception=mock_future.exception(),
            )
            for key in test_implementation_mapping
        }
    ...
    @patch.object(Command, "loadImplementation", autospec=True)
    @patch("stack.commands.ThreadPoolExecutor", autospec=True)
    def test_run_implementations_parallel_output_enabled_fast_implementations(
        self, mock_executor, mock_loadImplementation
    ):
        ...
        result = test_command.run_implementations_parallel(
            implementation_mapping=test_implementation_mapping, display_progress=True
        )
    ...
    @patch.object(Command, "loadImplementation", autospec=True)
    def test_run_implementations_parallel_output_enabled_slow_implementations(
        self, mock_loadImplementation
    ):
        ...
        test_command.impl_list = {key: next(gen) for key in test_implementation_mapping}
        ...
        assert result == {
            key: namedtuple("_", ("result", "exception"))(result=None, exception=None,)
            for key in test_implementation_mapping
        }
    ...
    def test_command_command_error_exception_handling(self, mock_eval, mock__import__):
        ...
        mock_eval.return_value.Command.return_value.runWrapper.side_effect = CommandError(
            cmd=create_autospec(spec=Command, instance=True), msg="test error",
        )
    ...
    @patch(target="stack.commands.__import__", create=True)
    @patch(target="stack.commands.eval", create=True)
    def test_command_command_error_exception_handling_verbose_off(
        self, mock_eval, mock__import__
    ):
        ...
        with pytest.raises(CommandError) as exception_info:
            test_command.command(
                command="foo.bar.baz", args=["a", "b=c"], verbose_errors=False
            )
    ...
    def test_command_exception_handling(
        self, mock_eval, mock__import__, verbose_errors
    ):
        ...
        with pytest.raises(RuntimeError) as exception_info:
            test_command.command(
                command="foo.bar.baz", args=["a", "b=c"], verbose_errors=verbose_errors
            )
