...
class TestRemoveFirmwareVersionRegexBasicPlugin:
    ...
    @patch(
        target="stack.commands.remove.firmware.version_regex.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.remove.firmware.version_regex.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(self, mock_unique_everseen, mock_lowered, basic_plugin):
        ...
        basic_plugin.owner.ensure_version_regexes_exist.assert_called_once_with(
            names=expected_args
        )
