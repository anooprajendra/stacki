...
class TestRemoveFirmwareMakeBasicPlugin:
    ...
    def test_remove_related_models(self, basic_plugin):
        ...
        assert [
            call(ANY, make) for make in mock_makes
        ] == basic_plugin.owner.db.select.mock_calls
        assert [
            call(
                command="remove.firmware.model", args=[*expected_models, f"make={make}"]
            )
            for make in mock_makes
    ...
    @patch(
        target="stack.commands.remove.firmware.make.plugin_basic.lowered", autospec=True
    )
    @patch(
        target="stack.commands.remove.firmware.make.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        basic_plugin.owner.ensure_makes_exist.assert_called_once_with(
            makes=expected_args
        )
        mock_remove_related_models.assert_called_once_with(
            basic_plugin, makes=expected_args
        )
    ...
    def test_run_makes_do_not_exist(
        self,
        mock_unique_everseen,
        mock_lowered,
        mock_remove_related_models,
        ...
        basic_plugin.owner.ensure_makes_exist.side_effect = CommandError(
