...
class TestRemoveHostFirmwareMappingBasicPlugin:
    ...
    def test_get_firmware_mappings_to_remove(
        self, hosts, versions, make, model, basic_plugin
    ):
        ...
        ] == basic_plugin.get_firmware_mappings_to_remove(**test_inputs)
        basic_plugin.owner.db.select.assert_called_once_with(ANY, expected_query_params)

    @patch.object(
        target=Plugin, attribute="get_firmware_mappings_to_remove", autospec=True
    )
    ...
    @patch(
        target="stack.commands.remove.host.firmware.mapping.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.remove.host.firmware.mapping.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        expected_versions = tuple(
            version.strip()
            for version in mock_params["versions"].split(",")
            if version.strip()
        )
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("make", ""), ("model", ""), ("versions", ""),], params=mock_params,
        )
        mock_validate_make.assert_called_once_with(
            basic_plugin, make=mock_params["make"],
        )
        mock_validate_model.assert_called_once_with(
            basic_plugin, make=mock_params["make"], model=mock_params["model"],
        )
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, (mock_get_firmware_mappings_to_remove.return_value,),
        )
    ...
    @pytest.mark.parametrize(
        "failure_mock", ("validate_make", "validate_model", "ensure_firmwares_exist")
    )
    @patch.object(
        target=Plugin, attribute="get_firmware_mappings_to_remove", autospec=True
    )
