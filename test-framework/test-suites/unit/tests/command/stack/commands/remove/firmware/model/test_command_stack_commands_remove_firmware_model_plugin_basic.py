...
class TestRemoveFirmwareModelBasicPlugin:
    ...
    def test_remove_related_firmware(self, basic_plugin):
        ...
        assert [
            call(ANY, (mock_make, model)) for model in mock_models
    ...
    @patch(
        target="stack.commands.remove.firmware.model.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.remove.firmware.model.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, (expected_args, mock_params["make"])
        )
        assert [
    ...
    def test_run_models_do_not_exist(
        self,
        mock_unique_everseen,
        mock_lowered,
        mock_remove_related_firmware,
