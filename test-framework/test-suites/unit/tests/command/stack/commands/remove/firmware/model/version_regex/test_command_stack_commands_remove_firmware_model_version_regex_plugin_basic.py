...
from stack.exception import ArgRequired, CommandError, ParamError, ParamRequired
...
class TestRemoveFirmwareModelVersionRegexBasicPlugin:
    ...
    @patch(
        target="stack.commands.remove.firmware.model.version_regex.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.remove.firmware.model.version_regex.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(self, mock_unique_everseen, mock_lowered, basic_plugin):
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, (expected_args, expected_param)
        )
        ...
        basic_plugin.owner.ensure_models_exist.assert_called_once_with(
            models=expected_args, make=expected_param
        )
    ...
    def test_run_errors(
        self, mock_unique_everseen, mock_lowered, basic_plugin,
    ):
        ...
        basic_plugin.owner.ensure_models_exist.side_effect = CommandError(
            cmd=basic_plugin.owner, msg="Test failure"
        )
