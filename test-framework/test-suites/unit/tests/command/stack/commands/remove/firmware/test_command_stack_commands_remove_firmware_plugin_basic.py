...
class TestRemoveFirmwareBasicPlugin:
    ...
    def test_validate_inputs(self, make, model, versions, basic_plugin):
        ...
        if make:
            basic_plugin.owner.ensure_make_exists.assert_called_once_with(make=make,)
        else:
        ...
        if versions:
            basic_plugin.owner.ensure_firmwares_exist.assert_called_once_with(
                make=make, model=model, versions=versions,
            )
        else:
    ...
    @pytest.mark.parametrize(
        "failure_mock",
        ("ensure_make_exists", "ensure_model_exists", "ensure_firmwares_exist"),
    )
    ...
    def test_build_query(
        self, mock_make, mock_model, mock_versions, expected_query_params, basic_plugin,
    ):
    ...
    @patch(
        target="stack.commands.remove.firmware.plugin_basic.unique_everseen",
        autospec=True,
    )
    ...
    def test_run(
        ...
        basic_plugin.owner.db.select.assert_called_once_with(
            mock_query, mock_query_params
        )
        chained = (
            call(basic_plugin.owner.db.select.return_value[0][1])
            .resolve(strict=True)
            .unlink()
        )
        assert chained.call_list() == mock_path.mock_calls
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, basic_plugin.owner.db.select.return_value[0][0],
        )
    ...
    def test_run_errors(
        ...
        mock_build_query.return_value = (mock_query, mock_query_params)
        basic_plugin.owner.db.select.return_value = [["id", "path"]]
        mock_validate_inputs.side_effect = CommandError(
            cmd=basic_plugin.owner, msg="Test error"
        )
