...
class TestSetFirmwareModelVersionRegexBasicPlugin:
    ...
    @patch(
        target="stack.commands.set.firmware.model.version_regex.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.set.firmware.model.version_regex.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(self, mock_unique_everseen, mock_lowered, basic_plugin):
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY,
            (
                basic_plugin.owner.get_version_regex_id.return_value,
                expected_args,
                mock_params["make"],
            ),
        )
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("make", ""), ("version_regex", "")], params=mock_params,
        )
        assert [
            call(mock_args),
    ...
    @pytest.mark.parametrize(
        "failure_mock", ("ensure_models_exist", "ensure_version_regex_exists")
    )
    @patch(
        target="stack.commands.set.firmware.model.version_regex.plugin_basic.lowered",
        autospec=True,
