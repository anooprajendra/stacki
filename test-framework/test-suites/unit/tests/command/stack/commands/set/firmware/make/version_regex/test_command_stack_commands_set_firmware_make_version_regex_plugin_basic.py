...
class TestSetFirmwareMakeVersionRegexBasicPlugin:
    ...
    @patch(
        target="stack.commands.set.firmware.make.version_regex.plugin_basic.lowered",
        autospec=True,
    )
    @patch(
        target="stack.commands.set.firmware.make.version_regex.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(self, mock_unique_everseen, mock_lowered, basic_plugin):
        ...
        basic_plugin.owner.db.execute.assert_called_once_with(
            ANY, (basic_plugin.owner.get_version_regex_id.return_value, expected_args),
        )
        basic_plugin.owner.get_version_regex_id.assert_called_once_with(
            name=mock_params["version_regex"]
        )
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("version_regex", "")], params=mock_params,
        )
        assert [
            call(mock_args),
        ...
        basic_plugin.owner.ensure_version_regex_exists.assert_called_once_with(
            name=mock_params["version_regex"]
        )

    @pytest.mark.parametrize(
        "failure_mock", ("ensure_makes_exist", "ensure_version_regex_exists")
    )
    @patch(
        target="stack.commands.set.firmware.make.version_regex.plugin_basic.lowered",
        autospec=True,
