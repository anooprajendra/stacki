...
class TestListFirmwareModelBasicPlugin:
    ...
    def test_validate_models(self, basic_plugin):
        ...
        basic_plugin.owner.ensure_models_exist.assert_called_once_with(
            make=mock_make, models=mock_models
        )
    ...
    def test_get_expanded_results(self, make, models, expected, basic_plugin):
        ...
        expected_results = {
            ...
            "values": [
                (row[0], row[1:]) for row in basic_plugin.owner.db.select.return_value
            ],
        }

        assert expected_results == basic_plugin.get_expanded_results(
    ...
    @patch(
        target="stack.commands.list.firmware.model.plugin_basic.lowered", autospec=True
    )
    @patch(
        target="stack.commands.list.firmware.model.plugin_basic.unique_everseen",
        autospec=True,
    )
    def test_run(
        self,
        ...
        basic_plugin.owner.fillParams.assert_called_once_with(
            names=[("expanded", "false"), ("make", "")], params=mock_params,
        )
        ...
        mock_validate_models.assert_called_once_with(
            basic_plugin, make=mock_params["make"], models=expected_args,
        )
