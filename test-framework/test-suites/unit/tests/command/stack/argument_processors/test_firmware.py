...
class TestFirmwareArgumentProcessor:
    ...
    def test_get_make_id(self, argument_processor):
        ...
        assert argument_processor.db.select.return_value[0][
            0
        ] == argument_processor.get_make_id(make=mock_make,)
    ...
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="make_exists", autospec=True
    )
    ...
    def test_ensure_make_exists_errors(
        self, mock_make_exists, test_input, return_value, argument_processor
    ):
    ...
    def test_ensure_unique_makes(self, mock_make_exists, argument_processor):
        ...
        assert [
            call(argument_processor, mock_make) for mock_make in mock_makes
        ] == mock_make_exists.mock_calls

    @pytest.mark.parametrize(
        "test_input, return_value",
        (([], False), (["", "", ""], False), (["foo", "bar", "baz"], True)),
    )
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="make_exists", autospec=True
    )
    def test_ensure_unique_makes_error(
        self, mock_make_exists, test_input, return_value, argument_processor
    ):
    ...
    @pytest.mark.parametrize(
        "test_input, return_value",
        (([], True), (["", "", ""], True), (["foo", "bar", "baz"], False)),
    )
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="make_exists", autospec=True
    )
    def test_ensure_makes_exist_error(
        self, mock_make_exists, test_input, return_value, argument_processor
    ):
    ...
    def test_get_model_id(self, argument_processor):
        ...
        argument_processor.db.select.assert_called_once_with(
            ANY, (mock_make, mock_model)
        )
    ...
    def test_model_exists(self, return_value, argument_processor):
        ...
        assert return_value == argument_processor.model_exists(
    ...
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="model_exists", autospec=True
    )
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="ensure_make_exists", autospec=True
    )
    def test_ensure_model_exists(
        self, mock_ensure_make_exists, mock_model_exists, argument_processor
    ):
        ...
        mock_ensure_make_exists.assert_called_once_with(
            argument_processor, make=mock_make
        )
        mock_model_exists.assert_called_once_with(
            argument_processor, make=mock_make, model=mock_model
        )
    ...
    def test_ensure_unique_models_error(
        self, mock_model_exists, test_input, return_value, argument_processor
    ):
    ...
    def test_firmware_exists(self, return_value, argument_processor):
        ...
        assert return_value == argument_processor.firmware_exists(
            make=mock_make, model=mock_model, version=mock_version,
        )
        argument_processor.db.count.assert_called_once_with(
            ANY, (mock_make, mock_model, mock_version)
        )

    @patch.object(
        target=FirmwareArgumentProcessor, attribute="firmware_exists", autospec=True
    )
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="ensure_model_exists", autospec=True
    )
    def test_ensure_firmware_exists(
        self, mock_ensure_model_exists, mock_firmware_exists, argument_processor
    ):
        ...
        argument_processor.ensure_firmware_exists(
            make=mock_make, model=mock_model, version=mock_version
        )
        ...
        mock_firmware_exists.assert_called_once_with(
            argument_processor, make=mock_make, model=mock_model, version=mock_version,
        )
    ...
    def test_ensure_firmwares_exist(
        self, mock_ensure_model_exists, mock_firmware_exists, argument_processor
    ):
        ...
        argument_processor.ensure_firmwares_exist(
            make=mock_make, model=mock_model, versions=mock_versions,
        )
    ...
    # which the class under test is expected to be mixed in with.
    @patch.object(target=FirmwareArgumentProcessor, attribute="call", create=True)
    def test_get_common_frontend_ip_no_common_network(
        self, mock_call, argument_processor
    ):
    ...
    # which the class under test is expected to be mixed in with.
    @patch.object(target=FirmwareArgumentProcessor, attribute="call", create=True)
    def test_get_common_frontend_ip_no_interface_ip(
        self, mock_call, argument_processor
    ):
    ...
    @patch.object(
        target=FirmwareArgumentProcessor,
        attribute="get_common_frontend_ip",
        autospec=True,
    )
    @patch(target="stack.argument_processors.firmware.Path", autospec=True)
    def test_get_firmware_url(
        self, mock_path, mock_get_common_frontend_ip, argument_processor
    ):
        ...
        mock_path.return_value.joinpath.return_value.__str__.return_value = (
            expected_path
        )
        expected_url = (
            f"http://{mock_get_common_frontend_ip.return_value}/install/{expected_path}"
        )
        mock_firmware_file = "foo"
        mock_hostname = "bar"

        result = argument_processor.get_firmware_url(
            hostname=mock_hostname, firmware_file=mock_firmware_file,
        )
        ...
        mock_get_common_frontend_ip.assert_called_once_with(
            argument_processor, hostname=mock_hostname
        )
    ...
    @patch(target="stack.argument_processors.firmware.Path", autospec=True)
    def test_get_firmware_url_file_does_not_exist(
        self, mock_path, mock_get_common_frontend_ip, argument_processor
    ):
        ...
        with pytest.raises(CommandError):
            argument_processor.get_firmware_url(
                hostname="foo", firmware_file="bar",
            )
    ...
    @patch(target="stack.argument_processors.firmware.Path", autospec=True)
    def test_get_firmware_url_no_common_ip(
        self, mock_path, mock_get_common_frontend_ip, argument_processor
    ):
    ...
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="imp_exists", autospec=True
    )
    ...
    def test_ensure_imp_exists_errors(
        self, mock_imp_exists, mock_imp, return_value, argument_processor
    ):
    ...
    def test_ensure_imps_exist(self, mock_imp_exists, argument_processor):
        ...
        assert [
            call(argument_processor, mock_imp) for mock_imp in mock_imps
        ] == mock_imp_exists.mock_calls

    @pytest.mark.parametrize(
        "mock_imps, return_value",
        (([], True), (["", "", ""], True), (["bar", "baz", "bag"], False),),
    )
    @patch.object(
        target=FirmwareArgumentProcessor, attribute="imp_exists", autospec=True
    )
    def test_ensure_imps_exist_error(
        self, mock_imp_exists, mock_imps, return_value, argument_processor,
    ):
    ...
    @patch.object(
        target=FirmwareArgumentProcessor,
        attribute="version_regex_exists",
        autospec=True,
    )
    def test_ensure_version_regex_exists(
        self, mock_version_regex_exists, argument_processor
    ):
        ...
        mock_version_regex_exists.assert_called_once_with(
            argument_processor, name=mock_name
        )
    ...
    def test_ensure_version_regex_exists_errors(
        self, mock_version_regex_exists, mock_name, return_value, argument_processor
    ):
        """Test that ensure_version_regex_exists works in the case where the input is valid and the version regex exists in the database."""
    ...
    def test_ensure_version_regexes_exist(
        self, mock_version_regex_exists, argument_processor
    ):
        ...
        assert [
            call(argument_processor, mock_name) for mock_name in mock_names
        ] == mock_version_regex_exists.mock_calls
    ...
    def test_ensure_version_regexes_exist_error(
        self, mock_version_regex_exists, mock_names, return_value, argument_processor
    ):
        ...
        (
            (
                [
                    [
                        ...
                        None,
                        "mock_model_regex",
                        "mock_model_regex_name",
                        "mock_model_regex_description",
                    ]
                ],
                (
                    "mock_model_regex",
            ...
            (
                [
                    [
                        "mock_make_regex",
                        "mock_make_regex_name",
                        "mock_make_regex_description",
                        None,
                        None,
                        None,
                    ]
