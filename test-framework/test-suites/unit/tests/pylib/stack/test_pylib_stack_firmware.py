...
class TestSupportedSchemes:
    ...
    def test_pretty_string(self):
        ...
        assert (
            ", ".join(stack.firmware.SUPPORTED_SCHEMES.__members__.keys())
...
class TestFirmware:
    ...
    def test_calculate_hash(self, mock_path, mock_ensure_hash_alg_supported, hash_alg):
        ...
        try:
            expected_hash = hashlib.new(
                name=hash_alg, data=mock_path.return_value.read_bytes.return_value
            ).hexdigest()
        except TypeError:
        ...
        assert expected_hash == stack.firmware.calculate_hash(
            file_path=mock_file, hash_alg=hash_alg, hash_value=expected_hash,
        )
        ...
        assert expected_hash == stack.firmware.calculate_hash(
            file_path=mock_file, hash_alg=hash_alg,
        )
        # Expect the path to be used to read the file
        mock_path.assert_called_once_with(mock_file)
        mock_path.return_value.read_bytes.assert_called_once_with()
    ...
    @patch(target="stack.firmware.ensure_hash_alg_supported", autospec=True)
    @patch(target="stack.firmware.Path", autospec=True)
    def test_calculate_hash_mismatched_provided_hash(
        self, mock_path, mock_ensure_hash_alg_supported, hash_alg
    ):
        ...
        with pytest.raises(stack.firmware.FirmwareError):
            stack.firmware.calculate_hash(
                file_path="foo", hash_alg=hash_alg, hash_value="foo",
            )

    @patch(target="stack.firmware.ensure_hash_alg_supported", autospec=True)
    @patch(target="stack.firmware.Path", autospec=True)
    def test_calculate_hash_unsupported_hash(
        self, mock_path, mock_ensure_hash_alg_supported
    ):
    ...
    def test_fetch_firmware(
        self, mock_fetch, mock_base_path, mock_path, mock_uuid4, scheme
    ):
        ...
        result = stack.firmware.fetch_firmware(
            source=mock_url, make=mock_make, model=mock_model, username=mock_username,
        )

        # Make sure base_dir is used to properly build the target directory
        chained_calls = (
            call.__truediv__(mock_make)
            .__truediv__(mock_model)
            .resolve()
        ...
        call_list.append(
            ...
            .__truediv__()
            .resolve()
            .__truediv__(mock_uuid4.return_value.hex)
        )
        ...
        mock_final_file = (
            mock_base_path.__truediv__.return_value.__truediv__.return_value.resolve.return_value.__truediv__.return_value
        )
        ...
        if scheme == stack.firmware.SUPPORTED_SCHEMES.file:
            ...
            stack.firmware.SUPPORTED_SCHEMES.http,
            stack.firmware.SUPPORTED_SCHEMES.https,
        ):
    ...
    @patch(target="stack.firmware.BASE_PATH", autospec=True)
    @patch(target="stack.download.fetch", autospec=True)
    def test_fetch_firmware_errors(
        self, mock_fetch, mock_base_path, mock_path, mock_uuid4, scheme
    ):
        ...
        with pytest.raises(stack.firmware.FirmwareError):
            stack.firmware.fetch_firmware(
                source=mock_url, make=mock_make, model=mock_model,
            )
    ...
    @patch(target="stack.firmware.BASE_PATH", autospec=True)
    @patch(target="stack.download.fetch", autospec=True)
    def test_fetch_firmware_unsupoorted_scheme(
        self, mock_fetch, mock_base_path, mock_path, mock_uuid4
    ):
    ...
    @patch(target="stack.firmware.BASE_PATH", autospec=True)
    @patch(target="stack.download.fetch", autospec=True)
    def test_fetch_firmware_forgotten_scheme(
        self, mock_fetch, mock_base_path, mock_path, mock_uuid4, mock_schemes
    ):
    ...
    @patch(target="stack.firmware.BASE_PATH", autospec=True)
    @patch(target="stack.download.fetch", autospec=True)
    def test_fetch_firmware_local_path(
        self, mock_fetch, mock_base_path, mock_path, mock_uuid4
    ):
