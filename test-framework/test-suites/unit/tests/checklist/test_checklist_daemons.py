...
from stack.checklist.threads import (
    CheckTimeouts,
    GlobalQueueAdder,
    LogParser,
    MQProcessor,
...
class TestChecklistDaemons:
    def test_LogParser_dhcp(self):
        ...
        logParser.start()
        s = (
            "fe-hostname dhcpd: DHCPREQUEST for 8.8.8.8 "
            "(1.1.1.1) from 55:55:55:55:55:55 via br0  avtest"
        )
        expectedStMsg = StateMessage("8.8.8.8", State.DHCPREQUEST, False, time.time())
    ...
    def test_LogParser_tftp(self):
        ...
        logParser.start()
        s = (
            '8.8.8.8 - - [02/Oct/2018:17:22:56 +0000] "GET '
            '/install/sbin/profile.cgi?os=sles&arch=x86_64&np=4 HTTP/1.1" 200 51261'
        )
        s = (
            "fe-hostname in.tftpd[3339]: RRQ from 8.8.8.8 "
            "filename pxelinux.cfg/MACaddr"
        )
        expectedStMsg = StateMessage(
            "8.8.8.8", State.TFTP_RRQ, False, time.time(), "pxelinux.cfg/MACaddr"
        )
    ...
    def test_LogParser_SSLAccessLog(self):
        ...
        expectedStMsg = StateMessage(
            "8.8.8.8", State.Profile_XML_Sent, False, time.time()
        )
    ...
    def test_GlobalQueueAdder(self):
        ...
        qAdder.daemon = True
        qAdder.start()
        sm = StateMessage("8.8.8.8", State.Cracklib_Dict_Sent, False, time.time())
        localQ.put(sm)
        sm1 = globalQ.get()
        matchedFlag = False
    ...
    def test_MQProcessors(self):
        ...
        expectedSm = StateMessage(
            "8.8.8.8",
            State.Partition_XML_Present,
            False,
            time.time(),
