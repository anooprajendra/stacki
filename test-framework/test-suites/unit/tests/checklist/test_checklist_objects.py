...
class TestBackend:
    ...
    def test_isPostPkgInstallStage(self):
        ...
        sm3 = StateMessage(
            "10.25.241.111", State.Ludicrous_Populated, False, time.time()
        )
...
class TestStateSequence:
    def test_nextExpectedState(self):
        ...
        assert (
            StateSequence.nextExpectedState(State.Set_Bootaction_OS, "sles", "12.x")
            == State.Rebooting_HDD
        )
        assert (
            StateSequence.nextExpectedState(State.Rebooting_HDD, "sles", "12.x")
            != State.DHCPDISCOVER
        )
    ...
    def text_getTimeThreshold(self):
        ...
        assert (
            StateSequence.getTimeThreshold(State.Set_Bootaction_OS, "sles", "12.x")
            == 210
        )
