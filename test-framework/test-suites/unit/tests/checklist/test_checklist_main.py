...
class TestChecklistMain:
    ...
    def test_processTftp(self):
        ...
        sm1 = StateMessage(
            "8.8.8.8", State.TFTP_RRQ, False, time.time(), "initrd-sles-12-sp3-x86_64"
        )
        assert (
            c.processTftp(sm1) == Checklist.SUCCESS_TFTP
            and sm1.state == State.Initrd_RRQ
        )
        sm2 = StateMessage(
            "8.8.8.9", State.TFTP_RRQ, False, time.time(), "initrd-sles-12-sp3-x86"
        )
        assert c.processTftp(sm2) == Checklist.REFRESH_STATE
        sm3 = StateMessage(
            "8.8.8.8", State.TFTP_RRQ, False, time.time(), "vmlinuz-sles-12-sp3-x86_64"
        )
        assert (
            c.processTftp(sm3) == Checklist.SUCCESS_TFTP
            and sm3.state == State.VMLinuz_RRQ_Install
        )
        sm4 = StateMessage("8.8.8.9", State.TFTP_RRQ, False, time.time(), "chain.c32")
        assert (
            c.processTftp(sm4) == Checklist.SUCCESS_TFTP
            and sm4.state == State.Rebooting_HDD
        )
    ...
    def test_restoreDhcpMsgs(self):
        ...
        sm1 = StateMessage("8.8.8.8", State.Set_DB_Partitions, False, time.time())
        sm2 = StateMessage("8.8.8.8", State.Set_Bootaction_OS, False, time.time())
        ...
        sm4 = StateMessage("8.8.8.9", State.AUTOINST_Present, False, time.time())
