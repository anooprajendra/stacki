...
INPUT_DATA = [
    ...
    [
        "m7800_parse_partitions_input_new_format.txt",
        "m7800_parse_partitions_output.json",
    ],
    [
        "m7800_parse_partitions_with_member_named_all_input.txt",
        "m7800_parse_partitions_with_member_named_all_output.json",
    ],
    [
        "m7800_parse_partitions_with_member_named_all_input_new_format.txt",
        "m7800_parse_partitions_with_member_named_all_output.json",
    ],
]
...
class TestSwitchMellanoxM7800:
    def test___init__(self, mock_expectmore):
        ...
        test_switch = SwitchMellanoxM7800(
            switch_ip_address=ip, switchname=name, username=user, password=password
        )
        ...
        assert mock_expectmore.return_value.PROMPTS == (
    ...
    def test_connect(self, mock_mellanoknok, mock_expectmore):
        ...
        test_switch = SwitchMellanoxM7800(
            switch_ip_address="fakeip", password="fakepassword"
        )
        test_switch.connect()
        ...
        mock_mellanoknok.assert_called_with(
            test_switch.switch_ip_address, password=test_switch.password
        )
    ...
    @patch("stack.switch.mellanoknok.Mellanoknok", autospec=True)
    def test_connect_swallow_mellanoknok_exceptions(
        self, mock_mellanoknok, mock_expectmore
    ):
        ...
        try:
            ...
            pytest.fail(
                "SwitchMellanoxM7800.connect() raised an exception when it wasn't expected to."
            )
    ...
    def test_partitions_can_parse_guid_members(
        self, mock_expectmore, input_file, output_file, test_file
    ):
    ...
    def test_install_firmware(self, mock_expectmore, test_file):
        ...
        mock_expectmore.return_value.ask.return_value = (
            Path(test_file("switch/m7800_install_image_console.txt"))
            .read_text()
            .splitlines()
        )
        ...
        mock_expectmore.return_value.ask.assert_called_with(
            f"image install {firmware_name}", timeout=ANY
        )
    ...
    @pytest.mark.parametrize(
        "protocol", SwitchMellanoxM7800.SUPPORTED_IMAGE_FETCH_PROTOCOLS
    )
    ...
    def test_image_fetch_error_with_message(self, mock_expectmore):
        ...
        firmware_url = (
            f"{test_switch.SUPPORTED_IMAGE_FETCH_PROTOCOLS[0]}://sometrustworthysite.ru"
        )
    ...
    RelevantResponsesTestData = namedtuple(
        "RelevantResponsesTestData",
        ["start", "end", "test_response", "expected_output"],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            RelevantResponsesTestData(
                "start", "end", ["start", "boo", "bar", "end"], ["boo", "bar"]
            ),
            ...
            RelevantResponsesTestData(
                "start",
                "end",
                ["irrelevant start", "boo", "bar", "irrelevant end"],
                ["boo", "bar"],
            ),
            RelevantResponsesTestData(
                "start",
                "end",
                ["start irrelevant", "boo", "bar", "end irrelevant"],
                ["boo", "bar"],
            ),
            RelevantResponsesTestData(
                "start", "end", [" start ", "boo", "bar", " end "], ["boo", "bar"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            RelevantResponsesTestData(
                "start", "end", ["end", "boo", "bar", "start"], None
            ),
            # missing start
    ...
    def test__get_installed_images(self, mock_expectmore, input_file, test_file):
        ...
        assert (
            test_switch._get_installed_images(command_response=test_console_response)
            == expected_data
        )
    ...
    def test__get_boot_partitions(self, mock_expectmore, input_file, test_file):
        ...
        assert (
            test_switch._get_boot_partitions(command_response=test_console_response)
            == expected_data
        )
    ...
    def test_disable_fallback_reboot(self, mock__get_errors, mock_expectmore):
        ...
        mock__get_errors.assert_called_with(
            command_response=mock_expectmore.return_value.ask.return_value
        )
