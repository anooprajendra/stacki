...
from stack.switch import Switch, SwitchException
from stack.switch.x1052 import (
    SwitchDellX1052,
...
def powerset(iterable):
    ...
    return itertools.chain.from_iterable(
        itertools.combinations(combinations_list, item)
        for item in range(len(combinations_list) + 1)
    )
...
class Testx1052Decorators:
    ...
    def decorator_test_setup(self):
        ...
        def _setup(**match_index_kwargs):
            ...
            mock_self = create_autospec(spec=SwitchDellX1052, instance=True,)
            mock_self.CONSOLE_PROMPT = SwitchDellX1052.CONSOLE_PROMPT
            mock_self.CONFIGURE_CONSOLE_PROMPT = (
                SwitchDellX1052.CONFIGURE_CONSOLE_PROMPT
            )
            ...
            mock_self.proc = create_autospec(spec=ExpectMore, instance=True,)
    ...
    def test__requires_regular_console(self, decorator_test_setup):
        ...
        mock_method_standin, mock_self = decorator_test_setup(
            side_effect=(
                SwitchDellX1052.CONSOLE_PROMPTS.index(
                    SwitchDellX1052.CONFIGURE_CONSOLE_PROMPT
                ),
    ...
    def test__requires_regular_console_fails_to_get_to_correct_prompt(
        self, decorator_test_setup
    ):
    ...
    def test__requires_configure_console_fails_to_get_to_correct_prompt(
        self, decorator_test_setup
    ):
...
class TestSwitchDellX1052:
    ...
    def test_connect_failure_to_converse(self):
        ...
        test_switch.proc.isalive.return_value = False
        # Make connect fail.
        test_switch.proc.conversation.side_effect = ExpectMoreException(
            "Test exception"
        )
    ...
    def test__read_paged_output(self, mock_timer):
        ...
        expected_prompts = [
            *SwitchDellX1052.CONSOLE_PROMPTS,
            SwitchDellX1052.PAGING_PROMPT,
        ]
        ...
        test_switch = self.SwitchDellX1052UnderTest(
            ...
            after_kwargs={"return_value": ["More: <return>foo"]},
        )
    ...
    def test_download(self, mock_path, mock__check_success):
        ...
        mock_path.return_value.resolve.return_value.__truediv__.assert_called_once_with(
            test_switch.current_config
        )
        # Expect the file to be touched with exist_ok True so the file already existing is not a problem.
        mock_path.return_value.resolve.return_value.__truediv__.return_value.touch.assert_called_once_with(
            exist_ok=True
        )
        # Expect the file to be made accessible by anyone so TFTP can get it.
        mock_path.return_value.resolve.return_value.__truediv__.return_value.chmod.assert_called_once_with(
            mode=0o777
        )
    ...
    def test_upload(self, mock__check_success):
        ...
        expected_prompts = [
            *SwitchDellX1052.CONSOLE_PROMPTS,
            SwitchDellX1052.OVERWRITE_PROMPT,
        ]
        ...
        assert test_switch.proc.ask.call_args_list == [
            call(
                cmd=f"copy tftp://{test_switch.stacki_server_ip}/{test_switch.new_config} temp",
        ...
        assert mock__check_success.call_args_list == [
            call(
                test_switch,
                results=["tftp confirm"],
                success_string=SwitchDellX1052.COPY_SUCCESS,
            ),
            call(
                test_switch,
                results=["switch local copy confirm"],
                success_string=SwitchDellX1052.COPY_SUCCESS,
            ),
        ]
    ...
    def test_apply_configuration_no_overwrite_prompt(self, mock__check_success):
        ...
        # Expect the file to be applied and the overwite prompt to be accepted.
        test_switch.proc.ask.assert_called_once_with(
            cmd="write", prompt=expected_prompts
        )
        # Expect success to be checked
        mock__check_success.assert_called_once_with(
            test_switch, results=["apply"], success_string=SwitchDellX1052.COPY_SUCCESS
        )
    ...
    def test__check_success(self):
        ...
        test_switch = self.SwitchDellX1052UnderTest()
        assert test_switch._check_success(
            results=mock_results, success_string=mock_success_string
        )
    ...
    def test__check_success_not_found(self):
        ...
        test_switch = self.SwitchDellX1052UnderTest()
        assert not test_switch._check_success(
            results=mock_results, success_string=mock_success_string
        )
        ...
        powerset(
            {
                ...
                "hardware": Input(input="HW version    00.00.04", result=["00.00.04"]),
            }.items()
        ),
    )
    ...
    def test__check_parsed_versions(self):
        ...
        mock_parsed_versions = namedtuple(
            "MockParsedVersions", ("software", "boot", "hardware")
        )(
    ...
    @pytest.mark.parametrize(
        ...
        powerset(
            {
                "software": [
                    "3.0.0.94 ( date  10-Sep-2017 time  22:31:38 )",
                    "3.0.0.94 ( date  10-Sep-2017 time  22:31:38 )",
                ],
                "boot": [
                    "1.0.0.25 ( date  05-Apr-2017 time  09:55:19 )",
                    "1.0.0.25 ( date  05-Apr-2017 time  09:55:19 )",
                ],
    ...
    @patch.object(
        target=SwitchDellX1052, attribute="_check_parsed_versions", autospec=True
    )
    ...
    def test_get_versions(self, mock__parse_versions, mock__check_parsed_versions):
        ...
        mock__parse_versions.return_value = namedtuple(
            "MockParsedVersions", ("software", "boot", "hardware")
        )(
            software=["3.0.0.94 ( date  10-Sep-2017 time  22:31:38 )"],
            boot=["1.0.0.25 ( date  05-Apr-2017 time  09:55:19 )"],
        ...
        mock__check_parsed_versions.assert_called_once_with(
            test_switch, parsed_versions=mock__parse_versions.return_value
        )
    ...
    @patch.object(target=SwitchDellX1052, attribute="_parse_versions", autospec=True)
    def test_get_versions_errors(
        self, mock__parse_versions, mock__check_parsed_versions
    ):
    ...
    def test__upload_tftp(self, mock_path, mock_named_temporary_file):
        ...
        test_switch._upload_tftp(
            source=mock_source, destination=mock_destination, timeout=mock_timeout
        )
        ...
        mock_named_temporary_file.assert_called_once_with(
            dir=full_tftp_dir.resolve.return_value
        )
    ...
    def test_upload_software(self, mock__upload_tftp, mock__check_success):
        ...
        mock__upload_tftp.assert_called_once_with(
            test_switch, source=mock_software_file, destination="image", timeout=600,
        )
    ...
    def test_upload_boot(self, mock__upload_tftp, mock__check_success):
        ...
        mock__upload_tftp.assert_called_once_with(
            test_switch, source=mock_software_file, destination="boot", timeout=600,
        )
        # The success should have been checked.
        mock__check_success.assert_called_once_with(
            test_switch,
    ...
    def test_reload(self, mock_timer):
        ...
        expected_prompts = [
            SwitchDellX1052.CONTINUE_PROMPT,
            SwitchDellX1052.SHUTDOWN_PROMPT,
        ]
        test_switch = self.SwitchDellX1052UnderTest(
            match_index_kwargs={
                "side_effect": [
    ...
    def test_reload_no_confirm_prompt(self, mock_timer):
        ...
        test_switch.proc.say.assert_any_call(
            cmd="reload", prompt=expected_prompts,
        )
        # Expect that the process was terminated without any prompts or quit commands
        test_switch.proc.end.assert_called_once_with(quit_cmd="", prompt="")
