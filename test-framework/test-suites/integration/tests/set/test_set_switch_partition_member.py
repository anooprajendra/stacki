...
class TestSetSwitchPartitionMember:
    ...
    def test_set_for_membership_behavior(
        self,
        host,
        add_ib_switch,
        add_ib_switch_partition,
    ...
    def test_can_add_then_set(
        ...
        result = host.run(
            " ".join(cmd).replace("add", "set").replace("member ", "membership ")
        )
        assert result.rc == 0
    ...
    def test_can_duplicate_names_that_resolve_same(
        self, host, add_ib_switch, add_host_with_interface, test_file
    ):
        with open(test_file("add/add_nondefault_member_output.json")) as output:
            expected_output = output.read()
        ...
        result = host.run(
            f"stack set switch partition membership switch-0-0 name={same_parts[0]} guid=00:00:00:00:00:00:00:00"
        )
        assert result.rc == 0
        for partition_name in same_parts[1:]:
            result = host.run(
                f"stack set switch partition membership switch-0-0 name={partition_name} guid=00:00:00:00:00:00:00:00"
            )
            assert result.rc == 0
            result = host.run(
                "stack list switch partition member switch-0-0 output-format=json"
    ...
    def test_cannot_add_to_non_ib(self, host, add_switch):
        result = host.run(
            f"stack set switch partition membership switch-0-0 name=default guid=fake"
        )
        assert result.rc != 0

    def test_cannot_add_with_enforce_sm(
        self, host, add_ib_switch, add_ib_switch_partition
    ):
        # by design this should fail if there's no actual switch to talk to.
        result = host.run(
            f"stack set switch partition membership switch-0-0 name=Default guid=fake enforce_sm=true"
        )
        assert result.rc != 0
    ...
    def test_two_switches_same_partition_name(
        ...
        result = host.run(
            "stack list switch partition member switch-0-1 output-format=json"
        )
        assert result.rc == 0
