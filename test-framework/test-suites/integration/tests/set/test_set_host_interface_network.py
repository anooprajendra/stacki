...
class TestSetHostInterfaceNetwork:
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface network frontend-0-0 interface=eth9 network=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_mac(self, host, add_network):
        result = host.run(
            "stack set host interface network frontend-0-0 mac=00:11:22:33:44:55 network=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_combo(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
        ...
        assert (
            result.stderr
            == 'error - combination of "eth1, 00:11:22:33:44:55" does not exist for host "backend-0-0"\n'
        )

    def test_by_interface(self, host, add_host_with_interface, add_network):
        # Set the host interface network
        result = host.run(
            "stack set host interface network backend-0-0 interface=eth0 network=test"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host, add_network):
        ...
        # Set the host interface network
        result = host.run(
            "stack set host interface network backend-0-0 mac=00:11:22:33:44:55 network=test"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host_with_interface, add_network):
        ...
        result = host.run(
            "stack set host interface network backend-0-0 backend-0-1 interface=eth0 network=test"
        )
        assert result.rc == 0
