...
class TestSetHostInterfaceInterface:
    ...
    def test_no_selector(self, host):
        result = host.run(
            "stack set host interface interface frontend-0-0 interface=eth0"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface interface frontend-0-0 interface=eth2 mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface interface frontend-0-0 interface=eth2 network=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_combo(self, host, add_host, add_network):
        ...
        assert (
            result.stderr
            == 'error - combination of "00:11:22:33:44:55, test" does not exist for host "backend-0-0"\n'
        )
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface interface backend-0-0 interface=eth0 network=test"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the host interface
        result = host.run(
            "stack set host interface interface backend-0-0 interface=eth0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
    ...
    def test_all_parameters_insensitive(self, host, add_host, add_network):
        # Add an interface with a mac and network to our test host
        result = host.run(
            "stack add host interface backend-0-0 mac=aa:bb:cc:dd:ee:ff network=test"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface interface backend-0-0 backend-0-1 interface=eth0 network=test"
        )
        assert result.rc == 0
