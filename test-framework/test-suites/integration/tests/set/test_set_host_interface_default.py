...
class TestSetHostInterfaceDefault:
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface default frontend-0-0 interface=eth9 channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface default frontend-0-0 mac=00:11:22:33:44:55 channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface default frontend-0-0 network=test channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_by_interface(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack set host interface default backend-0-0 interface=eth0 default=false"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the default host interface
        result = host.run(
            "stack set host interface default backend-0-0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack set host interface default backend-0-0 backend-0-1 interface=eth0"
        )
        assert result.rc == 0
