...
class TestSetHostInterfaceOptions:
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface options frontend-0-0 interface=eth9 options=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface options frontend-0-0 mac=00:11:22:33:44:55 options=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface options frontend-0-0 network=test options=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface options backend-0-0 network=test options=test_options"
        )
        assert result.rc == 0
    ...
    def test_by_interface(self, host, add_host_with_interface):
        # Set the host interface options
        result = host.run(
            "stack set host interface options backend-0-0 interface=eth0 options=test_options"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host interface options backend-0-0 interface=eth0 options=null"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the host interface options
        result = host.run(
            "stack set host interface options backend-0-0 mac=00:11:22:33:44:55 options=test_options"
        )
        assert result.rc == 0
