...
class TestSetHostInterfaceMac:
    ...
    def test_no_selector(self, host):
        result = host.run(
            "stack set host interface mac frontend-0-0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface mac frontend-0-0 interface=eth9 mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface mac frontend-0-0 network=test mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface mac backend-0-0 mac=00:11:22:33:44:55 network=test"
        )
        assert result.rc == 0
    ...
    def test_by_interface(self, host, add_host_with_interface):
        # Set the host interface mac
        result = host.run(
            "stack set host interface mac backend-0-0 interface=eth0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host interface mac backend-0-0 interface=eth0 mac=null"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host):
        ...
        result = host.run(
            "stack set host interface mac backend-0-0 backend-0-1 mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
