...
class TestSetSwitchPartition:
    ...
    SWITCH_PARTITION_TEST_DATA = [
        ...
        [
            "AAA",
            "defmember=limited",
            "add_nondefault_partition_with_defmember_limited_output.json",
        ],
        [
            "aaa",
            "defmember=full",
            "add_nondefault_partition_with_defmember_full_output.json",
        ],
        [
            "aaa",
            "ipoib=true defmember=full",
            "add_nondefault_partition_with_multiple_opts_output.json",
        ],
        [
            "aaa",
            "defmember=full ipoib=true",
            "add_nondefault_partition_with_multiple_opts_diff_order_output.json",
        ],
    ]
    ...
    @pytest.mark.parametrize(
        "partition_name,options,output_file", SWITCH_PARTITION_TEST_DATA
    )
    def test_set_as_add_behavior(
        self, host, add_ib_switch, partition_name, options, output_file, test_file
    ):
        ...
        result = host.run(
            f'stack set switch partition options switch-0-0 name={partition_name} options="{options}"'
        )
        assert result.rc == 0
    ...
    def test_set_for_options_behavior(
        self, host, add_ib_switch, partition_name, options, output_file, test_file
    ):
        ...
        for row in TestSetSwitchPartition.SWITCH_PARTITION_TEST_DATA:
            ...
            result = host.run(
                f'stack set switch partition options switch-0-0 name={partition_name} options="{new_opt_str}"'
            )
            assert result.rc == 0
    ...
    @pytest.mark.parametrize(
        "partition_name,options", SWITCH_PARTITION_NEGATIVE_TEST_DATA
    )
    ...
    def test_can_duplicate_names_that_resolve_same(
        self, host, add_ib_switch, test_file
    ):
        ...
        result = host.run(
            f"stack set switch partition options switch-0-0 name={same_parts[0]}"
        )
        assert result.rc == 0
        for partition_name in same_parts[1:]:
            result = host.run(
                f"stack set switch partition options switch-0-0 name={partition_name}"
            )
            assert result.rc == 0
            result = host.run(
                "stack list switch partition switch-0-0 output-format=json"
    ...
    def test_cannot_add_with_enforce_sm(self, host, add_ib_switch):
        ...
        result = host.run(
            f"stack add switch partition switch-0-0 name=Default enforce_sm=true"
        )
        assert result.rc != 0

        result = host.run(
            f"stack set switch partition options switch-0-0 name=Default enforce_sm=true"
        )
        assert result.rc != 0
    ...
    def test_two_switches_same_partition_name(
        ...
        assert result.rc == 0
        result = host.run(
            f'stack set switch partition options switch-0-1 name={partition_name} options="{options}"'
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack list switch partition switch-0-0 switch-0-1 output-format=json"
        )
        assert result.rc == 0
