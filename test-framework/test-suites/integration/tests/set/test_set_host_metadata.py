...
class TestSetHostMetadata:
    ...
    def test_single_host(self, host, add_host):
        ...
        result = host.run(
            "stack list host attr backend-0-0 attr=metadata output-format=json"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host, host_os):
        ...
        result = host.run(
            "stack set host metadata backend-0-0 backend-0-1 metadata=test"
        )
        assert result.rc == 0

        # Check that the change made it into the database
        result = host.run(
            f"stack list host attr a:backend attr=metadata output-format=json"
        )
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {
