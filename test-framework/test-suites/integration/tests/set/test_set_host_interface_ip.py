...
class TestSetHostInterfaceIP:
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface ip frontend-0-0 interface=eth9 ip=127.0.0.1"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == 'error - interface "eth9" does not exist for host "frontend-0-0"\n'
        )

    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface ip frontend-0-0 mac=00:11:22:33:44:55 ip=127.0.0.1"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == 'error - mac "00:11:22:33:44:55" does not exist for host "frontend-0-0"\n'
        )

    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface ip frontend-0-0 network=test ip=127.0.0.1"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == 'error - network "test" does not exist for host "frontend-0-0"\n'
        )

    def test_invalid_combo(self, host, add_host, add_network):
        # Add an interface with an interface and network to our test host
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 network=test"
        )
        assert result.rc == 0

        # Add a second interface with an interface and a mac to our test host
        result = host.run(
            "stack add host interface backend-0-0 interface=eth1 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
        ...
        assert (
            result.stderr
            == 'error - combination of "eth1, test" does not exist for host "backend-0-0"\n'
        )
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 network=test ip=192.168.0.2"
        )
        assert result.rc == 0
    ...
    def test_by_interface(self, host, add_host_with_interface):
        # Set the host IP address
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=192.168.0.2"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=null"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the host IP address
        result = host.run(
            "stack set host interface ip backend-0-0 mac=00:11:22:33:44:55 ip=192.168.0.2"
        )
        assert result.rc == 0
    ...
    def test_by_network_with_auto(self, host, add_host):
        ...
        result = host.run(
            "stack add network test address=10.0.0.0 mask=255.0.0.0 gateway=10.0.0.1"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack add host interface backend-0-1 interface=eth0 network=test"
        )
        assert result.rc == 0

        # Set the IP address on the second test host
        result = host.run(
            "stack set host interface ip backend-0-1 network=test ip=10.0.0.2"
        )
        assert result.rc == 0

        # Set the first test host IP address using AUTO
        result = host.run(
            "stack set host interface ip backend-0-0 network=test ip=auto"
        )
        assert result.rc == 0
    ...
    def test_by_interface_with_auto(self, host, add_host):
        ...
        # Set the IP address on the second test host
        result = host.run(
            "stack set host interface ip backend-0-1 interface=eth0 ip=10.0.0.2"
        )
        assert result.rc == 0

        # Set the first test host IP address using AUTO
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=auto"
        )
        assert result.rc == 0
    ...
    def test_by_mac_with_auto(self, host, add_host):
        ...
        result = host.run(
            "stack add host interface backend-0-0 mac=00:11:22:33:44:55 network=test"
        )
        assert result.rc == 0

        # Add an interface with a different mac to our second test host
        result = host.run(
            "stack add host interface backend-0-1 mac=AA:BB:CC:DD:EE:FF network=test"
        )
        assert result.rc == 0

        # Set the IP address on the second test host
        result = host.run(
            "stack set host interface ip backend-0-1 mac=AA:BB:CC:DD:EE:FF ip=10.0.0.2"
        )
        assert result.rc == 0

        # Set the first test host IP address using AUTO
        result = host.run(
            "stack set host interface ip backend-0-0 mac=00:11:22:33:44:55 ip=auto"
        )
        assert result.rc == 0
    ...
    def test_auto_no_more_ips(self, host, add_host):
        ...
        result = host.run(
            "stack add network test address=192.168.0.0 mask=255.255.255.252 gateway=192.168.0.1"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host interface ip backend-0-1 interface=eth0 ip=192.168.0.2"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host):
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 backend-0-1 ip=192.168.0.1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
