...
class TestSetSwitchHostPort:
    ...
    def test_one_arg(self, host, add_host, add_switch):
        ...
        result = host.run(
            "stack set switch host port switch-0-0 host=backend-0-0 interface=eth0 port=2"
        )
        assert result.rc == 0
    ...
    def test_no_hosts(self, host, add_switch):
        result = host.run(
            "stack set switch host port switch-0-0 host=a:test interface=eth0 port=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_multiple_hosts(self, host, add_host, add_switch):
        ...
        result = host.run(
            "stack set switch host port switch-0-0 host=a:backend interface=eth0 port=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_port(self, host, add_host, add_switch):
        result = host.run(
            "stack set switch host port switch-0-0 host=backend-0-0 interface=eth0 port=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_bad_host_interface_combo(self, host, add_host, add_switch):
        ...
        result = host.run(
            "stack set switch host port switch-0-0 host=backend-0-0 interface=eth1 port=2"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
