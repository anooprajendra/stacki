...
class TestSetSwitchHostInterface:
    ...
    def test_one_arg(self, host, add_host, add_switch):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 network=private"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface backend-0-0 interface=eth1 network=private"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface switch-0-0 interface=eth0 network=private ip=127.0.0.1"
        )
        assert result.rc == 0

        # Add our backend to the test switch
        result = host.run(
            "stack add switch host switch-0-0 host=backend-0-0 interface=eth0 port=1"
        )
        assert result.rc == 0

        # Now set the host interface to be eth1
        result = host.run(
            "stack set switch host interface switch-0-0 host=backend-0-0 interface=eth1 port=1"
        )
        assert result.rc == 0
    ...
    def test_no_hosts(self, host, add_switch):
        result = host.run(
            "stack set switch host interface switch-0-0 host=a:test interface=eth0 port=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_multiple_hosts(self, host, add_host, add_switch):
        ...
        result = host.run(
            "stack set switch host interface switch-0-0 host=a:backend interface=eth0 port=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_port(self, host, add_host, add_switch):
        result = host.run(
            "stack set switch host interface switch-0-0 host=backend-0-0 interface=eth0 port=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_missing_port(self, host, add_host, add_switch):
        result = host.run(
            "stack set switch host interface switch-0-0 host=backend-0-0 interface=eth0 port=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_missing_interface(self, host, add_host, add_switch):
        ...
        assert (
            result.stderr
            == 'error - interface "eth1" does not exist for host "backend-0-0"\n'
        )
