...
class TestSetHostInterfaceChannel:
    def test_no_hosts(self, host):
        ...
        assert result.stderr == dedent(
			...
			{host ...} {channel=string} [interface=string] [mac=string] [network=string]
		"""
        )
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface channel frontend-0-0 interface=eth9 channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface channel frontend-0-0 mac=00:11:22:33:44:55 channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface channel frontend-0-0 network=test channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_combo(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface channel backend-0-0 network=test interface=eth1 channel=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface channel backend-0-0 network=test channel=1"
        )
        assert result.rc == 0
    ...
    def test_by_interface(self, host, add_host_with_interface):
        # Set the host interface channel
        result = host.run(
            "stack set host interface channel backend-0-0 interface=eth0 channel=1"
        )
        assert result.rc == 0
        ...
        # Now set the channel back to NULL
        result = host.run(
            "stack set host interface channel backend-0-0 interface=eth0 channel=null"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the host interface channel
        result = host.run(
            "stack set host interface channel backend-0-0 mac=00:11:22:33:44:55 channel=1"
        )
        assert result.rc == 0
    ...
    def test_all_parameters(self, host, add_host, add_network):
        ...
        # Set the host interface channel
        result = host.run(
            "stack set host interface channel backend-0-0 interface=eth0 "
            "mac=00:11:22:33:44:55 network=test channel=1"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack set host interface channel backend-0-0 backend-0-1 interface=eth0 channel=1"
        )
        assert result.rc == 0
