...
class TestSetHostInterfaceVlan:
    ...
    def test_invalid_interface(self, host):
        result = host.run(
            "stack set host interface vlan frontend-0-0 interface=eth9 vlan=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_mac(self, host):
        result = host.run(
            "stack set host interface vlan frontend-0-0 mac=00:11:22:33:44:55 vlan=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_network(self, host):
        result = host.run(
            "stack set host interface vlan frontend-0-0 network=test vlan=1"
        )
        assert result.rc == 255
        assert (
            result.stderr
    ...
    def test_invalid_vlan(self, host, add_host_with_interface):
        result = host.run(
            "stack set host interface vlan backend-0-0 interface=eth0 vlan=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_by_network(self, host, add_host, add_network):
        ...
        result = host.run(
            "stack set host interface vlan backend-0-0 network=test vlan=1"
        )
        assert result.rc == 0
    ...
    def test_by_interface(self, host, add_host_with_interface):
        # Set the host interface vlan
        result = host.run(
            "stack set host interface vlan backend-0-0 interface=eth0 vlan=1"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host interface vlan backend-0-0 interface=eth0 vlan=0"
        )
        assert result.rc == 0
    ...
    def test_by_mac(self, host, add_host):
        ...
        # Set the host interface vlan
        result = host.run(
            "stack set host interface vlan backend-0-0 mac=00:11:22:33:44:55 vlan=1"
        )
        assert result.rc == 0
