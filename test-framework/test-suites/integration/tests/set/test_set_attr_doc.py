...
class TestSetAttrDoc:
    def test_not_existing(self, host):
        ...
        assert (
            result.stderr
            == "error - Cannot set documentation for a non-existant attribute\n"
        )
    ...
    def test_set_and_update(self, host):
        ...
        assert result.rc == 0
        assert json.loads(result.stdout) == [{"attr": "test", "doc": "foo"}]
        ...
        result = host.run("stack list attr doc attr=test output-format=json")
        assert result.rc == 0
        assert json.loads(result.stdout) == [{"attr": "test", "doc": "bar"}]
