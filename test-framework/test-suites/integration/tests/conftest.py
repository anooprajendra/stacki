...
pytest_plugins = [
    f"fixtures.{p.stem}"
    for p in Path(__file__).parent.glob("fixtures/*.py")
    if p.stem != "__init__"
]


def pytest_addoption(parser):
    parser.addoption("--audit", action="store_true", help="audit filesystem changes")
...
def inject_code(host):
    ...
    def _inner(code_file):
        ...
        result = host.run(f'cp "{code_file}" {site_pkgs_path}/sitecustomize.py')
