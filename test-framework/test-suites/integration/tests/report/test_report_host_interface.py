class TestReportHostInterface:
    def test_exclude_interface_ip_no_network(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=192.0.2.1"
        )
        assert result.rc == 0
    ...
    def test_single(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run("stack add host interface backend-0-0 mac=00:00:00:00:00:03")
        assert result.rc == 0
    ...
    def test_bonded(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 mac=00:00:00:00:00:01 channel=bond0"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface backend-0-0 interface=eth1 mac=00:00:00:00:00:02 channel=bond0"
        )
        assert result.rc == 0
    ...
    def test_vlan(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0.1 network=vlan ip=10.10.10.2 vlan=1"
        )
        assert result.rc == 0
    ...
    def test_ipmi(self, host, add_host, host_os, test_file):
        ...
        result = host.run(
            "stack add network ipmi address=10.10.10.0 mask=255.255.255.0 gateway=10.10.10.1"
        )
        assert result.rc == 0

        # Add an ipmi interface to our test backend
        result = host.run(
            "stack add host interface backend-0-0 interface=ipmi network=ipmi ip=10.10.10.2 channel=1"
        )
        assert result.rc == 0
    ...
    def test_virtual(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0:0 network=test ip=192.168.0.2"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface backend-0-0 interface=eth0:1 network=test ip=192.168.0.3"
        )
        assert result.rc == 0
    ...
    def test_dhcp(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 network=test options=dhcp"
        )
        assert result.rc == 0
    ...
    def test_dhcp_default(self, host, add_host, add_network, host_os, test_file):
        # Add a dhcp interface on the test backend
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 network=test options=dhcp default=true"
        )
        assert result.rc == 0
        ...
        # Does the output match what we expect?
        with open(
            test_file(f"report/host_interface_dhcp_default_{host_os}.txt")
        ) as output:
            assert result.stdout == output.read()
    ...
    def test_manual(self, host, add_host, add_network, host_os, test_file):
        ...
        result = host.run(
            'stack add host interface backend-0-0 interface=eth0 network=test options="onboot=no"'
        )
        assert result.rc == 0
    ...
    def test_only_device(self, host, add_host, add_network, host_os, test_file):
        ...
        # Does the output match what we expect?
        with open(
            test_file(f"report/host_interface_only_device_{host_os}.txt")
        ) as output:
            assert result.stdout == output.read()
