class TestReportHostBootfile:
    ...
    def test_for_host_with_full_IP_config(self, host):
        ...
        assert result.rc == 0
        result = host.run(
            "stack add network public address=192.172.0.0 mask=255.255.255.0 pxe=True"
        )
        assert result.rc == 0
        result = host.run(
            "stack add host interface backend-1-1 interface=eth1 ip=192.172.0.2 network=public default=True mac=52:54:00:6e:50:91"
        )
        assert result.rc == 0
    ...
    def test_for_host_with_interface_but_no_IP_or_network(self, host):
        ...
        assert result.rc == 0
        result = host.run(
            "stack add host interface backend-1-1 interface=eth1 default=True mac=52:54:00:6e:50:91"
        )
        assert result.rc == 0
    ...
    def test_for_combination_of_host_with__no_IP_or_network_and_full_config(self, host):
        ...
        assert result.rc == 0
        result = host.run(
            "stack add host interface backend-2-2 interface=eth1 default=True mac=52:54:00:6e:50:99"
        )
        assert result.rc == 0
