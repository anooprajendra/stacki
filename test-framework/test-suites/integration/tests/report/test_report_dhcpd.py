class TestReportDhcpd:
    def test_duplicate_interface(self, host):
        ...
        assert result.rc == 0
        result = host.run(
            "stack set host interface interface frontend-0-0 mac=00:11:22:33:44:55 interface=eth1"
        )
        assert result.rc == 0
    ...
    def test_invalid_channel(self, host):
        result = host.run(
            "stack add host interface frontend-0-0 interface=eth0 mac=00:11:22:33:44:55 channel=alvin"
        )
        assert result.rc == 0
    ...
    def test_invalid_ipmi_channel(self, host):
        result = host.run(
            "stack add host interface frontend-0-0 interface=ipmi channel=1"
        )
        assert result.rc == 0
    ...
    def test_single_pxe_network_sles(self, host, fake_os_sles, revert_etc, test_file):
        ...
        result = host.run(
            "stack set host interface mac frontend-0-0 interface=eth1 mac=00:11:22:33:44:55"
        )
        assert result.rc == 0
    ...
    def test_single_pxe_network_redhat(
        self, host, fake_os_redhat, revert_etc, test_file
    ):
        # Change the mac on eth1 so it will match our expected outputs
        result = host.run(
            "stack set host interface mac frontend-0-0 interface=eth1 mac=00:11:22:33:44:55"
    ...
    def test_multiple_pxe_networks(self, host, host_os, test_file):
        ...
        result = host.run(
            "stack add network test address=10.0.0.0 mask=255.255.255.0 gateway=10.0.0.1 pxe=true"
        )
        assert result.rc == 0

        # Add a second interface on the test network to the frontend
        result = host.run(
            "stack add host interface frontend-0-0 interface=eth3 network=test"
        )
        assert result.rc == 0
        ...
        with open(
            test_file(f"report/dhcpd_multiple_pxe_networks_{host_os}.txt")
        ) as output:
            assert result.stdout == output.read()
    ...
    def test_shared_network_sles(self, host, fake_os_sles, revert_etc, test_file):
        ...
        result = host.run(
            "stack add network test-0 address=10.0.0.0 mask=255.255.255.0 gateway=10.0.0.1 pxe=true"
        )
        assert result.rc == 0

        result = host.run(
            "stack add network test-1 address=10.0.1.0 mask=255.255.255.0 gateway=10.0.1.1 pxe=true"
        )
        assert result.rc == 0

        # Add two virtual interfaces on the frontend, one on each test network
        result = host.run(
            "stack add host interface localhost interface=eth2:0 network=test-0 ip=10.0.0.2"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface localhost interface=eth2:1 network=test-1 ip=10.0.1.2"
        )
        assert result.rc == 0
    ...
    def test_resolve_ip_exception(self, host, host_os, test_file):
        ...
        result = host.run(
            "stack add host interface frontend-0-0 interface=eth3 channel=eth4"
        )
        assert result.rc == 0
