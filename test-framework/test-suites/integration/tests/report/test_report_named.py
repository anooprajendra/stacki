...
class TestReportNamed:
    ...
    def test_with_local_site_add(self, host, revert_etc):
        ...
        assert rewritten_namedconf != og_namedconf
        rewritten_namedconf = rewritten_namedconf.strip().rstrip(
    ...
    def test_with_added_network(self, host, revert_etc, add_network):
        ...
        result = host.run(
            f'stack set network address {test_net["network"]} address=10.1.1.0'
        )
        assert result.rc == 0
        ...
        for net in nets:
            ...
            acl_line += (
                str(ipaddress.IPv4Interface(f'{net["address"]}/{net["mask"]}')) + ";"
            )
        ...
        rev_zone_addr = (
            ".".join(test_net["address"].split(".")[2::-1]) + ".in-addr.arpa"
        )
