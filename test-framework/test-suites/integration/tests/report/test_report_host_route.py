...
class TestReportHostRoute:
    def test_scope_resolving(
        ...
        result = host.run("stack add route address=192.168.0.3 gateway=private")
        assert result.rc == 0
        ...
        result = host.run("stack add route address=192.168.0.8 gateway=private")
        assert result.rc == 0
        ...
        with open(
            test_file(f"report/host_route_scope_resolving_{host_os}.txt")
        ) as output:
