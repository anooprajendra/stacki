...
class TestReportZones:
    ...
    def custom_entries(self, host, host_os, test_file):
        ...
        for name in ("reverse.test.domain.local", "test.domain.local"):
            shutil.copyfile(
                test_file(f"report/zones_{name}"), os.path.join(base_dir, name)
            )
    ...
    def test_no_custom_entries_sles(
        self, host, add_host, add_network, fake_os_sles, revert_etc, test_file
    ):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 network=test ip=192.168.0.2"
        )
        assert result.rc == 0

        # Add another with no ip address, which should get skipped
        result = host.run(
            "stack add host interface backend-0-0 interface=eth1 network=test mac=00:00:00:00:00:01"
        )
        assert result.rc == 0

        # Add an alias th generate a CNAME
        result = host.run(
            "stack add host interface alias backend-0-0 alias=foo interface=eth0"
        )
        assert result.rc == 0
    ...
    def test_no_custom_entries_redhat(
        self, host, add_host, add_network, fake_os_redhat, revert_etc, test_file
    ):
        # Set DNS on our test network
        result = host.run("stack set network dns test dns=true")
        assert result.rc == 0
    ...
    def test_with_custom_entries(
        self, host, add_host, add_network, host_os, custom_entries, test_file
    ):
        # Set DNS on our test network
        result = host.run("stack set network dns test dns=true")
        assert result.rc == 0
        ...
        # Does the output match what we expect?
        with open(
            test_file(f"report/zones_with_custom_entries_{host_os}.txt")
        ) as output:
            # The serial will change between runs, so we have to replace them with something known
            zones = re.sub(r"\d+ ; Serial", "0000000000 ; Serial", result.stdout)
