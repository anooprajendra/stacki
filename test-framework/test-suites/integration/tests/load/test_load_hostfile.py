...
class TestLoadHostfile:
    ...
    def test_load_hostfile(self, host, csvfile, revert_etc, test_file):
        ...
        # get filename
        input_tmp_file, output_tmp_file = self.update_csv_variables(
            host, csvfile, test_file
        )

        # Add a network address with 2 hosts
        result = host.run(
            "stack add network autoip address=10.1.1.4 mask=255.255.255.252"
        )
    ...
    def test_load_hostfile_ip_no_network(self, host, test_file):
        ...
        result = host.run(
            f'stack load hostfile file={test_file("load/load_hostfile_ip_no_network.csv")}'
        )
        assert result.rc != 0
    ...
    def test_load_hostfile_duplicate_interface(self, host, test_file):
        ...
        result = host.run(
            f'stack load hostfile file={test_file("load/load_hostfile_duplicate_interface.csv")}'
        )
        assert result.rc != 0
        assert (
            re.search(r'interface ".+" already specified for host', result.stderr)
            is not None
        )

    def test_load_hostfile_with_unicode(self, host, test_file):
        result = host.run(
            f'stack load hostfile file={test_file("load/load_hostfile_with_unicode.csv")}'
        )
        assert result.rc != 0
