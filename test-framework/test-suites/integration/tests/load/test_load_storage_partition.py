...
class TestLoadStoragePartition:
    ...
    def test_missing_size(self, host, test_file):
        ...
        assert result.rc == 255
        assert (
            result.stderr == 'error - empty value found for "size" column at line 2\n'
        )
    ...
    def test_missing_device(self, host, test_file):
        ...
        assert result.rc == 255
        assert (
            result.stderr == 'error - empty value found for "device" column at line 2\n'
        )
    ...
    def test_global_scope_overwrite(self, host, test_file):
        ...
        secondary_partition_config = test_file(
            "load/storage_partition_global_scope2.csv"
        )
        # Load the secondary partition config and then load the actual one.
        result = host.run(
            f"stack load storage partition file={secondary_partition_config}"
        )
        assert result.rc == 0
        result = host.run(
            f"stack load storage partition file={actual_partition_config}"
        )
        assert result.rc == 0
    ...
    def test_appliance_scope_overwrite(self, host, test_file):
        """Test that loading another storage partition in the same scope unloads the previous partitioning information."""
        actual_partition_config = test_file(
            "load/storage_partition_appliance_scope.csv"
        )
        secondary_partition_config = test_file(
            "load/storage_partition_appliance_scope2.csv"
        )
        # Load the secondary partition config and then load the actual one.
        result = host.run(
            f"stack load storage partition file={secondary_partition_config}"
    ...
    def test_raid_missing_level(self, host, test_file):
        ...
        assert result.rc == 255
        assert (
            'missing "--level=RAID" option for software raid device "md0"'
            in result.stderr
        )
    ...
    def test_raid_missing_device(self, host, test_file):
        ...
        assert result.rc == 255
        assert (
            'device "raid.02" not defined for software raid device "md0"'
            in result.stderr
        )
