...
class TestLoad:
    ...
    def test_load_processing_attr(self, host, stack_load, test_file, scope):
        ...
        result = host.run(
            f"stack list {scope if scope != 'global' else ''} attr output-format=json"
        )
        assert result.rc == 0
        ...
        expected_result = json.loads(
            Path(test_file(f"load/json/expected_attr_{scope}.json")).read_text()
        )
        ...
        assert any(
            listed_result == expected_result for listed_result in listed_results
    ...
    def test_load_processing_controller(self, host, stack_load, test_file, scope):
        ...
        # Run the list command to get the results for the global scope.
        result = host.run(
            f"stack list {scope if scope != 'global' else ''} storage controller output-format=json"
        )
        assert result.rc == 0
        listed_results = json.loads(result.stdout)
        ...
        # Load the expected result.
        expected_result = json.loads(
            Path(test_file(f"load/json/expected_controller_{scope}.json")).read_text()
        )
        # Since scope collapsing can cause more objects to be listed for a scope than was set at that scope,
        # we just look for the entry we expect to be in the list an call that a success.
        assert any(
    ...
    def test_load_processing_firewall(self, host, stack_load, test_file, scope):
        ...
        # Run the list command to get the results for the global scope.
        result = host.run(
            f"stack list {scope if scope != 'global' else ''} firewall output-format=json"
        )
        assert result.rc == 0
        listed_results = json.loads(result.stdout)
        ...
        # Load the expected result.
        expected_result = json.loads(
            Path(test_file(f"load/json/expected_firewall_{scope}.json")).read_text()
        )
        # Since scope collapsing can cause more objects to be listed for a scope than was set at that scope,
        # we just look for the entry we expect to be in the list an call that a success.
        assert any(
    ...
    def test_load_processing_partition(self, host, stack_load, test_file, scope):
        ...
        # Run the list command to get the results for the global scope.
        result = host.run(
            f"stack list {scope if scope != 'global' else ''} storage partition output-format=json"
        )
        assert result.rc == 0
        listed_results = json.loads(result.stdout)
        ...
        # Load the expected result.
        expected_result = json.loads(
            Path(test_file(f"load/json/expected_partition_{scope}.json")).read_text()
        )
        # Since scope collapsing can cause more objects to be listed for a scope than was set at that scope,
        # we just look for the entry we expect to be in the list an call that a success.
        assert any(
    ...
    def test_load_processing_route(self, host, stack_load, test_file, scope):
        ...
        # Run the list command to get the results for the global scope.
        result = host.run(
            f"stack list {scope if scope != 'global' else ''} route output-format=json"
        )
        assert result.rc == 0
        listed_results = json.loads(result.stdout)
        ...
        # Load the expected result.
        expected_result = json.loads(
            Path(test_file(f"load/json/expected_route_{scope}.json")).read_text()
        )
        # Since scope collapsing can cause more objects to be listed for a scope than was set at that scope,
        # we just look for the entry we expect to be in the list an call that a success.
        assert any(
    ...
    def test_load_multiple_partitions(self, host, stack_load, test_file, scope):
        ...
        # get the file path to use in load.
        real_partition_file_path = Path(test_file("load/json/partition.json")).resolve(
            strict=True
        )
        secondary_partition_file_path = Path(
            test_file("load/json/partition2.json")
        ).resolve(strict=True)
