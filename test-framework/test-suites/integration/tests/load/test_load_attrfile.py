...
class TestLoadAttrfile:
    ...
    def test_invalid_attr_space(self, host, test_file):
        ...
        assert (
            result.stderr
            == 'error - attribute "test 1" cannot have a space character\n'
        )
    ...
    def test_invalid_attr_multiple_slash(self, host, test_file):
        ...
        assert (
            result.stderr
            == 'error - attribute "test/1/1" cannot have more than one "/"\n'
        )
    ...
    def test_invalid_attr_dash(self, host, test_file):
        ...
        assert (
            result.stderr
            == 'error - attribute "test-1" contains an invalid character.\n"test-1" must be a valid ctoken\n'
        )
    ...
    def test_global_appliance_host_scope(self, host, add_host, test_file, revert_etc):
        ...
        with open(
            test_file("load/attrfile_global_appliance_host_scope.json")
        ) as output:
            assert json.loads(result.stdout) == json.loads(output.read())

    def test_environment_scope(
        self, host, add_host, add_environment, add_box, test_file, revert_etc
    ):
        # Load our attr file
