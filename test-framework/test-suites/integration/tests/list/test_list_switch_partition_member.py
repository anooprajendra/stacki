...
class TestListSwitchPartitionMember:
    ...
    def test_list_by_name(
        ...
        result = host.run(
            f"stack add switch partition member switch-0-0 name=default guid=00:00:00:00:00:00:00:00"
        )
        assert result.rc == 0
        ...
        assert result.rc == 0
        with open(test_file(f"add/add_default_member_output.json")) as output:
        ...
        result = host.run(
            f"stack add switch partition member switch-0-0 name=aaa guid=00:00:00:00:00:00:00:00"
        )
        assert result.rc == 0

        result = host.run(
            f"stack list switch partition member switch-0-0 name=aaa output-format=json"
        )
        assert result.rc == 0
        ...
        result = host.run(
            f"stack list switch partition member switch-0-0 name=bbb output-format=json"
        )
        assert result.rc == 0
        assert result.stdout.strip() == ""
        ...
        result = host.run(
            f"stack list switch partition member switch-0-0 name=badname output-format=json"
        )
        assert result.rc != 0
        assert result.stdout.strip() == ""
        assert result.stderr.strip() != ""
    ...
    def test_list_everything(
        ...
        with open(
            test_file("add/add_multiple_partitions_multiple_members_output.json")
        ) as output:
            expected_output = output.read()

        sort_key = lambda d: d["partition"]
        assert json.loads(result.stdout).sort(key=sort_key) == json.loads(
            expected_output
        ).sort(key=sort_key)

        result = host.run(
            "stack list switch partition member switch-0-0 switch-0-1 output-format=json expanded=true"
        )
        assert result.rc == 0
    ...
    def test_cannot_add_with_enforce_sm(self, host, add_ib_switch):
        ...
        result = host.run(
            f"stack list switch partition member switch-0-0 name=Default enforce_sm=true"
        )
        assert result.rc != 0
        assert result.stderr.strip() != ""
