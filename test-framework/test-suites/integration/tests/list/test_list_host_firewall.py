...
class TestListHostFirewall:
    ...
    def test_frontend_intrinsic_no_dns(self, host, test_file):
        ...
        with open(
            test_file("list/host_firewall_frontend_intrinsic_no_dns.json")
        ) as output:
            assert self._json_compare(
                json.loads(result.stdout), json.loads(output.read())
            )
    ...
    def test_frontend_intrinsic_with_dns(self, host, test_file):
        ...
        with open(
            test_file("list/host_firewall_frontend_intrinsic_with_dns.json")
        ) as output:
            assert self._json_compare(
                json.loads(result.stdout), json.loads(output.read())
            )
    ...
    def test_scope_overriding(
        self, host, add_host_with_interface, add_environment, host_os, test_file
    ):
