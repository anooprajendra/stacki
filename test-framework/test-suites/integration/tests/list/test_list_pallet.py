...
class TestListPallet:
    ...
    def test_with_arch(self, host, create_pallet_isos, revert_export_stack_pallets):
        ...
        result = host.run(
            f"stack add pallet {create_pallet_isos}/test-different-arch-1.0-prod.arm.disk1.iso"
        )
        assert result.rc == 0
    ...
    def test_with_release(self, host, create_pallet_isos, revert_export_stack_pallets):
        ...
        result = host.run(
            f"stack add pallet {create_pallet_isos}/test-different-release-1.0-test.x86_64.disk1.iso"
        )
        assert result.rc == 0
