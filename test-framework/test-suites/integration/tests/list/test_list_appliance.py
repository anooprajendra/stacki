...
class TestListAppliance:
    ...
    def test_no_args(self, host):
        ...
        assert {"appliance": "backend", "public": "yes"} in output

        assert {"appliance": "barnacle", "public": "no"} in output

        assert {"appliance": "frontend", "public": "no"} in output

        assert {"appliance": "switch", "public": "no"} in output

    def test_one_arg(self, host):
        result = host.run("stack list appliance backend output-format=json")
        assert result.rc == 0
        assert json.loads(result.stdout) == [{"appliance": "backend", "public": "yes"}]

    def test_multiple_args(self, host):
        result = host.run("stack list appliance frontend backend output-format=json")
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {"appliance": "frontend", "public": "no"},
            {"appliance": "backend", "public": "yes"},
        ]
