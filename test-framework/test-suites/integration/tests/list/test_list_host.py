...
class TestListHost:
    def test_lookup_by_IP(self, host, add_host_with_interface, host_os):
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=192.168.1.1"
        )
        assert result.rc == 0
    ...
    def test_lookup_by_FQDN_with_name(
        self, host, add_host_with_interface, add_network, host_os
    ):
