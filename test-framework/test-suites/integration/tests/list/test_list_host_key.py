...
class TestListHostKey:
    ...
    def test_no_args(self, host, add_host):
        ...
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "id": 1, "public key": "frontend_key"},
            {"host": "backend-0-0", "id": 2, "public key": "backend_key"},
        ]
    ...
    def test_one_arg(self, host, add_host):
        ...
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "id": 1, "public key": "frontend_key_1"},
            {"host": "frontend-0-0", "id": 2, "public key": "frontend_key_2"},
        ]
    ...
    def test_multiple_args(self, host, add_host):
        ...
        result = host.run(
            "stack list host key frontend-0-0 backend-0-0 output-format=json"
        )
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "id": 1, "public key": "frontend_key_1"},
            {"host": "frontend-0-0", "id": 2, "public key": "frontend_key_2"},
            {"host": "backend-0-0", "id": 3, "public key": "backend_key_1"},
            {"host": "backend-0-0", "id": 4, "public key": "backend_key_2"},
        ]

    def test_with_multiple_lines(self, host):
        # Add a key for the frontend with a newline
        result = host.run(
            'stack add host key frontend-0-0 key="frontend_key_1\nfrontend_key_2"'
        )
        assert result.rc == 0
        ...
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "id": 1, "public key": "frontend_key_1"},
            {"host": "frontend-0-0", "id": 1, "public key": "frontend_key_2"},
        ]
