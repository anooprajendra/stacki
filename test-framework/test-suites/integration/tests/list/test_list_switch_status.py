...
class TestListSwitchStatus:
    def test_x1052(self, host, add_host, inject_code, test_file):
        ...
        result = host.run(
            "stack add host interface backend-0-0 mac=00:00:00:00:00:00 interface=eth0 network=private"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack add host interface backend-0-1 interface=eth1 network=private"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack set host attr switch-0-0 attr=component.model value=x1052"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack add switch host switch-0-0 host=backend-0-1 interface=eth1 port=2"
        )
        assert result.rc == 0
    ...
    def test_x1052_not_same_network(self, host, add_network):
        ...
        result = host.run(
            "stack add host interface switch-0-0 interface=eth0 network=test ip=127.0.0.1"
        )
        assert result.rc == 0
        ...
        assert (
            result.stderr
            == 'error - "switch-0-0" and the frontend do not share a network\n'
        )
