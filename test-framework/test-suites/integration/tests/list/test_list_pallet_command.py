...
class TestListPalletCommand:
    ...
    def test_multiple_args(self, host, create_pallet_isos, revert_export_stack_pallets):
        ...
        result = host.run(
            f"stack add pallet {create_pallet_isos}/test_1-sles-1.0-prod.x86_64.disk1.iso"
        )
        assert result.rc == 0

        result = host.run(
            f"stack add pallet {create_pallet_isos}/test_1-redhat-1.0-prod.x86_64.disk1.iso"
        )
        assert result.rc == 0

        # List out the commands for the stacki pallet and our two test ones
        result = host.run(
            "stack list pallet command stacki test_1-sles test_1-redhat output-format=json"
        )
        assert result.rc == 0
    ...
    def test_missing_rollname(self, host):
        ...
        site_pkgs = [
            p for p in site.getsitepackages() if p.startswith("/opt/stack/lib/python")
        ][0]
        result = host.run(
