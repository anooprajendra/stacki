...
class TestListHostAttr:
    ...
    def test_with_backend(self, host, add_host):
        ...
        result = host.run(
            "stack list host attr a:frontend a:backend output-format=json"
        )
        assert result.rc == 0
        json.loads(result.stdout) == attr_obj
        ...
        result = host.run(
            "stack list host attr localhost backend-0-0 output-format=json"
        )
        assert result.rc == 0
        json.loads(result.stdout) == attr_obj
    ...
    def test_distinct_with_multiple_hosts(self, host, add_host):
        ...
        assert set(common_host_attrs["_common_"]).isdisjoint(
            new_host_attrs["backend-0-0"]
        )

    def test_common_with_multiple_hosts_single_attr_param(self, host, add_host):
        result = host.run(
            "stack list host attr display=distinct attr=hostname output-format=json"
        )
        assert result.rc == 0
        attr_obj = json.loads(result.stdout)
        ...
        result = host.run(
            "stack list host attr display=common attr=rank output-format=json"
        )
        assert result.rc == 0
        attr_obj = json.loads(result.stdout)
    ...
    def test_scope_resolving(self, host, add_host, add_environment, host_os, test_file):
        ...
        result = host.run("stack add attr attr=test.global value=test_1")
        assert result.rc == 0
        ...
        result = host.run(f"stack add os attr {host_os} attr=test.os value=test_3")
        assert result.rc == 0
        ...
        result = host.run("stack add host attr backend-0-0 attr=test.host value=test_5")
        assert result.rc == 0

        # Add a bunch of attrs that will be overridden to just one output
        result = host.run("stack add attr attr=test.override value=test_6")
        assert result.rc == 0
    ...
    def test_normal_user_no_shadow(self, host, add_host):
        # Add a shadow attr
        result = host.run(
            "stack set host attr backend-0-0 attr=test value=True shadow=True"
        )
        assert result.rc == 0

        # Make sure it got there
        result = host.run(
            "stack list host attr backend-0-0 attr=test shadow=True output-format=json"
        )
        assert result.rc == 0
    ...
    def test_const_overwrite(self, host, add_host):
        result = host.run(
            "stack set host attr backend-0-0 attr=const_overwrite value=False"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack list host attr backend-0-0 attr=rack output-format=json"
        )
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {
