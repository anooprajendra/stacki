...
class TestDumpApi:
    ...
    def test_dump_api(self, host):
        ...
        results = host.run(
            'stack add api blacklist command command="remove host interface"'
        )
        results = host.run(
            'stack add api blacklist command command="add host interface"'
        )
        ...
        assert (
            "remove host interface" and "add host interface" in dump["api"]["blacklist"]
        )
