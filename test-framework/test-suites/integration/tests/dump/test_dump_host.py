...
class TestDumpHost:
    ...
    def test_dump_host(self, host):
        ...
        assert results.rc == 0
        results = host.run(
            "stack add host attr backend-test attr=test value=test shadow=True"
        )
        assert results.rc == 0
        results = host.run(
        ...
        assert results.rc == 0
        results = host.run(
            "stack add host route backend-test address=192.168.0.2 gateway=192.168.0.3 netmask=255.255.255.0 syncnow=true interface=eth1"
        )
        assert results.rc == 0
        results = host.run(
        ...
        assert results.rc == 0
        results = host.run(
            'stack add host storage partition backend-test device=test options="test option" size=1 mountpoint=test partid=1 type=ext4'
        )
        assert results.rc == 0
        results = host.run(
