...
class TestDumpAppliance:
    ...
    def test_dump_appliance(self, host):
        ...
        results = host.run(
            "stack add appliance attr backend attr=test value=test shadow=False"
        )
        assert results.rc == 0
        results = host.run(
            "stack add appliance route backend address=192.168.0.0 gateway=192.168.0.1 netmask=255.255.255.0 interface=eth0"
        )
        assert results.rc == 0
        results = host.run(
            'stack add appliance firewall backend action=accept chain=input protocol=udp service=www network=private output-network=private rulename=appliancetest table=filter comment="test" flags="-m set"'
        )
        assert results.rc == 0
        results = host.run(
            'stack add appliance storage partition backend device=test options="test option" size=1 mountpoint=test partid=1 type=ext4'
        )
        assert results.rc == 0
        results = host.run(
            "stack add appliance storage controller backend adapter=1 arrayid=2 enclosure=3 raidlevel=4 slot=5 options=test"
        )
        assert results.rc == 0
