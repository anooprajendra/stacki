...
class TestDumpGlobal:
    ...
    def test_dump_global(self, host):
        ...
        assert results.rc == 0
        results = host.run(
            "stack add route address=192.168.0.0 gateway=192.168.0.1 interface=eth0 netmask=255.255.255.0"
        )
        assert results.rc == 0
        results = host.run(
            'stack add firewall action=accept chain=input protocol=udp service=www network=private output-network=private rulename=test table=filter comment="test" flags="-m set"'
        )
        assert results.rc == 0
        results = host.run(
            'stack add storage partition device=test options="test option" size=1 mountpoint=test partid=1 type=ext4'
        )
        assert results.rc == 0
        results = host.run(
            'stack add storage controller adapter=1 arrayid=2 enclosure=3 raidlevel=4 slot=5 options="test"'
        )
        assert results.rc == 0
