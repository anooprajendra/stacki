...
class TestDumpOs:
    ...
    def test_dump_os(self, host):
        ...
        assert results.rc == 0
        results = host.run(
            "stack add os route redhat address=192.168.0.0 gateway=192.168.0.1 netmask=255.255.255.0 interface=eth1"
        )
        assert results.rc == 0
        results = host.run(
            'stack add os firewall redhat action=accept chain=input protocol=udp service=www network=private output-network=private rulename=ostest table=filter comment="test" flags="-m set"'
        )
        assert results.rc == 0
        results = host.run(
            'stack add os storage partition redhat device=test options="test option" size=1 mountpoint=test partid=1 type=ext4'
        )
        assert results.rc == 0
        results = host.run(
            "stack add os storage controller redhat adapter=1 arrayid=2 enclosure=3 raidlevel=4 slot=5 options=test"
        )
        assert results.rc == 0
