...
class TestAnsibleDynamicInventory:
    ...
    def test_attributes(self, host, add_host):
        ...
        assert (
            inventory["_meta"]["hostvars"]["frontend-0-0"]["stacki_appliance"]
            == "frontend"
        )
        assert (
            inventory["_meta"]["hostvars"]["backend-0-0"]["stacki_appliance"]
            == "backend"
        )
        ...
        assert (
            inventory["_meta"]["hostvars"]["frontend-0-0"]["stacki_foo"] == "frontend"
        )
