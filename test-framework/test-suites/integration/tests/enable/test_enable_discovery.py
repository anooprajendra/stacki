...
import pytest
import zmq
from stack.mq import Message, ports
        ...
        for _ in range(timeout):
            try:
                ...
                while len(messages) < count:
                    channel, data = self._socket.recv_multipart(flags=zmq.NOBLOCK)

                    message = Message(message=data.decode(), channel=channel.decode())
...
class TestEnableDiscovery:
    def test_daemon_not_running(self, host):
        ...
        process_list = [
            process
            for process in host.process.filter(comm="stack")
    ...
    def test_daemon_detecting_two_nodes(self, host, revert_etc):
        ...
        lines = [line[20:] for line in log_file.content_string.strip().split("\n")]
