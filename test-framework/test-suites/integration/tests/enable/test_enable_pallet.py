...
class TestEnablePallet:
    ...
    def test_incompatable_os(
        self, host, create_pallet_isos, revert_export_stack_pallets
    ):
        # Add the pallet that is ubuntu
        result = host.run(
            f"stack add pallet {create_pallet_isos}/test-different-os-1.0-prod.x86_64.disk1.iso"
        )
        assert result.rc == 0
        ...
        assert (
            result.stderr
            == 'error - incompatible pallet "test-different-os" with OS "ubuntu"\n'
        )

    def test_wrong_version(self, host, create_pallet_isos, revert_export_stack_pallets):
        # Add our test pallet
        result = host.run(
            f"stack add pallet {create_pallet_isos}/test-different-version-test_foo-prod.x86_64.disk1.iso"
        )
        assert result.rc == 0
    ...
    def test_default_box(
        self, host, host_os, create_pallet_isos, revert_etc, revert_export_stack_pallets
    ):
        # Add our test pallet
        result = host.run(
