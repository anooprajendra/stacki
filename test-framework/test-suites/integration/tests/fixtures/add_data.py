...
def add_host_with_net(host, add_host_with_interface, add_network):
    ...
    def _inner(hostname, rack, rank, appliance, interface, ip, network, address, pxe):
        ...
        result = host.run(
            f"stack set host interface network {hostname} network={network} interface={interface}"
        )
        assert result.rc == 0

        # Set the interface IP
        result = host.run(
            f"stack set host interface ip {hostname} ip={ip} network={network}"
        )
        assert result.rc == 0
        ...
        result = host.run(
            f"stack add host interface a:frontend interface={new_interface} network={network} ip={ipaddress.ip_address(ip) + 1}"
        )
        assert result.rc == 0
    ...
    result = host.run(
        f"stack set host interface network backend-0-0 network=test interface=eth0 ip=192.168.0.3"
    )
    assert result.rc == 0

    # Add a frontend interface on the network.
    result = host.run(
        f"stack add host interface a:frontend interface=eth2 network=test ip=192.168.0.2"
    )
    assert result.rc == 0
...
@pytest.fixture(
    ...
    ids=(
        "stack_load_exec",
        "stack_load_bash",
        "stack_load_document_exec",
        "stack_load_document_bash",
    ),
)
def stack_load(request, host):
    ...
    def _load(dump_file, **kwargs):
        if "exec" in kwargs:
            raise ValueError(
                "Cannot pass exec param to this fixture. It handles it for you."
            )

        if "document" in kwargs:
            raise ValueError(
                "Cannot pass document param to this fixture. It handles it for you."
            )

        kwargs_string = " ".join(f"{key}={value}" for key, value in kwargs.items())
        return host.run(
            f"stack load {param_string}{dump_file} {exec_string} {kwargs_string}"
        )
