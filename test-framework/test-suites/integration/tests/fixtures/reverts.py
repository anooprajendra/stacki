...
def _add_overlay(target):
    ...
    subprocess.run(
        [
            ...
            f"overlay_{name}",
            "-o",
            ",".join(f"{k}={v}" for k, v in overlay_dirs.items()),
            target,
        ],
        check=True,
    )
        ...
        with open(f"/export/reports/revert_{name}.log", "a") as f:
            ...
            if mods:
                ...
                if request.fixturename not in parameters and (
                    usefixtures_marker is None
                    or request.fixturename not in usefixtures_marker.args
                ):
                    request.node.warn(
...
def revert_discovery():
    ...
    for _ in range(4):
        result = subprocess.run(
            ["stack", "disable", "discovery"], stdout=subprocess.PIPE, encoding="utf-8"
        )
...
def revert_routing_table():
    ...
    result = subprocess.run(
        ["ip", "route", "list"], stdout=subprocess.PIPE, encoding="utf-8", check=True
    )
