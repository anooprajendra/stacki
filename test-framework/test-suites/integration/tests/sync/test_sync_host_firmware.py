...
def test_sync_only_needed_files(
    ...
    result = host.run(
        f"stack add firmware 1.2.3 make=mellanox model=m7800 source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    result = host.run(
        f"stack add firmware 2.3.4 make=mellanox model=m7800 source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    # Add multiple firmwares for dell, and associate it with the host we are going to sync.
    result = host.run(
        f"stack add firmware 1.2.3.4 make=dell model=x1052-software source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    result = host.run(
        f"stack add firmware 2.3.4.5 make=dell model=x1052-software source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    result = host.run(
        f"stack add firmware 1.2.3.4 make=dell model=x1052-boot source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    result = host.run(
        f"stack add firmware 2.3.4.5 make=dell model=x1052-boot source={fake_local_firmware_file} hosts=backend-0-0"
    )
    assert result.rc == 0
    # Add a piece of firmware we don't care about. Don't associate it with any hosts.
    result = host.run(
        f"stack add firmware 1.2.3 make=mellanox model=m6036 source={fake_local_firmware_file}"
    )
    assert result.rc == 0
    ...
    firmware_files_on_disk = [
        firmware_file
        for firmware_file in stack.firmware.BASE_PATH.glob(f"**/*")
