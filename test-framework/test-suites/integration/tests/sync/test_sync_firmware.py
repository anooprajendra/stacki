...
def test_sync_firmware_resync(host, fake_local_firmware_file, revert_firmware):
    ...
    result = host.run(
        f"stack add firmware 1.2.3 make=mellanox model=m7800 source={fake_local_firmware_file}"
    )
    assert result.rc == 0
...
def test_sync_firmware_selective_resync(
    host, fake_local_firmware_file, revert_firmware
):
    ...
    assert result.rc == 0
    result = host.run(
        f"stack add firmware 2.3.4 make=mellanox model=m7800 source={fake_local_firmware_file}"
    )
    assert result.rc == 0
