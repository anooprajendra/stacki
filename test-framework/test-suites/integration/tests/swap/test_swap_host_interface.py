...
class TestSwapHostInterface:
    ...
    def test_three_interfaces(self, host):
        result = host.run(
            "stack swap host interface frontend-0-0 interfaces=eth0,eth1,eth2"
        )
        assert result.rc == 255
        assert result.stderr == "error - must supply exactly two interfaces\n"
    ...
    def test_minimal_parameters(self, host, add_host, revert_etc):
        ...
        result = host.run(
            "stack add host interface backend-0-0 mac=00:00:00:00:00:00 interface=eth0"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface backend-0-0 mac=11:11:11:11:11:11 interface=eth1"
        )
        assert result.rc == 0

        # Swap them
        result = host.run(
            "stack swap host interface backend-0-0 interfaces=eth0,eth1 sync-config=no"
        )
        assert result.rc == 0
    ...
    def test_all_parameters(self, host, add_host, add_network, revert_etc):
        ...
        # Swap them
        result = host.run(
            "stack swap host interface backend-0-0 interfaces=eth1,eth2 sync-config=no"
        )
        assert result.rc == 0
