...
class TestRemoveHostPartition:
    ...
    def test_no_parameters(self, host, add_host):
        ...
        result = host.run(
            "stack list host partition frontend-0-0 backend-0-0 output-format=json"
        )
        assert result.rc == 0
    ...
    def test_with_parition(self, host, add_host):
        # Add the same partition info to both hosts
        result = host.run(
            "stack add host partition frontend-0-0 device=sda1 mountpoint=/"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host partition backend-0-0 device=sda1 mountpoint=/"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host partition backend-0-0 device=sda2 mountpoint=/var"
        )
        assert result.rc == 0
    ...
    def test_with_all_parameters(self, host, add_host):
        # Add the same partition info to both hosts
        result = host.run(
            "stack add host partition frontend-0-0 device=sda1 mountpoint=/ uuid=test"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host partition backend-0-0 device=sda1 mountpoint=/ uuid=test"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host partition backend-0-0 device=sda2 mountpoint=/var uuid=foo"
        )
        assert result.rc == 0
        ...
        # Now remove just the backend's info for the sda1 device
        result = host.run(
            "stack remove host partition backend-0-0 device=sda1 partition=/ uuid=test"
        )
        assert result.rc == 0
    ...
    def test_multiple_hosts(self, host, add_host):
        ...
        result = host.run(
            "stack remove host partition frontend-0-0 backend-0-0 device=sda1 partition=/ uuid=test"
        )
        assert result.rc == 0
