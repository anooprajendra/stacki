...
class TestRemoveHostInterface:
    ...
    def test_invalid_mac(self, host, add_host):
        result = host.run(
            "stack remove host interface backend-0-0 mac=00:11:22:33:44:55"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == 'error - no mac address "00:11:22:33:44:55" exists on backend-0-0\n'
        )
    ...
    def test_multiple_args(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack remove host interface backend-0-0 backend-0-1 interface=eth0"
        )
        assert result.rc == 0

        # Confirm it is removed
        result = host.run(
            "stack list host interface backend-0-0 backend-0-1 output-format=json"
        )
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {
