...
class TestRemoveStorageController:
    ...
    def test_no_matches_on_adapter(self, host):
        result = host.run(
            "stack remove storage controller adapter=1 enclosure=* slot=*"
        )
        assert result.rc == 255
        assert (
            result.stderr == 'error - disk specification for "1/*/*" doesn\'t exist\n'
        )

    def test_no_matches_on_enclosure(self, host):
        result = host.run(
            "stack remove storage controller adapter=* enclosure=1 slot=*"
        )
        assert result.rc == 255
        assert (
            result.stderr == 'error - disk specification for "*/1/*" doesn\'t exist\n'
    ...
    def test_remove_single_slot(self, host):
        ...
        # Now remove it
        result = host.run(
            "stack remove storage controller enclosure=1 adapter=2 slot=3"
        )
        assert result.rc == 0
    ...
    def test_remove_everything(self, host):
        ...
        result = host.run(
            "stack remove storage controller enclosure=* adapter=* slot=*"
        )
        assert result.rc == 0
