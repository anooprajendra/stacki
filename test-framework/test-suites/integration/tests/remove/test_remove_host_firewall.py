...
class TestRemoveHostFirewall:
    ...
    def test_invalid(self, host, invalid_host):
        result = host.run(f"stack remove host firewall {invalid_host} rulename=test")
        assert result.rc == 255
    ...
    def test_one_arg(self, host, add_host):
        ...
        rules = [rule for rule in json.loads(result.stdout) if rule["name"] == "test"]
        assert rules == [
            {
                "host": "backend-0-0",
                "name": "test",
    ...
    def test_multiple_args(self, host, add_host):
        ...
        result = host.run(
            "stack list host firewall backend-0-0 backend-0-1 output-format=json"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack remove host firewall backend-0-0 backend-0-1 rulename=test"
        )
        assert result.rc == 0
