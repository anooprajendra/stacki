...
class TestRemoveApplianceRoute:
    ...
    def test_one_arg(self, host, add_appliance):
        ...
        result = host.run(
            "stack add appliance route test address=192.168.0.2 gateway=private"
        )
        assert result.rc == 0

        result = host.run(
            "stack add appliance route test address=192.168.0.3 gateway=private"
        )
        assert result.rc == 0
    ...
    def test_multiple_args(self, host, add_appliance):
        ...
        result = host.run(
            "stack add appliance route foo address=192.168.0.4 gateway=private"
        )
        assert result.rc == 0

        result = host.run(
            "stack add appliance route foo address=192.168.0.5 gateway=private"
        )
        assert result.rc == 0
