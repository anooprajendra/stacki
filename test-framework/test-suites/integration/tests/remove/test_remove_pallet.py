...
class TestRemovePallet:
    ...
    def test_no_parameters(self, host, create_pallet_isos, revert_etc):
        ...
        assert host.file(
            "/export/stack/pallets/minimal/1.0/sles12/sles/x86_64/roll-minimal.xml"
        ).exists
    ...
    def test_all_parameters(self, host, create_pallet_isos, revert_etc):
        ...
        # Now remove our test pallet
        result = host.run(
            "stack remove pallet minimal version=1.0 release=sles12 arch=x86_64 os=sles"
        )
        assert result.rc == 0
    ...
    def test_multiple_versions(
        self, host, create_pallet_isos, revert_etc, revert_export_stack_pallets
    ):
        # Add our minimal pallet
        result = host.run(
            f"stack add pallet {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso"
    ...
    def test_remove_partially_removed_pallet(
        self, host, create_pallet_isos, revert_export_stack_pallets, revert_etc
    ):
        # Add our minimal pallet
        result = host.run(
            f"stack add pallet {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso"
