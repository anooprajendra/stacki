...
class TestRemoveSwitchPartitionMember:
    ...
    SWITCH_PARTITION_MEMBER_TEST_DATA = [
        [
            ...
            "",
            "",
            "add_default_member_output.json",
        ],
        ...
        [
            ...
            "ib0",
            "full",
            "add_default_member_full_output.json",
        ],
        ...
        [
            ...
            "ib0",
            "limited",
            "add_nondefault_member_output.json",
        ],
        [
            "0x0aaa",
            "00:00:00:00:00:00:00:00",
            ...
            "",
            "full",
            "add_nondefault_member_full_output.json",
        ],
    ]
    ...
    def test_behavior(
        ...
        result = host.run(
            f"stack add host interface backend-0-0 interface=ib0 mac=00:00:00:00:00:00:00:00"
        )
        assert result.rc == 0
        ...
        cmd = [
            f"stack set switch partition membership switch-0-0 name={partition_name}"
        ]
        params = []
        if guid:
        ...
        result = host.run(
            "stack list switch partition member switch-0-0 output-format=json"
        )
        assert result.rc == 0
    ...
    def test_negative_behavior(
        ...
        result = host.run(
            f"stack add host interface backend-0-0 interface=ib0 mac={guid}"
        )
        assert result.rc == 0

        # should be able to add
        result = host.run(
            f"stack add switch partition member switch-0-0 name=Default guid={guid}"
        )
        assert result.rc == 0

        # should error on invalid name
        result = host.run(
            f"stack remove switch partition member switch-0-0 name=fake guid={guid}"
        )
        assert result.rc != 0
        ...
        result = host.run(
            f"stack remove switch partition member switch-0-0 name=aaa guid={guid}"
        )
        assert result.rc != 0
        assert result.stderr.strip() != ""
        ...
        result = host.run(
            f"stack remove switch partition member switch-0-0 name=default guid=5"
        )
        assert result.rc == 0
        assert result.stderr.strip() == ""
        assert result.stdout.strip() == ""
    ...
    def test_bad_input(
        ...
        result = host.run(
            f"stack add host interface backend-0-0 interface=ib0 mac={host_guid}"
        )
        assert result.rc == 0
        result = host.run(
            f"stack add switch partition member switch-0-0 name=default guid={host_guid}"
        )
        assert result.rc == 0
    ...
    def test_can_remove_twice(
        ...
        result = host.run(
            f"stack add switch partition member switch-0-0 name={partition_name} guid={guid}"
        )
        assert result.rc == 0
        result = host.run(
            "stack list switch partition member switch-0-0 output-format=json"
        ...
        for i in range(2):
            result = host.run(
                f"stack remove switch partition switch-0-0 name={partition_name} guid={guid}"
            )
            assert result.rc == 0
            assert result.stdout.strip() == ""
            result = host.run(
    ...
    def test_can_remove_names_that_resolve_same(self, host, add_ib_switch, test_file):
        ...
        for partition_name in same_parts[1:]:
            ...
            result = host.run(
                "stack list switch partition switch-0-0 output-format=json"
            )
            assert result.rc == 0
            assert json.loads(result.stdout) == json.loads(expected_output)

            result = host.run(
                f"stack remove switch partition switch-0-0 name={partition_name}"
            )
            assert result.rc == 0
    ...
    def test_cannot_remove_from_non_ib(self, host, add_switch):
        result = host.run(
            f"stack remove switch partition member switch-0-0 name=Default"
        )
        assert result.rc != 0
        assert result.stderr.strip() != ""
    ...
    def test_cannot_remove_with_enforce_sm(self, host, add_ib_switch):
        ...
        result = host.run(
            f"stack remove switch partition member switch-0-0 name=Default enforce_sm=true"
        )
        assert result.rc != 0
        assert result.stderr.strip() != ""
    ...
    def test_two_switches_same_partition_name(
        self,
        host,
        add_ib_switch,
        partition_name,
        ...
        # add second switch
        add_ib_switch(
            "switch-0-1", "0", "1", "switch", "Mellanox", "m7800", "infiniband"
        )

        result = host.run(
            f'stack add switch partition switch-0-0 name={partition_name} options="{options}"'
        )
        assert result.rc == 0
        result = host.run(
            f'stack add switch partition switch-0-1 name={partition_name} options="{options}"'
        )
        assert result.rc == 0
        ...
        assert result.rc == 0
        assert json.loads(
            result.stdout.strip().replace("switch-0-1", "switch-0-0")
        ) == json.loads(expected_output)

        result = host.run(
            "stack remove switch partition switch-0-0 switch-0-1 output-format=json"
        )
        assert result.rc == 0
    ...
    def test_remove_everything(
        self, host, add_host_with_interface, add_ib_switch, add_ib_switch_partition
    ):
        add_host_with_interface("backend-0-1", "0", "1", "backend", "eth0")
        # add hosts with ib interfaces
        for i in range(2):
            result = host.run(
                f"stack add host interface backend-0-{i} interface=ib0 mac=00:00:00:00:00:00:00:0{i}"
            )
            assert result.rc == 0
        ...
        result = host.run(
            "stack list switch partition member switch-0-0 switch-0-1 output-format=json"
        )
        assert result.rc == 0
