...
class TestRemoveHostGroup:
    ...
    def test_host_not_in_group(self, host, add_group):
    ...
    def test_single_arg(self, host, add_group):
        ...
        assert result.rc == 0
        assert json.loads(result.stdout) == [{"host": "frontend-0-0", "groups": "test"}]
        ...
        result = host.run("stack list host group frontend-0-0 output-format=json")
        assert result.rc == 0
        assert json.loads(result.stdout) == [{"host": "frontend-0-0", "groups": ""}]

    def test_multiple_args(self, host, add_host, add_group):
        ...
        # Confirm we are in the group now
        result = host.run(
            "stack list host group frontend-0-0 backend-0-0 output-format=json"
        )
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "groups": "test"},
            {"host": "backend-0-0", "groups": "test"},
        ]
        ...
        assert result.rc == 0
        assert json.loads(result.stdout) == [
            {"host": "frontend-0-0", "groups": ""},
            {"host": "backend-0-0", "groups": ""},
        ]
