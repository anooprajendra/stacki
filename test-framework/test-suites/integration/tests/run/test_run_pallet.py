...
class TestRunPallet:
    ...
    def test_one_arg(self, host, revert_export_stack_carts):
        ...
        result = host.run(
            'script -qfec "stack run pallet stacki" - | tr -d "\r" | head -n 1'
        )
        assert result.rc == 0
        ...
        result = host.run(
            'script -qfec "stack run pallet stacki" - | tr -d "\r" | bash -n'
        )
        assert result.rc == 0
    ...
    def test_multiple_args(
        ...
        # Make sure the top of the output matches what we expect
        result = host.run(
            f'script -qfec "stack run pallet stacki test_1-{host_os}" - | tr -d "\r" | head -n 1'
        )
        assert result.rc == 0
        assert result.stdout == "#! /bin/bash\n"
        ...
        # Run it through "bash -n" to do a syntax sanity check
        result = host.run(
            f'script -qfec "stack run pallet stacki test_1-{host_os}" - | tr -d "\r" | bash -n'
        )
        assert result.rc == 0
        assert result.stdout == ""

    def test_database_false(
        self,
        host,
        host_os,
        create_pallet_isos,
        ...
        # Make sure the top of the output matches what we expect
        result = host.run(
            f'script -qfec "stack run pallet stacki test_1-{host_os} database=false" - | tr -d "\r" | head -n 1'
        )
        assert result.rc == 0
        assert result.stdout == "#! /bin/bash\n"
        ...
        # Run it through "bash -n" to do a syntax sanity check
        result = host.run(
            f'script -qfec "stack run pallet stacki test_1-{host_os} database=false" - | tr -d "\r" | bash -n'
        )
        assert result.rc == 0
        assert result.stdout == ""
