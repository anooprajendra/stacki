...
class TestAddHostBridge:
    ...
    def test_invalid_interface(self, host):
        ...
        assert (
            result.stderr
            == "error - Could not find interface test configured on host backend-0-0\n"
        )
    ...
    def test_invalid_network(self, host):
        ...
        assert result.rc == 255
        assert (
            result.stderr == "error - Could not find network test on host backend-0-0\n"
        )

    def test_only_interface(self, host):
        # Set the network for the eth0 interface on backend-0-0
        result = host.run(
            "stack set host interface network backend-0-0 interface=eth0 network=private"
        )
        assert result.rc == 0
    ...
    def test_interface_and_network(self, host):
        ...
        # Add the bridge interface
        result = host.run(
            "stack add host bridge backend-0-0 name=br0 interface=eth0 network=private"
        )
        assert result.rc == 0
    ...
    def test_default_with_ip(self, host):
        ...
        result = host.run(
            "stack set host interface ip backend-0-0 interface=eth0 ip=192.168.0.1"
        )
        assert result.rc == 0

        # Make the eth0 interface on backend-0-0 the default
        result = host.run(
            "stack set host interface default backend-0-0 interface=eth0 default=true"
        )
        assert result.rc == 0
