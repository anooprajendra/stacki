...
class TestAddFirewall:
    ...
    def test_blank_service(self, host):
        result = host.run(
            'stack add firewall service="" chain=INPUT action=ACCEPT protocol=TCP'
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_service_range(self, host):
        result = host.run(
            "stack add firewall service=1:2:3 chain=INPUT action=ACCEPT protocol=TCP"
        )
        assert result.rc == 255
    ...
    def test_invalid_service_port(self, host):
        result = host.run(
            "stack add firewall service=0foo chain=INPUT action=ACCEPT protocol=TCP"
        )
        assert result.rc == 255
    ...
    def test_blank_chain(self, host):
        result = host.run(
            'stack add firewall service=1234 chain="" action=ACCEPT protocol=TCP'
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_blank_action(self, host):
        result = host.run(
            'stack add firewall service=1234 chain=INPUT action="" protocol=TCP'
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_blank_protocol(self, host):
        result = host.run(
            'stack add firewall service=1234 chain=INPUT action=ACCEPT protocol=""'
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_no_rulename(self, host):
        # Add the rule
        result = host.run(
            "stack add firewall service=1234 chain=INPUT action=ACCEPT protocol=TCP"
        )
        assert result.rc == 0
    ...
    def test_empty_rulename(self, host):
        # Add the rule
        result = host.run(
            'stack add firewall service=1234 chain=INPUT action=ACCEPT protocol=TCP rulename=""'
        )
        assert result.rc == 0
