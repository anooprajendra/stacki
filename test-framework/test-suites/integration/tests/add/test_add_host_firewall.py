...
class TestAddHostFirewall:
    ...
    def test_no_chain(self, host):
        result = host.run(
            "stack add host firewall frontend-0-0 service=1234 action=ACCEPT protocol=TCP"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_no_action(self, host):
        result = host.run(
            "stack add host firewall frontend-0-0 service=1234 chain=INPUT protocol=TCP"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_no_protocol(self, host):
        result = host.run(
            "stack add host firewall frontend-0-0 service=1234 chain=INPUT action=ACCEPT"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
