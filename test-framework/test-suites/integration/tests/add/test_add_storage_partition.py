...
class TestAddStoragePartition:
    ...
    def test_invalid_size(self, host):
        result = host.run(
            "stack add storage partition device=sda mountpoint=/ size=recommended"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_existing_partition(self, host):
        # Add it once
        result = host.run(
            "stack add storage partition device=sda mountpoint=/ size=1024"
        )
        assert result.rc == 0
        ...
        assert (
            result.stderr
            == 'error - partition specification for device "sda" and mount point "/" already exists\n'
        )
