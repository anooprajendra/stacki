...
class TestAddSwitchPartitionMember:
    ...
    def test_passed_no_args(self, host, add_ib_switch):
        result = host.run(f"stack add switch partition member")
        assert result.rc != 0
    ...
    def test_cannot_duplicate_names_that_resolve_same(
        ...
        result = host.run(
            f"stack add switch partition member switch-0-0 name={same_parts[0]} guid=00:00:00:00:00:00:00:00"
        )
        assert result.rc == 0
        for partition_name in same_parts[1:]:
            result = host.run(
                f"stack add switch partition member switch-0-0 name={partition_name} guid=00:00:00:00:00:00:00:00"
            )
            assert result.rc != 0
            result = host.run(
                "stack list switch partition member switch-0-0 output-format=json"
    ...
    def test_cannot_add_to_non_ib(self, host, add_switch):
        result = host.run(
            f"stack add switch partition member switch-0-0 name=default guid=fake"
        )
        assert result.rc != 0
    ...
    def test_cannot_add_with_enforce_sm(
        ...
        result = host.run(
            f"stack add switch partition member switch-0-0 name=Default guid=fake enforce_sm=true"
        )
        assert result.rc != 0
