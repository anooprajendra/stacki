...
class TestAddPallet:
    def test_no_pallet(self, host):
        ...
        assert (
            result.stderr
            == "error - no pallets specified and /mnt/cdrom is unmounted\n"
        )
    ...
    def test_invalid(self, host):
        ...
        assert (
            result.stderr
            == "error - The following arguments appear to be local paths that do not exist: /export/test.iso\n"
        )

    def test_username_no_password(self, host, create_pallet_isos):
        result = host.run(
            f"stack add pallet {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso username=test"
        )
        assert result.rc == 255
    ...
    def test_password_no_username(self, host, create_pallet_isos):
        result = host.run(
            f"stack add pallet {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso password=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_multiple_isos(
        self, host, host_os, create_pallet_isos, revert_export_stack_pallets
    ):
        # Add our minimal pallet
        ...
        # Check it made it in as expected
        result = host.run(
            "stack list pallet minimal test-different-arch output-format=json"
        )
        assert result.rc == 0
    ...
    def test_no_mountpoint(
        self, host, rmtree, create_pallet_isos, revert_export_stack_pallets
    ):
    ...
    def test_mountpoint_in_use(
        self, host, create_pallet_isos, revert_export_stack_pallets
    ):
        ...
        with ExitStack() as cleanup:
            result = host.run(
                f"mount {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso /mnt/cdrom"
            )
            assert result.rc == 0
    ...
    def test_mountpoint_in_use_mnt(
        self, host, create_blank_iso, create_pallet_isos, revert_export_stack_pallets
    ):
        with ExitStack() as cleanup:
    ...
    def test_mounted_cdrom(self, host, create_pallet_isos, revert_export_stack_pallets):
        with ExitStack() as cleanup:
            ...
            print(result)
            result = host.run(
                f"mount --read-only {create_pallet_isos}/minimal-1.0-sles12.x86_64.disk1.iso /mnt/cdrom"
            )
            assert result.rc == 0
    ...
    def test_pallet_already_mounted(
        self, host, create_pallet_isos, revert_export_stack_pallets
    ):
    ...
    def test_network_iso(
        self, host, run_pallet_isos_server, revert_export_stack_pallets
    ):
        # Add the minimal pallet ISO from the network
        result = host.run(
            "stack add pallet http://127.0.0.1:8000/minimal-1.0-sles12.x86_64.disk1.iso"
        )
        assert result.rc == 0
    ...
    def test_network_directory(
        self, host, run_file_server, revert_export_stack_pallets
    ):
        # Add the minimal pallet directory from the network
        result = host.run(
            "stack add pallet http://127.0.0.1:8000/pallets/minimal/1.0/sles12/sles/x86_64"
        )
        assert result.rc == 0
    ...
    def test_network_directory_with_extra_slash(
        self, host, run_file_server, revert_export_stack_pallets
    ):
    ...
    def test_invalid_iso(self, host, create_blank_iso):
        ...
        assert result.stderr.startswith(
            f"error - The following arguments do not appear to be pallets: {create_blank_iso}/blank.iso"
        )
    ...
    def test_add_pallet_updates_url(
        self,
        host,
        run_pallet_isos_server,
        create_pallet_isos,
