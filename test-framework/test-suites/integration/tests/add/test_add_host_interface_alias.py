...
class TestAddHostInterfaceAlias:
    # split possible?
    def test_to_multiple_interfaces_across_multiple_hosts(
        self, host, revert_etc, test_file
    ):
        result = host.run(
            f'stack load hostfile file={test_file("add/add_host_alias_hostfile.csv")}'
        )
        assert result.rc == 0

        result = host.run(
            "stack add host interface alias backend-0-0 alias=test0-eth0 interface=eth0"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack add host interface alias backend-0-0 alias=test0-eth1 interface=eth1"
        )
        assert result.rc == 0
        result = host.run(
            "stack add host interface alias backend-0-1 alias=test1-eth0 interface=eth0"
        )
        assert result.rc == 0
        result = host.run(
            "stack add host interface alias backend-0-1 alias=test1-eth1 interface=eth1"
        )
        assert result.rc == 0
    ...
    def test_add_numeric_alias(self, host, add_host_with_interface):
        ...
        result = host.run(
            "stack add host interface alias backend-0-0 alias=42 interface=eth0"
        )
        assert result.rc != 0
    ...
    def test_add_duplicate_alias_same_host_interface(
        self, host, add_host_with_interface, test_file
    ):
        result = host.run(
            "stack add host interface alias backend-0-0 alias=test0-eth0 interface=eth0"
        )
    ...
    def test_add_duplicate_alias_same_host(self, host, revert_etc, test_file):
        ...
        assert result.rc == 0
        result = host.run(
            "stack add host interface alias backend-0-0 alias=test interface=eth0"
        )
        assert result.rc == 0
        result = host.run(
            "stack add host interface alias backend-0-0 alias=test interface=eth1"
        )
        assert result.rc == 0
    ...
    def test_add_duplicate_alias_different_host(self, host, revert_etc, test_file):
        ...
        result = host.run(
            "stack add host interface alias backend-0-1 alias=test0-eth0 interface=eth0"
        )
        assert result.rc != 0
    ...
    def test_add_multiple_aliases_same_host_interface(
        self, host, add_host_with_interface, test_file
    ):
        result = host.run(
            "stack add host interface alias backend-0-0 alias=test0-eth0 interface=eth0"
        ...
        assert result.rc == 0
        result = host.run(
            "stack add host interface alias backend-0-0 alias=2-test0-eth0 interface=eth0"
        )
        assert result.rc == 0
        ...
        with open(
            test_file("add/add_host_alias_multiple_aliases_same_host_interface.json")
        ) as output:
            expected_output = output.read()
        assert json.loads(result.stdout) == json.loads(expected_output)
    ...
    def test_hostname_in_use(self, host, add_host):
        result = host.run(
            "stack add host interface alias frontend-0-0 alias=backend-0-0 interface=eth0"
        )
        assert result.rc == 255
    ...
    def test_invalid_alias(self, host, add_host):
        result = host.run(
            "stack add host interface alias frontend-0-0 alias=127.0.0.1 interface=eth0"
        )
        assert result.rc == 255
    ...
    def test_invalid_interface(self, host, add_host):
        result = host.run(
            "stack add host interface alias frontend-0-0 alias=foo interface=eth7"
        )
        assert result.rc == 255
