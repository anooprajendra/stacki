...
class TestAddStorageController:
    ...
    def test_invalid_adapter(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 adapter=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_negative_adapter(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 adapter=-1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_enclosure(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 enclosure=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_negative_enclosure(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 enclosure=-1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_slot(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_existing_slot(self, host):
        # Add it once
        result = host.run(
            "stack add storage controller adapter=1 enclosure=2 slot=3 arrayid=4 raidlevel=0"
        )
        assert result.rc == 0
        ...
        assert result.rc == 255
        assert (
            result.stderr == 'error - disk specification for "1/2/3" already exists\n'
        )

    def test_invalid_hotspare(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 hotspare=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_negative_hotspare(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 hotspare=-1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_duplicate_hotspare(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=2 hotspare=1,1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_existing_hotspare(self, host):
        # Add it once
        result = host.run(
            "stack add storage controller adapter=1 enclosure=2 hotspare=3 arrayid=4 raidlevel=0"
        )
        assert result.rc == 0
    ...
    def test_hotspare_overlap_slots(self, host):
        result = host.run(
            "stack add storage controller arrayid=1 raidlevel=0 slot=1 hotspare=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_arrayid(self, host):
        result = host.run(
            "stack add storage controller arrayid=test raidlevel=0 slot=2 enclosure=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_arrayid_global_no_hotspares(self, host):
        result = host.run(
            "stack add storage controller arrayid=global raidlevel=0 slot=2 enclosure=1"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_stars(self, host):
        result = host.run(
            "stack add storage controller arrayid=* enclosure=1 slot=* raidlevel=5"
        )
        assert result.rc == 0
