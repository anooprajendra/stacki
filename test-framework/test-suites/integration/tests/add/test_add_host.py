...
class TestAddHost:
    ...
    HOST_POSITIVE_TEST_DATA = [
        ...
        (
            "backend-4-3",
            "appliance=backend rack=4 rank=3",
            "add_host_with_appliance_rack_rank.json",
        ),
        (
            "backend-7-7",
            "appliance=backend rack=5 rank=5",
            "add_host_with_standard_naming_and_appliance_rack_rank.json",
        ),
    ]

    @pytest.mark.parametrize(
        "host_name, add_params, output_file", HOST_POSITIVE_TEST_DATA
    )
    def test_positive_behaviour(
        self, host, host_name, add_params, output_file, test_file
    ):
    ...
    @pytest.mark.parametrize(
        "host_name, add_params, output_file", HOST_NEGATIVE_TEST_DATA
    )
    ...
    def test_invalid_box(self, host):
        result = host.run(
            "stack add host test appliance=backend rack=0 rank=0 box=test"
        )
        assert result.rc == 255
    ...
    def test_invalid_installaction(self, host, host_os):
        result = host.run(
            "stack add host test appliance=backend rack=0 rank=0 installaction=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == f'error - "test" install boot action for "{host_os}" is missing\n'
        )

    def test_invalid_osaction(self, host, host_os):
        result = host.run(
            "stack add host test appliance=backend rack=0 rank=0 osaction=test"
        )
        assert result.rc == 255
        assert (
            result.stderr
            == f'error - "test" os boot action for "{host_os}" is missing\n'
        )
    ...
    def test_with_environment(self, host, host_os):
        ...
        # Add our host
        result = host.run(
            "stack add host test appliance=backend rack=0 rank=0 environment=test"
        )
        assert result.rc == 0
