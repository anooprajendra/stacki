...
class TestAddHostInterface:
    ...
    def test_invalid_name(self, host):
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 name=test.example.com"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_invalid_vlan(self, host):
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 vlan=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_ip_auto_no_network(self, host):
        # Add the interface
        result = host.run(
            "stack add host interface backend-0-0 " "interface=eth0 ip=auto"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_duplicate_default_interface(self, host):
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth0 default=true"
        )
        assert result.rc == 0
        ...
        result = host.run(
            "stack add host interface backend-0-0 interface=eth1 default=true"
        )
        assert result.rc == 0
