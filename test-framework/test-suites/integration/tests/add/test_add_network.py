...
class TestAddNetwork:
    ...
    def test_space_in_network(self, host):
        result = host.run(
            'stack add network "test network" address=192.168.0.0 mask=255.255.255.0'
        )
        assert result.rc == 255
    ...
    def test_invalid_mtu(self, host):
        result = host.run(
            "stack add network test address=192.168.0.0 mask=255.255.255.0 mtu=test"
        )
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_existing_network(self, host):
        result = host.run(
            "stack add network private address=192.168.0.0 mask=255.255.255.0"
        )
        assert result.rc == 255
    ...
    def test_invalid_network(self, host):
        ...
        assert (
            result.stderr
            == "error - 321.0.0.0/255.255.255.0 is not a valid network address and subnet mask combination\n"
        )
