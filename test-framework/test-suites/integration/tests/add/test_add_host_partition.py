...
class TestAddHostPartition:
    ...
    def test_with_existing_info(self, host, add_host):
        ...
        result = host.run(
            "stack add host partition backend-0-0 device=sda mountpoint=/delete_me"
        )
        assert result.rc == 0

        result = host.run(
            "stack add host partition backend-0-0 device=sda mountpoint=/delete_me_too"
        )
        assert result.rc == 0
    ...
    def test_all_parameters(self, host, add_host):
        # Add the partition info
        result = host.run(
            "stack add host partition backend-0-0 device=sda "
