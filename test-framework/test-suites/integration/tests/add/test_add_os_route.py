...
class TestAddOSRoute:
    ...
    def test_no_os(self, host):
        result = host.run("stack add os route address=192.168.0.2 gateway=192.168.0.1")
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
			error - "os" argument is required
    ...
    def test_no_address(self, host):
        result = host.run("stack add os route ubuntu gateway=192.168.0.1")
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
    ...
    def test_no_gateway(self, host):
        result = host.run("stack add os route ubuntu address=192.168.0.2")
        assert result.rc == 255
        assert result.stderr == dedent(
            """\
