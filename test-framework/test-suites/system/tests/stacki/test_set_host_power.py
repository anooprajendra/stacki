...
class TestSetHostPower:
    ...
    def test_multiple_hosts(self, host):
        result = host.run(
            'stack set host power backend-0-0 backend-0-1 command="status"'
        )
        assert result.rc == 0
        power_count = 0
    ...
    def test_invalid_method(self, host):
        result = host.run(
            'stack set host power backend-0-0 command="status" method=invalid_method'
        )
        assert result.rc == 255

    def test_invalid_ipmi(self, host):
        result = host.run(
            'stack set host power backend-0-0 command="status" debug=y method=ipmi'
        )
        assert result.rc == 0
