...
def test_sync_host_network_hosts(host):
    ...
    def assert_files_diff():
        ...
        for this_host in hosts:
            ...
            result = host.run(
                rf"ssh {this_host} grep \'curatorial nonsense garbage\' /etc/hosts"
            )
            assert result.rc == 0
    ...
    result = host.run(
        f"""stack run host a:backend command='printf "\n# {garbage_str}\n" >> /etc/hosts' """
    )
    assert result.rc == 0
    # write the frontend's version for later comparison
    result = host.run(
        f"stack iterate host a:backend command='scp /etc/hosts %:/etc/hosts.fe'"
    )
    ...
    result = host.run(
        f"""stack iterate host a:backend command='grep "curatorial nonsense garbage" /etc/hosts' """
    )
    assert result.rc == 0
