...
result = subprocess.run(
    ["ip", "-oneline", "link", "show"], capture_output=True, universal_newlines=True
)

ndx = 0
for line in result.stdout.splitlines():
    device, link, mac = re.match(
        r"""
		\d+:\s+         # Index
		(\S+)           # Device name
    ...
    command.extend(["--header", f"X-RHN-Provisioning-MAC-{ndx}: {device} {mac}"])
...
for line in result.stdout.splitlines():
    if line.startswith("MAC Address"):
        ...
        command.extend(["--header", f"X-RHN-Provisioning-MAC-{ndx}: ipmi {mac}"])
...
command.append(
    f"https://{server}/install/sbin/profile.cgi" f"?os=sles&arch=x86_64&np={cpus}"
)
