...
def partition_init_path(element_partition, initialize, partition, partition_id):
    ...
    if initialize:
        if partition["size"] == 0:
            ...
            ElementTree.SubElement(element_partition, "size").text = (
                "%dM" % partition["size"]
            )
...
def partition_mount_uuid(
    element_partition, initialize, partition, mnt, format_partition
):
    ...
    if not format_partition and "uuid" in partition:
        ElementTree.SubElement(element_partition, "uuid").text = (
            "%s" % partition["uuid"]
        )
...
def do_partitions(disk, initialize, element_partition_list):
    ...
    for each_partition in partitions:
        if each_partition["device"] != disk:
            continue
        partitions_exist.append(
            output_partition(each_partition, initialize, element_partition_list)
        )
